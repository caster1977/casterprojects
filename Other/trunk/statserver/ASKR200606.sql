...
SELECT RIGHT(ani,7),DATE_FORMAT(datum,"%d%m%y"),TIME_FORMAT(vrijeme,"%H%i"),'70',LEFT(ddi,3),'3' FROM irda.irda_200606 i 
...
SELECT RIGHT(ani,7),DATE_FORMAT(datum,"%d%m%y"),TIME_FORMAT(vrijeme,"%H%i"),TIME_FORMAT(SEC_TO_TIME(ceiling(dur/60)*60),"%H%i"),'22',LEFT(ddi,3),'1' FROM irda.irda_200606 i 
...
FROM irda.irda_200606 i 
WHERE (datum<>'2006-06-01') AND (LEFT(ddi,3)='092') AND (dur>0) AND (v_mreza>'') AND (druga_info=0) AND (ani LIKE '00175______')
ORDER BY ddi,datum,vrijeme;
#095
INSERT INTO arj.new_report(numA,data,time,price,t_serv,t_tar)
SELECT RIGHT(ani,7),DATE_FORMAT(datum,"%d%m%y"),TIME_FORMAT(vrijeme,"%H%i"),'35',LEFT(ddi,3),'3'
...
INTO OUTFILE 'd:\\ASKR\\200606\\spr5006.009' FIELDS TERMINATED BY '|'  LINES TERMINATED BY '\n'
FROM arj.new_report r
LEFT JOIN arj.black b ON (r.numA LIKE b.phn)
WHERE b.phn IS null;

SELECT t_serv,count(*),sum(dur)
INTO OUTFILE 'd:\\ASKR\\200606\\spr5006.sta' FIELDS TERMINATED BY '|'  LINES TERMINATED BY '\n'
FROM arj.new_report r
LEFT JOIN arj.black b ON (r.numA LIKE b.phn)
WHERE b.phn IS null
GROUP BY t_serv;
...
SELECT RIGHT(ani,7),DATE_FORMAT(datum,"%d%m%y"),TIME_FORMAT(vrijeme,"%H%i"),'35',LEFT(ddi,3),'3'
FROM irda.irda_200606 i 
WHERE (datum<>'2006-06-01') AND (LEFT(ddi,3)='092') AND (dur>0) AND (v_mreza>'') AND (druga_info=0) AND ((ani LIKE '00172______') OR (ani LIKE '02______'))
ORDER BY ddi,datum,vrijeme;
#095
INSERT INTO arj.new_report(numA,data,time,price,t_serv,t_tar)
SELECT RIGHT(ani,7),DATE_FORMAT(datum,"%d%m%y"),TIME_FORMAT(vrijeme,"%H%i"),'35',LEFT(ddi,3),'3'
...
INTO OUTFILE 'd:\\ASKR\\200606\\088.unl' FIELDS TERMINATED BY '|'  LINES TERMINATED BY '\n'
FROM arj.new_report r
LEFT JOIN arj.black b on r.numA LIKE b.phn
WHERE b.phn IS null and t_serv='088';
#OTHER
SELECT cod,type,cat,numA,numB,data,time,dur,'0',v_serv,t_serv,t_tar,''
INTO OUTFILE 'd:\\ASKR\\200606\\009.unl' FIELDS TERMINATED BY '|'  LINES TERMINATED BY '\n'
FROM arj.new_report r
LEFT JOIN arj.black b on r.numA LIKE b.phn
WHERE b.phn IS null and t_serv!='088';

SELECT t_serv,count(*),sum(dur)
INTO OUTFILE 'd:\\ASKR\\200606\\009.sta' FIELDS TERMINATED BY '|'  LINES TERMINATED BY '\n'
FROM arj.new_report r
LEFT JOIN arj.black b on r.numA LIKE b.phn
WHERE b.phn IS null 
GROUP BY t_serv;
