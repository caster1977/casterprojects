...
SELECT DISTINCT RIGHT(ani,7),DATE_FORMAT(datum,"%d%m%y"),TIME_FORMAT(vrijeme,"%H%i"),'70',LEFT(ddi,3),'3' FROM irda.irda_200705 i 
...
SELECT DISTINCT RIGHT(ani,7),DATE_FORMAT(datum,"%d%m%y"),TIME_FORMAT(vrijeme,"%H%i"),TIME_FORMAT(SEC_TO_TIME(ceiling(dur/60)*60),"%H%i"),'22',LEFT(ddi,3),'1' FROM irda.irda_200705 i 
...
INTO OUTFILE 'd:\\ASKR\\200705\\spr5005.109' FIELDS TERMINATED BY '|'  LINES TERMINATED BY '\n'
FROM arj.new_report r
LEFT JOIN arj.black b ON (r.numA LIKE b.phn)
WHERE b.phn IS null;

SELECT t_serv,count(*),sum(dur)
INTO OUTFILE 'd:\\ASKR\\200705\\spr5005.sta' FIELDS TERMINATED BY '|'  LINES TERMINATED BY '\n'
FROM arj.new_report r
LEFT JOIN arj.black b ON (r.numA LIKE b.phn)
WHERE b.phn IS null
GROUP BY t_serv;
...
#188
SELECT cod,type,cat,numA,numB,data,time,dur,'0',CONCAT('17',LEFT(t_serv,1),'09'),t_serv,t_tar,''
INTO OUTFILE 'd:\\ASKR\\200705\\188.unl' FIELDS TERMINATED BY '|'  LINES TERMINATED BY '\n'
FROM arj.new_report r
LEFT JOIN arj.black b on r.numA LIKE b.phn
...
#OTHER
SELECT cod,type,cat,numA,numB,data,time,dur,'0',CONCAT('17',LEFT(t_serv,1),'09'),t_serv,t_tar,''
INTO OUTFILE 'd:\\ASKR\\200705\\109.unl' FIELDS TERMINATED BY '|'  LINES TERMINATED BY '\n'
FROM arj.new_report r
LEFT JOIN arj.black b on r.numA LIKE b.phn
WHERE b.phn IS null and t_serv!='188';

SELECT t_serv,count(*),sum(dur)
INTO OUTFILE 'd:\\ASKR\\200705\\109.sta' FIELDS TERMINATED BY '|'  LINES TERMINATED BY '\n'
FROM arj.new_report r
LEFT JOIN arj.black b on r.numA LIKE b.phn
WHERE b.phn IS null 
GROUP BY t_serv;
