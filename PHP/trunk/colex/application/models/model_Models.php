...
class model_models extends model
{
  public function sel_data($data = null)
  {
    ...
    $result = self::open("{call colex_sel_models (?, ?)}", array($id, null));
    try
    {
      $data = array();
  ...
  public function add_data($data = null)
  {
    ...
    if (isset($name))
    {
      ...
      $result = self::open("{call colex_upd_models (?, ?, ?)}", array($id, $name, $active));
      try
      {
  ...
  public function edit_data($data = null)
  {
    ...
    {
      $result = self::open("{call colex_upd_models (?, ?, ?)}", array($id, $name, (isset($active) ? 1 : 0)));
      try
      {
        if ($row = sqlsrv_fetch_array($result, SQLSRV_FETCH_ASSOC))
        {
  ...
  public function delete_data($data = null)
  {
    ...
    if (isset($id))
    {
      $result = self::open("{call colex_del_models (?)}", array($id));
      try
      {
