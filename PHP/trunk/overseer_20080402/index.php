...
session_start();
        ...
        //if (!session_register("user","password"))
        //  die("Не могу зарегестрировать переменные в сессии!");

require ("arrays.inc");

$base_url = "http://".getenv('HTTP_HOST')."/overseer/";

// если не указан шаг или шаг не попадает в промежуток [2..5] - устанавливаем его равным 1 (т.е переходим в начало)
if ((!(($step>=2)&&($step<=3)))||($step==""))
  $step=1;

// если не указан номер отчёта - устанавливаем отчёт номер 1
if ($rep=="")
  $rep=1;

// если отчёт выходит за пределы [1..кол-во отчётов] - устанавливаем отчёт номер 1
if (!(($rep>=1)&&($rep<=count($reports))))
  $rep=1;

// если шаг равен 3 - значит используем определённые для каждого из отчётов имена
// иначе - используем стандартную шапку
if ($step==3)
  $title=$reports[$rep]["Наименование"];
else $title="Формирование отчётов по работе справочно-информационного цеха МГТС";
...
switch ($step):
  ...
  case 2: // $step==2
    ...
    if ($action==""):
      if ($rep==1):
        $action="print";
      elseif (($rep==2)||($rep==3)||($rep==4)||($rep==5)||($rep==6)||($rep==7)||($rep==12)||($rep==14)||($rep==15)):
        $action="print";
      endif;
    endif;
    if (($rep==1)||($rep==2)||($rep==3)||($rep==4)||($rep==5)||($rep==14)||($rep==15))  // указание ориентации страницы для отчётов
      $orientation="книжная";
  ...
  case 3: // $step==3
    ...
    if ($rep<10):
      define('REPNUM',"step3rep0$rep.inc");
    else:
      define('REPNUM',"step3rep$rep.inc");
    endif;
    require(REPNUM);
  break;
endswitch;
