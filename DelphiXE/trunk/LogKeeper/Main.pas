...
begin
  ProcedureHeader('Процедура копирования текста выделенного элемента списка полученных сообщений в буфер обмена', LogGroupGUID);
...
begin
  if (((Configuration.bKeepErrorLog)and(aMessageType=lmtError))or((Configuration.bKeepWarningLog)and(aMessageType=lmtWarning))or((Configuration.bKeepInfoLog)and(aMessageType=lmtInfo))or((Configuration.bKeepSQLLog)and(aMessageType=lmtSQL))or
    ...
    begin
      ...
      ListItem.SubItems.Add(string(Configuration.LogServer.sMySQLUser)); // текущий залогированный пользователь
...
begin
          ...
          if hConnection=nil then
            ...
            begin
              ...
              if not bConnected then
                Routines_GenerateError('Возникла ошибка при попытке подключения к MySQL-серверу '+sMySQLHost+'!'+Routines_GetMySQLErrorInfo(LogServer), sErrorMessage, bError)
              else
  ...
  if bError then
    ...
    begin
      ...
      ApplicationEvents1.OnIdle:=ApplicationEvents1Idle; // начало работы обработчика отправки сообщений на сервер
    end;
    ...
    begin
      if bConnected then
        begin
          ...
          LogThis('<< Выполнение операции отключения от MySQL-сервера '+sMySQLHost+' завершено.', LogGroupGUID, lmtDebug);
        end;
    end;
...
var
  sl: TStringList;
begin
  ProcedureHeader('Процедура копирования текста SQL-запроса выделенного элемента списка полученных сообщений в буфер обмена', LogGroupGUID);
            ...
            begin
              ...
              if b and(j>-1) then
                ...
                begin
                  ...
                  Screen.Cursor:=crDefault;
                  MessageBox(Handle, PChar('Строк с GUID '+s+' ниже указанной в списке протокола более не найдено!'), PChar('LogKeeper - Внимание!'), MB_OK+MB_ICONWARNING+MB_DEFBUTTON1);
                  Application.ProcessMessages;
            ...
            begin
              ...
              if b and(j>-1) then
                ...
                begin
                  aCursor:=Screen.Cursor;
                  Screen.Cursor:=crDefault;
                  MessageBox(Handle, PChar('Строк с GUID '+s+' выше указанной в списке протокола более не найдено!'), PChar('LogKeeper - Внимание!'), MB_OK+MB_ICONWARNING+MB_DEFBUTTON1);
                  Application.ProcessMessages;
                  Screen.Cursor:=aCursor;
                end;
            end;
...
begin
  ...
  if ListBox1.Items.Count>0 then
    begin
        ...
        if not bError then
          begin
            ...
            if StrToTimeDef(sl[4], dt)=dt then
              Routines_GenerateError('Время сообщения ['+sl[4]+'] не соответствует допустимому!', sErrorMessage, bError);
          end;
        if not bError then
          if Length(sl[5])<>38 then
            Routines_GenerateError('GUID группы сообщения ['+sl[5]+'] не соответствует допустимому!', sErrorMessage, bError);
        if not bError then
          if StrToIntDef(sl[6], -1)=-1 then
            Routines_GenerateError('Индекс сообщения ['+sl[6]+'] не соответствует допустимому!', sErrorMessage, bError);
        if not bError then
          begin
      ...
      if bError then
        begin
          ListBox1.Items.Append(sMessage); // восстановление незаписаной строки в конец списка
          LogThis(sErrorMessage, LogGroupGUID, lmtError);
          Application.ProcessMessages;
...
var
  ...
  sl: TStringList;
begin
  ProcedureHeader('Процедура установки состояния доступности для действий локального меню списка полученных сообщений', LogGroupGUID);
  b:=False;
...
begin
  ...
  LogThis(PChar('Действие "'+Action_Connect.Caption+'" '+Routines_GetConditionalMessage(Action_Connect.Enabled, 'в', 'от')+'ключено, действие "'+Action_Disconnect.Caption+'" '+Routines_GetConditionalMessage(Action_Connect.Enabled, 'в',
        'от')+'ключено.'), LogGroupGUID, lmtInfo);
  Application.ProcessMessages;
...
begin
  if StatusBar1.Panels[STATUSBAR_MESSAGES_COUNT_PANEL_NUMBER].Text<>'Сообщений: '+IntToStr(ListBox1.Items.Count) then
    begin
...
begin
  ...
  MainForm.Width:=Screen.WorkAreaWidth; // установка ширины окна в ширину рабочего стола
  MainForm.Height:=Configuration.iMainFormHeight; // установка высоты окна
  MainForm.Left:=Screen.WorkAreaLeft; // установка левого угла в крайнюю левую точку рабочего стола
  if Configuration.bBottomMainFormAlignment then // установка окна в новые позиции
    MainForm.Top:=Screen.WorkAreaHeight-MainForm.Height
  else
    MainForm.Top:=Screen.WorkAreaTop;
  LogThis('Установлены следующие координаты и размеры главного окна: X='+IntToStr(MainForm.Left)+', Y='+IntToStr(MainForm.Top)+', W='+IntToStr(MainForm.Width)+', H='+IntToStr(MainForm.Height)+'.', LogGroupGUID, lmtInfo);
  if Configuration.bFullScreen then // максимизация окна при условии выставленной опции
    begin
...
begin
  ...
  if not((Configuration.bInterraptionInitiated)and(Configuration.wExitCicleCounter>=EXIT_COUNTER_MAX)) then
    begin
      LogThis('Завершение работы отменено всвязи с наличием запущенной клиентской программы. Будет выполнена минимизация главного окна.', LogGroupGUID, lmtWarning);
      Do_MinimizeMainForm;
      CanClose:=False;
    end
  else
  ...
  Configuration.bInterraptionInitiated:=True; // выставляем флаг инициации выхода из программы по нажатии кнопки выхода
...
begin
  ...
  if Msg.message=msgLogKeeperServer then
    begin
      ...
      Configuration.wExitCicleCounter:=0; // обнулить счётчик проверок наличия клиентов перед выходом, т.к. было получено сообщение от одной из клиентских программ
      Handled:=True;
    end;
  if Msg.message=msgLogKeeperClient then
    begin
      ...
      Configuration.bInterraptionInitiated:=False; // запретить выход из программы при достижении счётчика
      if Configuration.wExitCicleCounter>1 then
        LogThis('Процедура выхода из программы прервана всвязи с получением ответа от клиентской программы на '+IntToStr(Configuration.wExitCicleCounter*(word(ExitTimer.Interval div 1000)))+' сек.', LogGroupGUID, lmtWarning);
      Configuration.wExitCicleCounter:=0; // обнулить счётчик проверок наличия клиентов перед выходом, т.к. был получен ответ от одной из клиентских программ
      Handled:=True;
    end;
...
begin
  ...
  if Configuration.bInterraptionInitiated then
    LogThis('Был инициирован выход из программы! До выхода осталось '+IntToStr((EXIT_COUNTER_MAX-Configuration.wExitCicleCounter)*(word(ExitTimer.Interval div 1000)))+' сек.', LogGroupGUID, lmtWarning)
  else
    if Configuration.wExitCicleCounter>0 then
      LogThis('Ни одна клиентская программа не дала отклик на опрос! До выхода осталось '+IntToStr((EXIT_COUNTER_MAX-Configuration.wExitCicleCounter)*(word(ExitTimer.Interval div 1000)))+' сек.', LogGroupGUID, lmtWarning);
  if Configuration.wExitCicleCounter=EXIT_COUNTER_MAX then
    begin
      if Configuration.bInterraptionInitiated then
        ...
        LogThis('Производится завершение работы программы всвязи с отсутствием клиентских программ.', LogGroupGUID, lmtInfo);
    end;
  if Configuration.wExitCicleCounter<EXIT_COUNTER_MAX then
    begin
      Configuration.wExitCicleCounter:=Configuration.wExitCicleCounter+1;
      BroadcastSystemMessage(BSF_IGNORECURRENTTASK or BSF_POSTMESSAGE, @Recipients, msgLogKeeperClient, Handle, 0); // отправка бродкастового сообщения для очередной проверки наличия клиентов
    end
  else
    begin
...
begin
  ProcedureHeader('Процедура реакции на срабатывание таймера ответственного за повторное подключение к MySQL-серверу', LogGroupGUID);

  if Configuration.LogServer.bConnected then // // если уже было установлено соединение
