...
//  bDoubleSpace:=False;
//  bDoublePlus:=False;
  MainForm.LogThis('Первоначальное заполнение списка телефонов...',GroupGUID,lmtInfo);
  RefreshPhoneList;
  MainForm.LogThis('Окончание процедуры заполнения списка телефонов...',GroupGUID,lmtInfo);
  UpdateState;
		        ...
		        begin
			        ...
			        if ResultSet<>nil then
                      ...
                      for j:=0 to (i-1) do
                          ...
                          if not bError then
                            begin
      									      MainForm.LogThis('Попытка получения очередной строки выборки (#'+IntToStr(j)+').',GroupGUID,lmtInfo);
                                  ...
                                  ListItem.SubItems.Add(ResultRow[3]); // добавлен
                                  ListItem.SubItems.Add(ResultRow[4]); // кто добавил
                                  ListItem.SubItems.Add(ResultRow[8]); // приоритет
                                  ListItem.SubItems.Add(ResultRow[6]); // id
                                  ListItem.SubItems.Add(ResultRow[7]); // _phn_phone
                                  if ResultRow[0]='стационарный' then
                                    begin
                                      ListItem.ImageIndex:=0;
                                      ListItem.StateIndex:=-1;
                                    end
                                  else
                                    if ResultRow[0]='мобильный' then
                                      begin
                                        ListItem.ImageIndex:=1;
                                        ListItem.StateIndex:=2;
                                      end
                                    else
                                      if ResultRow[0]='мобильный (Velcom/МЦС)' then
                                        begin
                                          ListItem.ImageIndex:=1;
                                          ListItem.StateIndex:=4;
                                        end
                                      else
                                        if ResultRow[0]='мобильный (МТС)' then
                                          begin
                                            ListItem.ImageIndex:=1;
                                            ListItem.StateIndex:=3;
                                          end
                                        else
                                          if ResultRow[0]='мобильный (Diallog/БелСел)' then
                                            begin
                                              ListItem.ImageIndex:=1;
                                              ListItem.StateIndex:=6;
                                            end
                                          else
                                            if ResultRow[0]='мобильный (БеСТ)' then
                                              begin
                                                ListItem.ImageIndex:=1;
            ...
            begin
              ...
              if (i>=0) then
                begin
                  if i=1 then
                    begin
                      MainForm.LogThis('Количество строк полученной выборки соответствует требуемому (1).',GroupGUID,lmtInfo);
                      ResultRow:=mysql_fetch_row(ResultSet);
                      if ResultRow<>nil then
                        begin
...
begin
  ...
  MainForm.LogThis('Попытка открытия окна добавления телефона...',GroupGUID,lmtInfo);
  bError:=False;
	ResultSet:=nil;
  s_house_sym:='';
  s_subhouse_sym:='';
            ...
            begin
              ...
              if i=0 then
                begin
                  ...
                  if i=1 then
                    begin
                      MainForm.LogThis('Добавление новой записи в таблицу minsk.otdel произведено успешно',GroupGUID,lmtInfo);
                      i_otdel_id:=iLastID;
                    end
                  else
                    begin
            ...
            begin
              ...
              if i=0 then
                begin
                  ...
                  i:=mysql_affected_rows(MainForm.MySQLConnectionHandler);
                  if i=1 then
                    MainForm.LogThis('Добавление новой записи в таблицу _phn произведено успешно',GroupGUID,lmtInfo)
                  else
                    begin
                      s:='Количество обработанных строк ('+IntToStr(i)+') не соответствует требуемому (1)!';
                      bError:=True;
...
begin
  ...
  MainForm.LogThis('Попытка открытия окна изменения телефона...',GroupGUID,lmtInfo);
  bError:=False;
  ResultSet:=nil;
  i_phn_id:=-1;
  i_phn_otdel_id:=-1;
      ...
      if lvPhones.Selected<>nil then
        begin
          ...
          if i_phn_id>-1 then
            ...
            'LEFT JOIN minsk.otdel ON otdel_id=phn_otdel_id '+
            'LEFT JOIN '+MainForm.Configuration.sMySQLDatabase+'._usr ON usr_id=phn_modified_by_user_id WHERE phn_id='+IntToStr(i_phn_id)+';' // запрос на получение данных для заполнения полей формы
          else
            begin
            ...
            begin
              ...
              if i=0 then
                begin
                  ...
                  i:=mysql_affected_rows(MainForm.MySQLConnectionHandler);
                  if i=1 then
                    MainForm.LogThis('Изменение записи в таблице minsk.otdel произведено успешно',GroupGUID,lmtInfo)
                  else
                    begin
                      s:='Количество обработанных строк ('+IntToStr(i)+') не соответствует требуемому (1)!';
                      bError:=True;
            ...
            begin
              ...
              if i=0 then
                begin
                  ...
                  i:=mysql_affected_rows(MainForm.MySQLConnectionHandler);
                  if i=1 then
                    MainForm.LogThis('Изменение записи в таблице _phn произведено успешно',GroupGUID,lmtInfo)
                  else
                    begin
                      s:='Количество обработанных строк ('+IntToStr(i)+') не соответствует требуемому (1)!';
                      bError:=True;
      ...
      if not bError then
        begin
          ...
          if s_phone_num>'' then
            begin
              ...
              if hTargetWnd <> 0 then
                begin
                  ...
                  MainForm.LogThis('Произведена отправка во внешнюю программу следующей строки данных: ['+sParameter+']',GroupGUID,lmtInfo);
                end
              else
                begin
            ...
            begin
              s:='Был получен пустой номер телефона!';
              bError:=True;
            end;
           if bError then
            begin
