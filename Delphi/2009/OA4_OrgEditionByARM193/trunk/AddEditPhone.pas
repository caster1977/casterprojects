...
begin
  ...
  for i:=1 to Length(aString) do                                              // в цикле производим перебор всех символов строки-источника
    begin
      if not (aString[i] in ['0'..'9']) then                                  // если очередной символ не является цифрой
...
begin
  ...
  MainForm.SetBusy(GroupGUID);                                                  // включаем индикатор "занятости"
  ...
  MainForm.SetReady(GroupGUID);                                                 // включаем индикатор готовности
  Application.ProcessMessages;                                                  // обрабатывем накопившиеся системные сообщения
end;
...
begin
  sPhone:=                                                                      // получаем очищенную от символов-разделителей строку с номером телефона
    ...
    StringReplace(
    StringReplace(
    StringReplace(edbxPhoneNumber.Text,
    ' ','',[rfReplaceAll]),                                                     // очищаем от пробелов
    '(','',[rfReplaceAll]),                                                     // от круглых скобок
    ')','',[rfReplaceAll]),
    '-','',[rfReplaceAll]),                                                     // от знаков минуса
  ...
  if StringIsNumeric(sPhone) then                                               // если строка состоит из цифр
    begin
      ...
      if (Pos('0044',sPhone)=1) and (Length(sPhone)=Length('0044')+7) then
        cmbbxPhoneType.ItemIndex:=cmbbxPhoneType.Items.IndexOf('мобильный (Velcom/МЦС)')
      else cmbbxPhoneType.ItemIndex:=cmbbxPhoneType.Items.IndexOf('');
    end
  else cmbbxPhoneType.ItemIndex:=cmbbxPhoneType.Items.IndexOf('');              // если строка состоит не из цифр
end;
...
begin
  edbxPhoneNumber.Text:=
    ...
    StringReplace(
    StringReplace(
    Trim(edbxPhoneNumber.Text),
    '  ',' ',[rfReplaceAll]),                                                   // убираем двойные пробелы
...
procedure TAddEditPhoneForm.edbxPhonePriorityKeyPress(Sender: TObject;
  var Key: Char);
begin
...
begin
  if MainForm.CurrentUser.bDBEdition then                                       // если пользователь имеет права редактирования БД
    mePhoneComments.Text:=MainForm.ValidateString(mePhoneComments.Text);        // то выполняем валидацию значения строки поля ввода "Комментария"
end;
