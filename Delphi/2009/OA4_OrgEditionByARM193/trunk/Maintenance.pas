...
uses
  ...
  Dialogs, StdCtrls, ComCtrls, Main, mysql;
...
begin
  ...
  PChar('OA4 - Подтверждение импорта базы данных'),MB_YESNO+MB_ICONEXCLAMATION+MB_DEFBUTTON2)=IDYES;
  if not b then
    begin
  ...
  if not bError then
    begin
      ...
      q:='CREATE TABLE '+MainForm.Configuration.sMySQLDatabase+'._org ('+
        ...
        'KEY org_firm_code (org_firm_code), '+
        'FULLTEXT KEY org_work_time (org_work_time) '+
  ...
  if not bError then
    begin
      ...
      q:='CREATE TABLE '+MainForm.Configuration.sMySQLDatabase+'._msr ('+
        ...
        'KEY msr_unknown_duration (msr_unknown_duration), '+
        'FULLTEXT KEY msr_type (msr_type), '+
        'FULLTEXT KEY msr_name (msr_name), '+
        'FULLTEXT KEY msr_author (msr_author), '+
        'FULLTEXT KEY msr_producer (msr_producer), '+
        'FULLTEXT KEY msr_performer (msr_performer), '+
  ...
  if not bError then
    begin
      ...
      q:='CREATE TABLE '+MainForm.Configuration.sMySQLDatabase+'._phn ('+
        ...
        'phn_priority tinyint(3) unsigned NOT NULL default "0", '+
        ...
        'PRIMARY KEY  (phn_id), '+
        'KEY phn_priority (phn_priority), '+
        'KEY phn_owner_is_measure (phn_owner_is_measure), '+
        'KEY phn_owner_id (phn_owner_id), '+
        'KEY phn_otdel_id (phn_otdel_id), '+
        'KEY phn_type (phn_type), '+
        'KEY phn_compared (phn_compared), '+
  ...
  if not bError then
    begin
      ...
      q:='CREATE TABLE '+MainForm.Configuration.sMySQLDatabase+'._evn ('+
        ...
        'evn_by_user_id tinyint(3) unsigned NOT NULL default "0", '+
        'evn_details text NOT NULL, '+
        'PRIMARY KEY  (evn_id), '+
  ...
  if not bError then
    begin
      ...
      pbImportDetail.Max:=1;
      Application.ProcessMessages;
      q:='INSERT INTO '+MainForm.Configuration.sMySQLDatabase+'._org '+
        'SELECT a.id, '+ // org_id
        '"0", '+ // org_erased
  ...
  try
    if not bError then
      begin
        ...
        pbImportDetail.Position:=pbImportDetail.Min;
        Application.ProcessMessages;
        q:='SELECT CONCAT("INSERT INTO minsk.firma(name,main,city_id,street_id,comment,ext_comment) VALUES ('+#39+' ",TRIM(_org_name)," | ",TRIM(_org_type),'+      '"'+#39+','+#39+'!R'+#39+',2507,1077,'+#39+'",org_id,"'+#39+','+#39+'",_org_address,"'+#39+');") FROM '+MainForm.Configuration.sMySQLDatabase+'._org;';
        MainForm.LogThis(q,GroupGUID,lmtSQL);
        i:=mysql_real_query(MainForm.MySQLConnectionHandler,PChar(q),Length(q));
        if i=0 then
          begin
  ...
  if not bError then
    begin
      ...
      pbImportDetail.Max:=1;
      Application.ProcessMessages;
      q:='INSERT INTO '+MainForm.Configuration.sMySQLDatabase+'._msr '+
        'SELECT '+
      	'NULL, '+
      	'"0", '+
        ...
        'FROM rne2.measures a '+
      	...
      	'LEFT JOIN rne2.measureproducers e ON e.id=a.measureproducerid '+
        'ORDER BY a.id; ';
      MainForm.LogThis(q,GroupGUID,lmtSQL);
      i:=mysql_real_query(MainForm.MySQLConnectionHandler,PChar(q),Length(q));
      if i=0 then
...
procedure TMaintenanceForm.FormKeyUp(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
