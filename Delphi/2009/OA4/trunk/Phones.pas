...
begin
  ...
  if (GetWindowLong(h,GWL_STYLE) and WS_VSCROLL)=WS_VSCROLL then
    begin
      lvPhones.Column[2].Width:=
        lvPhones.Width - lvPhones.Column[0].Width - lvPhones.Column[1].Width - integer(b)*70 - (lvPhones.BevelWidth*2) - 2 - GetSystemMetrics(SM_CXVSCROLL);
    end
  else
    begin
      lvPhones.Column[2].Width:=
        lvPhones.Width - lvPhones.Column[0].Width - lvPhones.Column[1].Width - integer(b)*70 - (lvPhones.BevelWidth*2) - 2;
    end;
...
begin
  ...
  else MainForm.LogThis('Двойной щелчок сделан мимо элементов списка телефонов.',LogGroupGUID,lmtInfo);
...
begin
  ...
  else MainForm.LogThis('Список телефонов пуст.',LogGroupGUID,lmtInfo);
...
//                  'ORDER BY phn_priority, phn_type, phone;';
                  ...
                  'LEFT JOIN '+MainForm.Configuration.sMySQLDatabase+'._org ON minsk.otdel.firm_code=_org.org_firm_code '+
                  'LEFT JOIN '+MainForm.Configuration.sMySQLDatabase+'._phn ON _phn.phn_otdel_id=otdel.otdel_id '+
                  'LEFT JOIN '+MainForm.Configuration.sMySQLDatabase+'._usr ON _usr.usr_id=otdel.oper '+
                  'WHERE ((!(_phn.phn_owner_is_measure="1")) OR ISNULL(_phn.phn_owner_is_measure)) AND _org.org_id='+IntToStr(owner_id)+' '+
                  'ORDER BY otdel.sort_level, otdel.phn_owner, otdel.phone;';
                end
              else
                begin
                  s:='Возникла ошибка в процессе получения ID организации-владельца!';
