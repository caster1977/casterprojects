...
uses
  ...
  Dialogs, ActnList, ComCtrls, StdCtrls, Buttons, ExtCtrls, AppEvnts,
...
type
  ...
  public
    ...
    procedure LogThis(const a: AnsiString; const sGroupGIUD: string; const MessageType: TLogMessagesType);
  ...
  function CutFirstBrace(aAnsiString: AnsiString): AnsiString;
  function CutLastBrace(aAnsiString: AnsiString): AnsiString;
  function CutBraces(aAnsiString: AnsiString): AnsiString;
...
begin
  if Condition then
    Result:=TrueMessage
  else Result:=FalseMessage;
end;

function CutFirstBrace(aAnsiString: AnsiString): AnsiString; // обрезать всё с начала до первой скобки включительно
var
  aPos: integer;
begin
  aPos:=Pos('{', aAnsiString);
  Result:=Copy(aAnsiString,aPos+1,Length(aAnsiString)-aPos-1);
end;

function CutLastBrace(aAnsiString: AnsiString): AnsiString; // обрезать всё с конца до последней скобки включительно
var
  aPos: integer;
begin
  aPos:=Pos('}',AnsiReverseString(aAnsiString));
  Result:=Copy(aAnsiString,1,Length(aAnsiString)-aPos-1);
end;

function CutBraces(aAnsiString: AnsiString): AnsiString;
begin
  Result:=CutLastBrace(CutFirstBrace(aAnsiString));
end;
...
begin
  ...
  else PageControl1.ActivePage:=tsClear;
end;
            ...
            begin
              if not bError then
                begin
                                ...
                                if ResultRow<>nil then
                                  begin
                                    if (ResultRow[0]='1') then
					                  ...
					                  else GenerateError('Получено некорректное количество строк результирующей выборки ('+IntToStr(i)+')!',s,bError);
                          end
                        else GenerateError('Не удалось получить результирующую выборку по последнему SQL-запросу!',s,bError);
                      finally
                        if ResultSet<>nil then
                          begin
                            // LogThis('Освобождение ресурсов, занятых результатом выборки...',LogGroupGUID,lmtInfo);
                  ...
                  else GenerateError('Возникла ошибка при выполнении запроса!',s,bError);
                  ProgressBar1.StepIt;
                end;
            end;
          lvOrg.Items.EndUpdate;
                                ...
                                begin
                                  ...
                                  if i=0 then
                                    begin
                                      ...
                                      else GenerateError('Количество обработанных строк ('+IntToStr(i)+') не соответствует требуемому (1)!',s,bError);
                                    end
                                  else GenerateError('Возникла ошибка при выполнении запроса!',s,bError);
                                end
                              else GenerateError('Количество обработанных строк ('+IntToStr(i)+') не соответствует требуемому (1)!',s,bError);
          ...
          if lvOrg.Selected<>nil then
            if lvOrg.Selected.ImageIndex=DELETEDICON_ID then
              begin
                ...
                b:=MessageBox(Handle,PChar('Вы пытаетесь воостановить мероприятие помеченной на удаление организации.'+#13#10+
                  'Данная операция невозможна без восстановления самой организации.'+#13#10+
...
begin
  ...
  miActions.Visible:=Action_DB_Backup.Visible or Action_DB_Cleaning.Visible or Action_DB_Defragmentation.Visible or
    Action_DB_Maintenance.Visible or Action_DB_AddMeasure.Visible or Action_DB_DeleteNode.Visible or
    Action_DB_RecoverNode.Visible or Action_DB_RefreshList.Visible or Action_PostMessage.Visible or
...
var
  ...
  GroupId: integer;
  sExternalLog_All,
  sExternalLog_Dates, sExternalLog_Dates_Start, sExternalLog_Dates_Stop,
  sExternalLog_Types, sExternalLog_Types_1, sExternalLog_Types_2, sExternalLog_Types_3, sExternalLog_Types_4, sExternalLog_Types_5,
  sExternalLog_Names: string;
  sSym: string;
  iNum: integer;
begin
	bError:=False;
  ...
  if not bError then
    begin
      q:='SELECT '+
        'UCASE(TRIM(SUBSTRING_INDEX(m_f.name,"|",-1))) AS x0, '+ // тип организации
        ...
        GetConditionalMessage((chkbxHideErased.Visible and chkbxHideErased.Enabled and chkbxHideErased.Checked) or (not chkbxHideErased.Visible), 'AND (r_m.msr_erased="0" OR ISNULL(r_m.msr_erased)) ', '')+
        GetConditionalMessage(chkbxHideOld.Enabled and chkbxHideOld.Checked, 'AND ((r_m.msr_stop_datetime>="'+FormatDateTime('yyyy-mm-dd',Now)+' 00:00:00") OR ISNULL(r_m.msr_stop_datetime)) ', '')+ // если установлен флаг "скрывать устаревшие" выбрать только те мероприятия у которых дата/время начала не указаны или больше чем начало текущих суток
        ...
        case Configuration.iOrgSortColumn of
          1: q:=q+'GROUP BY x4 ORDER BY x0, x7, x1, x3;';
          2: q:=q+'GROUP BY x4 ORDER BY x0, x3, x1, x7;';
          else q:=q+'GROUP BY x4 ORDER BY x0, x1, x7, x3;';
        end;
      LogThis(q,LogGroupGUID,lmtSQL);
    end;
                    ...
                    begin
                      if not bError then
                        begin
                              ...
                              if GroupId>-1 then
                                begin
                                  ...
                                  if Trim(ResultRow[7])>'' then
                                    begin
                                      if (Pos('ПРОСПЕКТ',Trim(ResultRow[7]))>1) or // если название улицы не содержит данного фрагмента в начале или вообще не содержит
                                        (Pos('БУЛЬВАР',Trim(ResultRow[7]))>1) or
                                        (Pos('ПРОЕЗД',Trim(ResultRow[7]))>1) or
                                        (Pos('ПЕРЕУЛОК',Trim(ResultRow[7]))>1) or
                                        (Pos('ПОСЕЛОК',Trim(ResultRow[7]))>1) or
                                        (Pos('ШОССЕ',Trim(ResultRow[7]))>1) or
                                        (Pos('ТУПИК',Trim(ResultRow[7]))>1) or
                                        (Pos('ТРАКТ',Trim(ResultRow[7]))>1) or
                                        (Pos('МИКРОРАЙОН',Trim(ResultRow[7]))>1) or
                                        (Pos('ПЛОЩАДЬ',Trim(ResultRow[7]))>1) or
                                        (Pos('МЕТРО',Trim(ResultRow[7]))>1) or
                                        (Pos('КОЛЬЦО',Trim(ResultRow[7]))>1) or
                                        (Pos('ДЕРЕВНЯ',Trim(ResultRow[7]))>1) then
                                        begin
                          ...
                          else GenerateError('Возникла ошибка при загрузке данных строки выборки!',s,bError);
                          ProgressBar1.StepIt;
                        end;
                    end;
                  lvOrg.Items.EndUpdate;
  ...
  sExternalLog_Dates:=
    GetConditionalMessage((sExternalLog_Dates_Start+sExternalLog_Dates_Stop)>'','Период времени: ','')+
    sExternalLog_Dates_Start+
  ...
  sExternalLog_Names:=GetConditionalMessage( // получение строки для первого поля поиска
    edbxSearchName.Text>'',
    GetConditionalMessage(not Configuration.bShowSearchAddressOtherEditBox,'Поиск по всем полям: ','Поиск по типу и наименованию: ')+edbxSearchName.Text,
    '');
  ...
  if bError then
    ShowErrorBox(s, LogGroupGUID)
  else LogThis('Процедура выполнена без ошибок.',LogGroupGUID,lmtInfo);
    ...
    begin
      ...
      if lvMsr.Selected<>nil then
        begin
          ...
          else GenerateError('Возникла ошибка в процессе получения ID мероприятия-владельца!',s,bError);
          if not bError then
            begin
              //очистка значений контролов
              Clear_ID_Creation_Modification_Labels;
            ...
            begin
              ...
              if not bError then
                begin
                  ...
                  else GenerateError('Возникла ошибка в процессе получения ID организации-владельца!',s,bError);
                  if bError then
                    ShowErrorBox(s, LogGroupGUID)
                  else
                    begin
                ...
                begin
                  if i_firm_code>-1 then
                    begin
                      ...
                      else GenerateError('Не найдена програма-получатель данных!',s,bError);
                    end
                  else GenerateError('Был получен некорректный номер идентификатора фирмы!',s,bError);
			...
			if ResultSet<>nil then
              ...
              if not bError then
                cmbbxMsrType.ItemIndex:=cmbbxMsrType.Items.IndexOf(s1)
					...
					else GenerateError('Получено недопустимое количество строк результирующей выборки ('+IntToStr(i)+')!',s,bError);
				end
			else GenerateError('Не удалось получить результирующую выборку по последнему SQL-запросу!',s,bError);
	    if ResultSet<>nil then
		    begin
...
begin
  ...
  if not Configuration.bImmediatelyQuit then
    begin
      Screen.Cursor:=crDefault;
      if Configuration.bDontShowQuitConfirmation
        then b:=True
      else b:=MessageBox(Handle,PChar('Вы действительно хотите завершить работу программы?'),PChar('OA4 - Подтверждение выхода'),MB_OKCANCEL+MB_ICONQUESTION+MB_DEFBUTTON2)=IDOK;
      Screen.Cursor:=crHourGlass;
...
begin
  ...
  if ((ch0 in ['1'..'7']) and (cText[1]=':')) then
    begin
      ...
      case ch0 of
        '1': LogThis(PChar('Получена команда получения информации об организации с кодом фирмы равным ['+sData+'].'),LogGroupGUID,lmtInfo); // INFO_GET_FIRMA_BY_CODE		=1;
        '2': LogThis(PChar('Получена команда получения информации о мероприятии с кодом отдела равным ['+sData+'].'),LogGroupGUID,lmtInfo); // INFO_GET_OTDEL_BY_CODE		=2;
  ...
  if Configuration.bUseMultibuffer then
    begin
      ...
      try
        ...
        raMultiBuffer[j].MsrOtherInfo:=TextStream.DataString;
      ...
      if j>-1 then
        ...
        else ShowErrorBox('Произошла ошибка при попытке сохранения данных о мероприятии в мультибуфере!', LogGroupGUID)
      else ShowErrorBox('Произошла ошибка при попытке сохранения данных о мероприятии в мультибуфере! Буфер заполнен. Выполните предварительную очистку!', LogGroupGUID);
    end
  else
    begin
      MsrBufferRec.bStoredDataExists:=False;
      ...
      try
        reMsrOtherInfo.Lines.SaveToStream(TextStream);
        MsrBufferRec.MsrOtherInfo:=TextStream.DataString;
      ...
      else ShowErrorBox('Произошла ошибка при попытке сохранения данных о мероприятии во временном буфере!', LogGroupGUID);
    end;
        ...
        begin
          ...
          if i_org_id>-1 then
            begin
              ...
              'LEFT JOIN minsk.firma ON firma.firm_code=otdel.firm_code '+
              'LEFT JOIN '+Configuration.sMySQLDatabase+'._org ON _org.org_firm_code=firma.firm_code '+
              'LEFT JOIN '+Configuration.sMySQLDatabase+'._phn ON _phn.phn_otdel_id=otdel.otdel_id '+
              'WHERE  ((!(_phn.phn_owner_is_measure="1")) OR ISNULL(_phn.phn_owner_is_measure)) AND _org.org_id='+IntToStr(i_org_id)+';';
      ...
      if lvOrg.Selected<>nil then
        begin
          ...
          if not bError then
            begin
              q:='SELECT '+
              'UCASE(TRIM(r_m.msr_type)) AS x0, '+ // тип мероприятия
              ...
              GetConditionalMessage((chkbxHideErased.Visible and chkbxHideErased.Enabled and chkbxHideErased.Checked) or (not chkbxHideErased.Visible), 'AND r_m.msr_erased="0" ', '')+
              GetConditionalMessage(chkbxHideOld.Enabled and chkbxHideOld.Checked, 'AND ((r_m.msr_stop_datetime>="'+FormatDateTime('yyyy-mm-dd',Now)+' 00:00:00") OR ISNULL(r_m.msr_stop_datetime)) ', '')+
              GetConditionalMessage(chkbxStartDate.Visible and chkbxStartDate.Enabled and chkbxStartDate.Checked, 'AND (ISNULL(r_m.msr_stop_datetime) OR (r_m.msr_stop_datetime>="'+FormatDateTime('yyyy-mm-dd hh:nn:ss',dtpStartDate.DateTime)+'")) ', '')+ // будут отобраны все мероприятия не окончившиеся на указанную дату
              GetConditionalMessage(chkbxStopDate.Visible and chkbxStopDate.Enabled and chkbxStopDate.Checked, 'AND (ISNULL(r_m.msr_start_datetime) OR (r_m.msr_start_datetime<="'+FormatDateTime('yyyy-mm-dd hh:nn:ss',dtpStopDate.DateTime)+'")) ', '')+ // будут указаны все мероприятия начинающиеся не позднее указанной даты
              GetConditionalMessage(chkbxPremier.Visible and chkbxPremier.Enabled and chkbxPremier.Checked, 'AND r_m.msr_is_premier="1" ', '')+
              GetConditionalMessage(chkbxTour.Visible and chkbxTour.Enabled and chkbxTour.Checked, 'AND r_m.msr_is_tour="1" ', '')+
              GetConditionalMessage(chkbxChildren.Visible and chkbxChildren.Enabled and chkbxChildren.Checked, 'AND r_m.msr_for_children="1" ', '')+
              GetConditionalMessage(chkbxTeenagers.Visible and chkbxTeenagers.Enabled and chkbxTeenagers.Checked, 'AND r_m.msr_for_teenagers="1" ', '')+
              GetConditionalMessage(chkbxAdultsOnly.Visible and chkbxAdultsOnly.Enabled and chkbxAdultsOnly.Checked, 'AND r_m.msr_for_adults_only="1" ', '');
              ...
              case Configuration.iMsrSortColumn of
                1: q:=q+'r_m.msr_start_datetime, x1, r_m.msr_stop_datetime';
                2: q:=q+'r_m.msr_stop_datetime, x1, r_m.msr_start_datetime';
                else q:=q+'x1, r_m.msr_start_datetime, r_m.msr_stop_datetime';
              end;
                                      ...
                                      ResultRow:=mysql_fetch_row(ResultSet);
                                          ...
                                          if sMsrID<>ResultRow[4] then
                                            begin
                                                ...
                                                begin
                                                  ...
                                                  if ResultRow[24]<>'' then
                                                    begin
                                                      ...
                                                      HTMLtoRTF(sMsrOtherInfoRTF,reMsrOtherInfo);
                                                    end;
                                                  TextStream:=TStringStream.Create('');
                                                  try
                                                    reMsrOtherInfo.Lines.SaveToStream(TextStream);
                                                    sMsrOtherInfoRTF:=TextStream.DataString;
                                                  finally
                                                    TextStream.Free;
                                                  end;
                                                end
                                              ...
                                              {iPhonesCounter:=1;}
                                              sPhones:='';
                                              if Trim(''+ResultRow[30]+ResultRow[31]+ResultRow[32])>'' then
                                                sPhones:={IntToStr(iPhonesCounter)+'.'+#09+}ResultRow[30]+#09+ResultRow[31]+#09+ResultRow[32];
                                            end
                                          else
                                            begin
                                              // добавление нового телефона к значению переменной sPnones;
                                              if (sPhones>'') and (Trim(''+ResultRow[30]+ResultRow[31]+ResultRow[32])>'') then
                                                sPhones:=sPhones+#13#10;
                                              sPhones:=sPhones+ResultRow[30]+#09+ResultRow[31]+#09+ResultRow[32];
                                            end;
                                          if j=(i-1) then
          ...
          if not bError then
            begin
			        q:='SELECT '+
              'CONCAT(" ",TRIM(SUBSTRING_INDEX(m_f.name,"|",-1))), '+
              'CONCAT(" ",TRIM(SUBSTRING_INDEX(m_f.name,"|",1))), '+
              'r_o._org_address, '+
              'm_c.name, '+
              'm_s.name, '+
              ...
              'm_f.flat_num, '+
              'm_f.flat_sym, '+
              'r_o.org_how_to_reach, '+
              'IFNULL(r_o.org_work_time,m_f.work), '+
              'r_o.org_other_info, '+
              'r_o.org_other_info_rtf, '+
              'CONCAT(r_uc.usr_fullname,", ",r_o.org_created_at), '+
              'CONCAT(r_um.usr_fullname,", ",r_o.org_modified_at) '+
              ...
              'LEFT JOIN '+Configuration.sMySQLDatabase+'._usr r_uc ON r_uc.usr_id=r_o.org_created_by_user_id '+
              'LEFT JOIN '+Configuration.sMySQLDatabase+'._usr r_um ON r_um.usr_id=r_o.org_modified_by_user_id '+
              'WHERE r_o.org_id='+IntToStr(i_org_id)+';';
			        LogThis(q,LogGroupGUID,lmtSQL);
            end;
...
begin
  ...
  else btbtnPasteFromMsrBuffer.Enabled:=(PageControl1.ActivePage=tsMsr) and MsrBufferRec.bStoredDataExists and CurrentUser.bDBEdition;
end;
...
begin
  ...
  else ShowErrorBox('Справочный файл к данной программе не найден.', LogGroupGUID);
  ...
  with OptionsForm do
    try
                    ...
                    begin
                      ...
                      for j:=0 to i-1 do
                        begin
                          ...
                          else GenerateError('Возникла ошибка при загрузке данных строки выборки (порядковый номер строки равен '+IntToStr(j)+')!',s,bError);
                          ProgressBar1.StepIt;
                        end;
                    end
                  else GenerateError('Возникла ошибка при получении колчиества срок результирующей выборки!',s,bError);
                end
              else GenerateError('Не удалось получить результирующую выборку по последнему SQL-запросу!',s,bError);
            end
          else GenerateError('Возникла ошибка при выполнении запроса!',s,bError);
      ...
      if edbxAddMassMsrFormPositionX.Enabled then
        edbxAddMassMsrFormPositionX.Text:=IntToStr(Configuration.fpAddMassMsrForm.x)
      else edbxAddMassMsrFormPositionX.Clear;
      ...
      if edbxAddMassMsrFormPositionY.Enabled then
        edbxAddMassMsrFormPositionY.Text:=IntToStr(Configuration.fpAddMassMsrForm.y)
      else edbxAddMassMsrFormPositionY.Clear;
        ...
        begin
          ...
          if sgAutoReplaceList.Visible then
            begin
              ...
              else GenerateError('Возникла ошибка при очистке списка автозамены!',s,bError);
              if bError then
                ShowErrorBox(s, LogGroupGUID)
              else LogThis('Процедура выполнена без ошибок.',LogGroupGUID,lmtInfo);
            end;
        ...
        begin
          ...
          if ResultSet<>nil then
            begin
              ...
              else GenerateError('В системе не зарегестрироывано ни одного пользователя!',s,bError);
            end
          else GenerateError('Не удалось получить результирующую выборку по последнему SQL-запросу!',s,bError);
        end
      else GenerateError('Возникла ошибка при выполнении запроса!',s,bError);
              ...
              begin
                ...
                if ResultSet<>nil then
                  begin
                    ...
                    else GenerateError('В системе не зарегестрироывано ни одного пользователя с такими правами!',s,bError);
                  end
                else GenerateError('Не удалось получить результирующую выборку по последнему SQL-запросу!',s,bError);
              end
            else GenerateError('Возникла ошибка при выполнении запроса!',s,bError);
...
begin
  ...
  with TAboutForm.Create(Self) do
    try
      Button1.Visible:=not aIsPopupState;
      PreShowModal(sModalWinName, LogGroupGUID);
      ShowModal;
    finally
      PostShowModal(sModalWinName, LogGroupGUID);
        ...
        begin
          ...
          if ResultSet<>nil then
            begin
              ...
              else GenerateError('Возникла ошибка при получении количества строк выборки!',s,bError);
            end
          else GenerateError('Не удалось получить результирующую выборку по последнему SQL-запросу!',s,bError);
        end
      else GenerateError('Возникла ошибка при выполнении запроса!',s,bError);
...
begin
  ...
  else LogThis('Действие отменено, так как подключение к базе данных не было выполнено!',LogGroupGUID,lmtWarning);
                      ...
                      begin
                        ...
                        if ResultSet<>nil then
                          begin
                            ...
                            else GenerateError('Формирование отчётов невозможно, так как в таблице статистических данных не было найдено ни одной записи!',s,bError);
                          end
                        else GenerateError('Не удалось получить результирующую выборку по последнему SQL-запросу!',s,bError);
                      end
                    else GenerateError('Возникла ошибка при выполнении запроса!',s,bError);
                                          ...
                                          begin
                                            ...
                                            if ResultRow<>nil then
                                              begin
                                                ...
                                                q:=q+GetConditionalMessage(ResultRow[3]>'', ' ', '')+

                                                  GetConditionalMessage(StrToIntDef(ResultRow[4],-1)=0,

                                                    GetConditionalMessage(ResultRow[3]='msr', ' группы мероприятий', ''),

                                                    GetConditionalMessage(ResultRow[3]='usr', 'пользователя', '')+
                                                    GetConditionalMessage(ResultRow[3]='phn', 'телефона', '')+
                                                    GetConditionalMessage(ResultRow[3]='msg', 'сообщения', '')+
                                                    GetConditionalMessage(ResultRow[3]='org', 'организации', '')+
                                                    GetConditionalMessage(ResultRow[3]='msr', 'мероприятия', '')+
                                                    GetConditionalMessage(StrToIntDef(ResultRow[4],-1)>-1, ' ID #'+ResultRow[4], '')
                                                  );
                                                HTML:=HTML+'          <TD ';
                                                if not chkbxShowSQLQuery.Checked then
                                                  HTML:=HTML+'COLSPAN="2" ';
              ...
              try
                ...
                if r>0 then
                  begin
                    ...
                    else GenerateError('Возникла ошибка при сохранении отчёта во временный файл '+#39+q+#39+' - указанный файл уже существует!',s,bError);
                  end
                else GenerateError('Возникла ошибка при получении пути временного каталога!',s,bError);
                if bError then
                  ShowErrorBox(s, LogGroupGUID);
              finally
...
begin
  if Key=VK_RETURN then
    begin
      ...
      (stPhonesMark.Visible and (stPhonesMark.Font.Color=clRed) and (stPhonesMark.Caption='ЕСТЬ ТЕЛЕФОНЫ')) then
        Show_OrgPhonesForm;
    end;
        ...
        begin
          if lvOrg.Selected<>nil then // определяем ID организации
            begin
              ...
              else GenerateError('Возникла ошибка при получении ID организации-владельца создаваемого мероприятия!',s,bError);
              ProgressBar1.Max:=5;
            end
          else
            begin
        ...
        begin
          if lvOrg.Selected<>nil then // определяем ID организации
            begin
              ...
              else GenerateError('Возникла ошибка при получении ID организации-владельца редактируемого мероприятия!',s,bError);
              ProgressBar1.Max:=5;
            end
          else
            begin
          ...
          if lvMsr.Selected<>nil then // определяем ID редактируемого мероприятия
            begin
              ...
              else GenerateError('Возникла ошибка при получении ID редактируемого мероприятия!',s,bError);
              ProgressBar1.Max:=2;
            end
          else
            begin
        ...
        begin
          if bAddingMsrNow then
            ...
            begin
              ...
              GetConditionalMessage(chkbxMsrStart.Checked, 'msr_start_datetime="'+FormatDateTime('yyyy-mm-dd hh:nn:ss',dtpMsrStartDate.DateTime)+'", ', 'msr_start_datetime=NULL, ')+
              GetConditionalMessage(chkbxMsrStop.Checked, 'msr_stop_datetime="'+FormatDateTime('yyyy-mm-dd hh:nn:ss',dtpMsrStopDate.DateTime)+'", ', 'msr_stop_datetime=NULL, ')+
              'msr_unknown_duration="'+IntToStr(integer(chkbxMsrUnknownDuration.Checked))+'", '+ // msr_unknown_duration
          ...
          try
            ...
            if not bError then
              begin
                ...
                else GenerateError('Произошла ошибка при составлении списка "сеансов"!',s,bError);
              end;
                  ...
                  begin
                    with MsrBuffer do
                      begin
                        ...
                        else GenerateError('Возникла ошибка при получении ID организации-владельца дублируемого мероприятия!',s,bError);
                      end;
                  end
                else GenerateError('Возникла ошибка при получении ID организации-владельца дублируемого мероприятия!',s,bError);
              end;
              ...
              begin
                if lvMsr.Selected<>nil then // определяем ID организации
                  begin
                    ...
                    else GenerateError('Возникла ошибка при получении ID мероприятия-образца!',s,bError);
                  end
                else GenerateError('Возникла ошибка при получении ID мероприятия-образца!',s,bError);
              end;
              ...
              begin
                with MsrBuffer do
                  begin
                    ...
                    MsrOtherInfoPlane:=ValidateString(reMsrOtherInfo.Text);
                    TextStream:=TStringStream.Create('');
                    try
                      reMsrOtherInfo.Lines.SaveToStream(TextStream);
                      MsrOtherInfo:=TextStream.DataString;
                    ...
                    else GenerateError('Произошла ошибка при попытке получения данных о мероприятии-образце!',s,bError);
                  end;
              end;
                          ...
                          begin
                            ...
                            if not bError then
                              begin
                                ...
                                'msr_created_at, msr_created_by_user_id, msr_modified_at, msr_modified_by_user_id) VALUES ('+
                                'NULL, "0", '+ // msr_id, msr_erased
                                GetConditionalMessage(MsrHasStartDateTime, '"'+FormatDateTime('yyyy-mm-dd hh:nn:ss',MsrStartDateTime)+'", ', 'NULL, ')+
                                GetConditionalMessage(MsrHasStopDateTime, '"'+FormatDateTime('yyyy-mm-dd hh:nn:ss',MsrStopDateTime)+'", ', 'NULL, ')+
                                '"'+IntToStr(integer(MsrUnknownDuration))+'", '+ // msr_unknown_duration
                                ...
                                GetConditionalMessage(Trim(MsrOtherInfoPlane)='','NULL, NULL, ', NormalizeStringForQuery(MsrOtherInfoPlane,True,True)+', '+NormalizeStringForQuery(MsrOtherInfo,True,True)+', ')+
...
begin
  ...
  with PhonesForm do
    try
      ...
      else GenerateError('Не удалось определить выделенную организацию!',s,bError);
...
procedure TMainForm.LogThis(const a: AnsiString; const sGroupGIUD: string; const MessageType: TLogMessagesType);
var
  m: AnsiString;
  s, u: string;
  hLogClient: HWND;
...
begin
  ...
  else MessageBox(Handle,PChar('Не удалось отправить сообщение программе лог-клиента, так как не было найдено окно программы лог-клиента!'+#13#10+'Обратитесь к администратору!'),PChar('OA4 - Ошибка!'),MB_OK+MB_ICONERROR);
end;
