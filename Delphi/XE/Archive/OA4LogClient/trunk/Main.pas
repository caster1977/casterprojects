...
type
  THackControl= class(TControl);
    TMainForm= class(TForm)StatusBar1: TStatusBar;
    ...
    procedure VDTrayIcon1RightClick(Sender: TObject; Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
...
begin
  ...
  if (FindWindow('TMainForm', 'OA4')<>0) then // если не было найдено окно операторской программы
    begin
      MessageBox(MainForm.Handle, PChar('Работу данной программы завершать нельзя, т.к. в'+#10#13+'данный момент она используется программой "OA4"!'), PChar(MainForm.Caption+' - '+'Внимание!'), MB_OK+MB_ICONWARNING+MB_DEFBUTTON1);
      CanClose:=False;
    end
    ...
    begin
      ...
      if not bStartingNormalShutdown then // если в итоге переменная нормального завершения работы так и не установлена
        begin
          ...
          if MessageBox(Application.Handle, PChar('Работа приложения "OA4" была некорректно завершена!'+#13#10+'Вы хотите вновь запустить программу "OA4"?'), PChar(MainForm.Caption+' - '+'Ошибка!'), MB_YESNO+MB_ICONERROR+MB_DEFBUTTON1)=IDYES then
            // если пользователь решает всё же перезапустить приложение
            begin
{$IFDEF DEBUG}
...
begin
  if bAboutWindowExist then
    ...
    begin
      TrayIcon1.Visible:=True; // Configuration.bAlwaysShowTrayIcon;
...
procedure TMainForm.VDTrayIcon1RightClick(Sender: TObject; Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
begin
  SetForegroundWindow(MainForm.Handle);
...
begin
  ...
  ListBox1.Items.Add(string(cText));
