...
begin
  ...
  if not(csDesigning in ComponentState) then
    begin
      ...
      // регистраци€ глобальных переменных
      ...
      // отправка широковещательного сообщени€ клиентам на локальном компе
...
begin
  if Msg.Msg=msgLogKeeperClientAnswer then
    begin
      ...
      MessageBox(FOwnerForm.Handle, PWideChar('ѕолучено сообщение msgLogKeeperClientAnswer в форму '+FOwnerForm.Name+'!'), PWideChar('OA5 - »нформаци€!'), MB_OK+MB_ICONINFORMATION+MB_DEFBUTTON1);
      // if Msg.lParam=1 then
      // hBaseInfo:=Msg.wParam; // обновл€ем хэндл окна-приЄмника
      // Handled:=True;
    end;
  prOldWndProc(Msg);
end;
...
begin
  ...
  SendDebug('['+aString+']');
  SendDebug('Ќачало процедуры...');
end;

procedure TLogProvider.ExitMethod;
begin
  SendDebug('ќкончание процедуры.');
  if FGUIDList.Count>0 then
