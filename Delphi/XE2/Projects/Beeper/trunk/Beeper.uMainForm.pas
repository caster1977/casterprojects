...
begin
  ...
  begin
    TrayIcon.BalloonHint := 'Сигналы запущены';
    TrayIcon.BalloonTitle := 'Информация';
...
begin
  ...
  if TrayIcon.Visible then
  begin
    TrayIcon.BalloonHint := 'Сигналы остановлены';
    TrayIcon.BalloonTitle := 'Информация';
    TrayIcon.BalloonFlags := bfInfo;
    if FShowBaloonHints then
      TrayIcon.ShowBalloonHint;
...
begin
  ...
  with IniFile do
    try
      WriteBool('Подсказки', 'В трее', FShowBaloonHints);
      WriteBool('Звуки', 'Включены', FSoundEnabled);
...
begin
  with TConfigurationForm.Create(Self) do
    try
      ...
      CheckBox7.Checked := FShowBaloonHints;
      CheckBox8.Checked := FSoundEnabled;
      ...
      begin
        ...
        RemoveHotkeys;
        AssignHotkeys;
        FShowBaloonHints := CheckBox7.Checked;
        FSoundEnabled := CheckBox8.Checked;
      end;
      Free;
    end;
end;
  ...
  case Msg.message of
    WM_SYSTEM_TRAY_MESSAGE:
      begin
        case Msg.lParam of
          WM_MOUSEHOVER:
            TrayIcon.Hint := 'WM_MOUSEHOVER';
          WM_MOUSELEAVE:
            TrayIcon.Hint := 'WM_MOUSELEAVE';
        end;
      end;
  end;
end;
