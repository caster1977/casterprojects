...
begin
  ...
  begin
    ...
    pm := mysql_real_connect(MySQLConnectionHandler, PAnsiChar(Configuration.sMySQLHost),
      PAnsiChar(Configuration.sMySQLUser), PAnsiChar(Configuration.sMySQLPassword),
      PAnsiChar(Configuration.sMySQLDatabase), Configuration.iMySQLPort, nil, 1);
    ...
    begin
      ...
      if i = 2003 then
        sMessage :=
          'Возникла ошибка при попытке подключения к серверу MySQL, так как сервер MySQL не запущен!'
      else
...
begin
  if (FileExists(ExpandFileName(Application.HelpFile))) then
    ...
    MessageBox(MainForm.Handle, PChar('Извините, справочный файл к данной программе не найден.'),
      PChar(MainForm.Caption + ' - ' + 'Ошибка!'), MB_OK + MB_ICONERROR + MB_DEFBUTTON1);
end;
...
begin
  ...
  if MySQLConnectionHandler <> nil then
  begin
    LogThis('"Экземпляр подключения к MySQL-серверу не пуст. Производится отключение.',
      LogGroupGUID, lmtInfo);
    mysql_close(MySQLConnectionHandler);
    MySQLConnectionHandler := nil;
  end
  else
    LogThis('"Экземпляр подключения к MySQL-серверу пуст. Отключение не требуется.',
      LogGroupGUID, lmtInfo);
...
begin
  ...
  with IniFile do
    try
      Configuration.sLocalHost := Routines.GetLocalIP;
      ...
      Configuration.sMySQLDatabase :=
        AnsiString(ReadString('Сервер и база данных', 'sMySQLDatabase', ''));
      Configuration.sMySQLUser := AnsiString(ReadString('Сервер и база данных', 'sMySQLUser', ''));
      Configuration.sMySQLPassword :=
        AnsiString(ReadString('Сервер и база данных', 'sMySQLPassword', ''));
