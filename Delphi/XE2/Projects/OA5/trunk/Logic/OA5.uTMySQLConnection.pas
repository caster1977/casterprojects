...
begin
  if Assigned(FConnection) then
    if mysql_errno(FConnection) <> 0 then
      Result := sLineBreak + sLineBreak + 'Код ошибки: ' + sLineBreak + IntToStr(mysql_errno(FConnection)) + sLineBreak + sLineBreak +
        'Наименование ошибки: ' + sLineBreak + string(mysql_error(FConnection))
    else
      Result := ''
  else
    ...
    begin
      ...
      if Connection = nil then
        ...
        begin
          ...
          if Connection <> mysql_real_connect(Connection, PAnsiChar(AnsiString(Host)), PAnsiChar(AnsiString(Login)), PAnsiChar(AnsiString(Password)),
