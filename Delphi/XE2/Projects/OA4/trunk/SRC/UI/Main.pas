...
type
  ...
  EDITBALLOONTIP = record
    ...
    pszText: LPCWSTR;
    ttiIcon: Integer;
    // TTI_ERROR, TTI_INFO, TTI_NONE, TTI_WARNING, TTI_INFO_LARGE, TTI_WARNING_LARGE, TTI_ERROR_LARGE
  end;
...
type
  ...
  TMainForm = class(TForm)
    ...
    procedure lvOrgCustomDrawSubItem(Sender: TCustomListView; Item: TListItem; SubItem: Integer;
    ...
    procedure lvMsrCustomDrawItem(Sender: TCustomListView; Item: TListItem; state: TCustomDrawState;
  ...
  public
    ...
    procedure ProcedureHeader(aTitle, aLogGroupGUID: string;
      aSetCursorToHourGlasses, aSetBusy: boolean);
...
function CutFirstBrace(aString: AnsiString): AnsiString;
// обрезать всё с начала до первой скобки включительно
var
  aPos: Integer;
begin
...
function CutLastBrace(aString: AnsiString): AnsiString;
// обрезать всё с конца до последней скобки включительно
var
  aPos: Integer;
begin
...
begin
  ...
  begin
    ...
    try
      ...
      if AddCommas then
        NormalizeStringForQuery := '"' + string(System.AnsiStrings.StrPas(z)) + '"'
      else
        NormalizeStringForQuery := string(System.AnsiStrings.StrPas(z));
    finally
      if z <> nil then
...
var
  ...
  const
    nchars = 9;
    chars: array [1 .. nchars, 1 .. 2] of string = (('O', 'O'), ('o', 'o'), ('A', 'A'), ('a', 'a'),
      ('U', 'U'), ('u', 'u'), ('?', '?'), ('<', '<'), ('>', '>'));
  begin
...
procedure TMainForm.reMsrOtherInfoContextPopup(Sender: TObject; MousePos: TPoint;
  var Handled: boolean);
begin
...
procedure TMainForm.reOrgOtherInfoContextPopup(Sender: TObject; MousePos: TPoint;
  var Handled: boolean);
begin
...
begin
  ...
  begin
    ...
    begin
      ...
      pm := mysql_real_connect(MySQLConnectionHandler,
        ...
        PAnsiChar(AnsiString(Configuration.sMySQLDatabase)), Configuration.iMySQLPort, nil,
        Integer(1));
        ...
        begin
          ...
          begin
            ...
            begin
              ...
              q := 'SELECT COUNT(*) FROM ' + Configuration.sMySQLDatabase +
                '._usr WHERE usr_login="' + CurrentUser.sLogin + '" AND usr_password_md5=md5("' +
                CurrentUser.sPassword + '") AND usr_erased="0";';
              LogThis(q, LogGroupGUID, lmtSQL);
              i := mysql_real_query(MySQLConnectionHandler, PAnsiChar(AnsiString(q)), Length(q));
              if i = 0 then
              begin
                ...
                begin
                  ...
                  if i = 1 then
                  begin
                    LogThis('Количество строк выборки соответствует требуемому.',
                    ...
                    begin
                      LogThis('Указанный пользователь найден в базе данных!', LogGroupGUID,
                        lmtInfo);
                      LogThis('Освобождение ресурсов, занятых результатом выборки.',
                        LogGroupGUID, lmtInfo);
                      if ResultSet <> nil then
                        mysql_free_result(ResultSet);
                      LogThis('Получение данных и прав доступа указанного пользователя...',
                        LogGroupGUID, lmtInfo);
                      q := 'SELECT usr_id, usr_fullname, usr_is_admin, usr_can_edit_db, usr_can_backup_db, usr_can_clear_db, usr_can_defrag_db, usr_can_maintain_db, usr_can_edit_db_users FROM '
                        + Configuration.sMySQLDatabase + '._usr WHERE usr_login="' +
                        CurrentUser.sLogin + '" AND usr_password_md5=md5("' +
                        CurrentUser.sPassword + '");';
                      LogThis(q, LogGroupGUID, lmtSQL);
                      i := mysql_real_query(MySQLConnectionHandler, PAnsiChar(AnsiString(q)),
                        Length(q));
                      if i = 0 then
                      begin
                        LogThis('Запрос выполнен успешно.', LogGroupGUID, lmtInfo);
                        ...
                        if ResultSet <> nil then
                        begin
                          LogThis('Результирующая выборка получена успешно.', LogGroupGUID,
                            lmtInfo);
                          i := mysql_num_rows(ResultSet);
                          if i = 1 then
                          begin
                            ...
                            begin
                              ...
                              CurrentUser.bDBEdition :=
                                boolean(StrToIntDef(string(ResultRow[3]), 0));
                              CurrentUser.bDBBackup :=
                                boolean(StrToIntDef(string(ResultRow[4]), 0));
                              CurrentUser.bDBCleaning :=
                                boolean(StrToIntDef(string(ResultRow[5]), 0));
                              CurrentUser.bDBDefragmentation :=
                                boolean(StrToIntDef(string(ResultRow[6]), 0));
                              CurrentUser.bDBMaintenance :=
                                boolean(StrToIntDef(string(ResultRow[7]), 0));
                              CurrentUser.bDBUsersEdition :=
                                boolean(StrToIntDef(string(ResultRow[8]), 0));
                              LogThis('Получение данных и прав доступа указанного пользователя произведено успешно.',
                                LogGroupGUID, lmtInfo);
                              LogThis('Подключение пользователя к базе данных выполнено успешно.',
                                LogGroupGUID, lmtInfo);
                              ...
                              if Configuration.bOrgPanelHalfHeight then
                                pnlOrgLV.Height :=
                                  (pnlSearchResultsCreationModification.Height -
                                  pnlCreationModification.Height - Splitter2.Height) div 2
                              else
                              ...
                              begin
                                ...
                                lvOrg.ColumnClick := True;
                                lvOrgColumnClick(Sender,
                                  lvOrg.Columns[Configuration.iOrgSortColumn]);
                                lvMsr.ColumnClick := True;
                                lvMsr.ColumnClick := False;
                                lvMsr.ColumnClick := True;
                                lvMsrColumnClick(Sender,
                                  lvMsr.Columns[Configuration.iMsrSortColumn]);
                              end
                              else
                              begin
                              ...
                              if Configuration.bShowMeasuresListAsRichEdit and
                                (not CurrentUser.bDBEdition) then
                              begin
                                lvMsr.Visible := False;
                                reMsr.Visible := True;
                              end
                              ...
                              StatusBar1.Panels[STATUSBAR_SERVER_NAME].Width :=
                                StatusBar1.Canvas.TextWidth(Configuration.sMySQLHost) + 6;
                              StatusBar1.Panels[STATUSBAR_SERVER_NAME].Text :=
                                Configuration.sMySQLHost;
                            ...
                            s := 'Количество строк выборки (' + IntToStr(i) +
      ...
      begin
        ...
        s := string(mysql_error(MySQLConnectionHandler));
        if i = 2003 then
...
begin
  if PageControl1.ActivePage = tsOrg then
  begin
    reOrgOtherInfo.Paragraph.Numbering :=
      TNumberingStyle(not boolean(reOrgOtherInfo.Paragraph.Numbering));
  end;
  if PageControl1.ActivePage = tsMsr then
  begin
    reMsrOtherInfo.Paragraph.Numbering :=
      TNumberingStyle(not boolean(reMsrOtherInfo.Paragraph.Numbering));
  end;
  Update_RichEditTools;
end;
...
begin
  ...
  chkbxMsrForChildren.Checked := not chkbxMsrForAdultsOnly.Checked and
    not chkbxMsrForChildren.Enabled;
  chkbxMsrForTeenagers.Checked := not chkbxMsrForAdultsOnly.Checked and
    not chkbxMsrForTeenagers.Enabled;
end;
...
begin
  ...
  ProgressBar1.SetBounds(PanelRect.Left, PanelRect.Top, PanelRect.Right - PanelRect.Left,
    ...
    begin
      ...
      begin
        ...
        LogThis('В командной строке указан параметр "/NOLOGO",' +
          ' отключающий вывод окна "О программе..." при запуске программы.', '', lmtInfo);
      end;
      if s = '/SHOWID' then
      begin
        bParamShowID := True;
        LogThis('В командной строке указан параметр "/SHOWID",' +
          ' включающий отображение ID записей базы данных при работе программы.', '', lmtInfo);
      end;
    end;
  ...
  try
    SendMessage(edbxSearchName.Handle, EM_SETCUEBANNER, WPARAM(0),
      lParam(StringToWideChar('Поле названия', pwcGroupName, 1000)));
  finally
  ...
  GetMem(pwcGroupName, SizeOf(WideChar) * 1000);
  try
    SendMessage(edbxSearchAddress.Handle, EM_SETCUEBANNER, WPARAM(0),
      lParam(StringToWideChar('Поле местоположения', pwcGroupName, 1000)));
  finally
    FreeMem(pwcGroupName);
  end;
  ...
  GetMem(pwcGroupName, SizeOf(WideChar) * 1000);
  try
    SendMessage(edbxSearchOther.Handle, EM_SETCUEBANNER, WPARAM(0),
      lParam(StringToWideChar('Поле названия и инфо', pwcGroupName, 1000)));
  finally
    FreeMem(pwcGroupName);
  end;
...
begin
  ...
  begin
    if lvMsr.Columns[i].Caption[1] = '*' then
      lvMsr.Columns[i].Caption := Copy(lvMsr.Columns[i].Caption, 3,
        Length(lvMsr.Columns[i].Caption) - 2);
  end;
  lvMsr.Columns[Configuration.iMsrSortColumn].Caption := '* ' + lvMsr.Columns
    [Configuration.iMsrSortColumn].Caption;
end;

procedure TMainForm.lvMsrCustomDrawItem(Sender: TCustomListView; Item: TListItem;
  state: TCustomDrawState; var DefaultDraw: boolean);
var
  D: TDate;
  T: TTime;
...
begin
  ...
  begin
    ...
    begin
      D := EncodeDate(StrToInt(Copy(Item.SubItems[1], 7, 4)), StrToInt(Copy(Item.SubItems[1], 4, 2)
...
begin
  ...
  begin
    if lvOrg.Selected <> nil then // определяем ID организации
      iOrgID := StrToIntDef(lvOrg.Selected.SubItems[2], -1);
    // сохранение значения ID выделенной организации
  ...
  begin
    if lvOrg.Columns[i].Caption[1] = '*' then
      lvOrg.Columns[i].Caption := Copy(lvOrg.Columns[i].Caption, 3,
        Length(lvOrg.Columns[i].Caption) - 2);
  end;
  lvOrg.Columns[Configuration.iOrgSortColumn].Caption := '* ' + lvOrg.Columns
    [Configuration.iOrgSortColumn].Caption;
end;

procedure TMainForm.lvOrgCustomDrawSubItem(Sender: TCustomListView; Item: TListItem;
...
begin
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          q := 'SELECT org_erased FROM ' + Configuration.sMySQLDatabase + '._org WHERE org_id="'
            + s + '";';
          // LogThis(q,LogGroupGUID,lmtSQL);
          i := mysql_real_query(MySQLConnectionHandler, PAnsiChar(AnsiString(q)), Length(q));
          if i = 0 then
            ...
            try
              ...
              begin
                ...
                begin
                  ...
                  GenerateError('Получено некорректное количество строк результирующей выборки (' +
                    IntToStr(i) + ')!', s, bError);
              end
              else
                GenerateError
                  ('Не удалось получить результирующую выборку по последнему SQL-запросу!',
                  s, bError);
            finally
              if ResultSet <> nil then
              begin
  ...
  begin
    if lvOrg.Items.Count > 0 then
    begin
      LogThis('Производится обновление значков состояния выделенной организации.',
        LogGroupGUID, lmtInfo);
      ProgressBar1.Max := 1;
      lvOrg.Items.BeginUpdate;
      s := lvOrg.Selected.SubItems[2];
      LogThis('Производится обновление состояния организации с ID=' + s + '.',
        LogGroupGUID, lmtInfo);
      q := 'SELECT org_erased FROM ' + Configuration.sMySQLDatabase + '._org WHERE org_id="'
        + s + '";';
      LogThis(q, LogGroupGUID, lmtSQL);
      ...
      begin
        ...
        try
          ...
          begin
            ...
            if (i = 1) then
            begin
              LogThis('Количество строк выборки (' + IntToStr(i) +
                ') соответствует требуемому (1).', LogGroupGUID, lmtInfo);
              ResultRow := mysql_fetch_row(ResultSet);
              if ResultRow <> nil then
              begin
                if (ResultRow[0] = '1') then
            ...
            GenerateError('Не удалось получить результирующую выборку по последнему SQL-запросу!',
              s, bError);
        finally
          if ResultSet <> nil then
          begin
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          q := 'SELECT msr_erased FROM ' + Configuration.sMySQLDatabase + '._msr WHERE msr_id="'
            + s + '";';
          // LogThis(q,LogGroupGUID,lmtSQL);
          i := mysql_real_query(MySQLConnectionHandler, PAnsiChar(AnsiString(q)), Length(q));
          if i = 0 then
...
begin
  MessageBox(Handle, PChar('Данное действие на текущий момент в программе не реализовано.'),
    PChar('Невозможно выполнить операцию'), MB_OK + MB_ICONSTOP);
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      b := MessageBox(Handle,
        PChar('Вы подтверждаете пометку на удаление выделенной организации и всех относящихся к нему мероприятий?'),
        PChar('OA4 - Подтверждение пометки на удаление'), MB_YESNO + MB_ICONEXCLAMATION +
        MB_DEFBUTTON2) = IDYES;
      if b then
      begin
        Screen.Cursor := crHourGlass;
        ProgressBar1.Max := 4;
        LogThis('Будет произведена попытка пометки на удаление всех мероприятий организации с ID=' +
          s1, LogGroupGUID, lmtWarning);
        // составление строки запроса
        q := 'UPDATE ' + Configuration.sMySQLDatabase +
          '._msr SET msr_erased="1" WHERE msr_organization_id="' + s1 + '";';
        LogThis(q, LogGroupGUID, lmtSQL);
        i := mysql_real_query(MySQLConnectionHandler, PAnsiChar(AnsiString(q)), Length(q));
        if i = 0 then
        begin
          ...
          LogThis('Количество обработанных строк равно ' + IntToStr(i) + '.', LogGroupGUID,
            lmtInfo);
          // запись статистики в БД
          LogThis('Внесение записи в таблицу событий.', LogGroupGUID, lmtInfo);
          q1 := 'INSERT INTO ' + Configuration.sMySQLDatabase +
            '._evn VALUES (NULL, NOW(),"DEL","msr",0,' + CurrentUser.sID + ',' + #39 + q +
            #39 + ');';
          ...
          begin
            LogThis('Запрос выполнен успешно.', LogGroupGUID, lmtInfo);
            i := mysql_affected_rows(MySQLConnectionHandler);
            LogThis('Количество обработанных строк равно ' + IntToStr(i) + '.',
              LogGroupGUID, lmtInfo);
            if (i = 1) then
            begin
              LogThis('Запись в таблицу событий внесена успешно', LogGroupGUID, lmtInfo);
              ProgressBar1.StepIt;
              LogThis('Будет произведена попытка пометки на удаление организации с ID=' + s1,
                LogGroupGUID, lmtWarning);
              q := 'UPDATE ' + Configuration.sMySQLDatabase +
                '._org SET org_erased="1" WHERE org_id="' + s1 + '";';
              LogThis(q, LogGroupGUID, lmtSQL);
              i := mysql_real_query(MySQLConnectionHandler, PAnsiChar(AnsiString(q)), Length(q));
              if i = 0 then
              begin
                ...
                if i = 1 then
                begin
                  LogThis('Пометка организации на удаление произведена успешно',
                    LogGroupGUID, lmtInfo);
                  ProgressBar1.StepIt;
                  // запись статистики в БД
                  LogThis('Внесение записи в таблицу событий.', LogGroupGUID, lmtInfo);
                  q1 := 'INSERT INTO ' + Configuration.sMySQLDatabase +
                    '._evn VALUES (NULL, NOW(),"DEL","org",' + s1 + ',' + CurrentUser.sID + ',' +
                    #39 + q + #39 + ');';
                  LogThis(q1, LogGroupGUID, lmtSQL);
                  i := mysql_real_query(MySQLConnectionHandler, PAnsiChar(AnsiString(q1)),
                    Length(q1));
                  if i = 0 then
                  begin
                    LogThis('Запрос выполнен успешно.', LogGroupGUID, lmtInfo);
                    ...
                    begin
                      ...
                      GenerateError('Количество обработанных строк (' + IntToStr(i) +
                        ') не соответствует требуемому (1)!', s, bError);
                  end
                  else
                    GenerateError('Возникла ошибка при выполнении запроса!', s, bError);
                end
        ...
        LogThis('Попытка пометки на удаление организации с ID=' + s1 + ' была отменена.',
          LogGroupGUID, lmtInfo);
    end;
  end;
  ...
  begin
    ...
    begin
      ...
      Screen.Cursor := crHourGlass;
      ProgressBar1.Max := 2;
      LogThis('Будет произведена попытка пометки на удаление мероприятия с ID=' + s, LogGroupGUID,
        lmtWarning);
      // составление строки запроса
      q := 'UPDATE ' + Configuration.sMySQLDatabase + '._msr SET msr_erased="1" WHERE msr_id="'
        + s + '";';
      LogThis(q, LogGroupGUID, lmtSQL);
      i := mysql_real_query(MySQLConnectionHandler, PAnsiChar(AnsiString(q)), Length(q));
      if i = 0 then
      begin
        ...
        begin
          ...
          LogThis('Внесение записи в таблицу событий.', LogGroupGUID, lmtInfo);
          q1 := 'INSERT INTO ' + Configuration.sMySQLDatabase +
            '._evn VALUES (NULL, NOW(),"DEL","msr",' + s + ',' + CurrentUser.sID + ',' + #39 + q +
            #39 + ');';
          LogThis(q1, LogGroupGUID, lmtSQL);
          i := mysql_real_query(MySQLConnectionHandler, PAnsiChar(AnsiString(q1)), Length(q1));
          if i = 0 then
...
begin
  ...
  begin
    ...
    begin
      ...
      Screen.Cursor := crHourGlass;
      ProgressBar1.Max := 2;
      LogThis('Будет произведена попытка восстановления организации с ID=' + s, LogGroupGUID,
        lmtWarning);
      // составление строки запроса
      q := 'UPDATE ' + Configuration.sMySQLDatabase + '._org SET org_erased="0" WHERE org_id="'
        + s + '";';
      LogThis(q, LogGroupGUID, lmtSQL);
      i := mysql_real_query(MySQLConnectionHandler, PAnsiChar(AnsiString(q)), Length(q));
      if i = 0 then
      begin
        ...
        begin
          ...
          LogThis('Внесение записи в таблицу событий.', LogGroupGUID, lmtInfo);
          q1 := 'INSERT INTO ' + Configuration.sMySQLDatabase +
            '._evn VALUES (NULL, NOW(),"REC","org",' + s + ',' + CurrentUser.sID + ',' + #39 + q +
            #39 + ');';
          LogThis(q1, LogGroupGUID, lmtSQL);
          i := mysql_real_query(MySQLConnectionHandler, PAnsiChar(AnsiString(q1)), Length(q1));
          if i = 0 then
  ...
  begin
    ...
    begin
      if lvOrg.Selected <> nil then
        ...
        begin
          LogThis('Восстанавливаемое мероприятие принадлежит удалённой организации!', LogGroupGUID,
            lmtWarning);
          b := MessageBox(Handle,
            PChar('Вы пытаетесь воостановить мероприятие помеченной на удаление организации.' +
            sLineBreak + 'Данная операция невозможна без восстановления самой организации.' +
            sLineBreak + 'Вы желаете восстановить организацию, к которой относится мероприятие?'),
            PChar('OA4 - Подтверждение восстановления'), MB_YESNO + MB_ICONEXCLAMATION +
            MB_DEFBUTTON2) = IDYES;
          if b then
          begin
            s := lvOrg.Selected.SubItems[2];
            ...
            // составление строки запроса
            q := 'UPDATE ' + Configuration.sMySQLDatabase +
              '._org SET org_erased="0" WHERE org_id="' + s + '";';
            LogThis(q, LogGroupGUID, lmtSQL);
            i := mysql_real_query(MySQLConnectionHandler, PAnsiChar(AnsiString(q)), Length(q));
            ProgressBar1.StepIt;
            if i = 0 then
            begin
              ...
              begin
                ...
                // запись статистики в БД
                LogThis('Внесение записи в таблицу событий.', LogGroupGUID, lmtInfo);
                q := 'INSERT INTO ' + Configuration.sMySQLDatabase +
                  '._evn VALUES (NULL, NOW(),"REC","org",' + s + ',' + CurrentUser.sID + ',' + #39 +
                  q + #39 + ');';
                LogThis(q, LogGroupGUID, lmtSQL);
                i := mysql_real_query(MySQLConnectionHandler, PAnsiChar(AnsiString(q)), Length(q));
                ProgressBar1.StepIt;
          ...
          begin
            // выход
            LogThis('Восстанавление мероприятия и организации-владельца отменено.', LogGroupGUID,
              lmtWarning);
            Screen.Cursor := crDefault;
            Set_Ready(LogGroupGUID);
            Exit;
      ...
      begin
        ...
        Screen.Cursor := crHourGlass;
        ProgressBar1.Max := 2;
        LogThis('Будет произведена попытка восстановления мероприятия с ID=' + s, LogGroupGUID,
          lmtWarning);
        // составление строки запроса
        q := 'UPDATE ' + Configuration.sMySQLDatabase + '._msr SET msr_erased="0" WHERE msr_id="'
          + s + '";';
        LogThis(q, LogGroupGUID, lmtSQL);
        i := mysql_real_query(MySQLConnectionHandler, PAnsiChar(AnsiString(q)), Length(q));
        ProgressBar1.StepIt;
        ...
        begin
          ...
          begin
            ...
            LogThis('Внесение записи в таблицу событий.', LogGroupGUID, lmtInfo);
            q := 'INSERT INTO ' + Configuration.sMySQLDatabase +
              '._evn VALUES (NULL, NOW(),"REC","msr",' + s + ',' + CurrentUser.sID + ',' + #39 + q +
              #39 + ');';
            LogThis(q, LogGroupGUID, lmtSQL);
            i := mysql_real_query(MySQLConnectionHandler, PAnsiChar(AnsiString(q)), Length(q));
            ProgressBar1.StepIt;
...
begin
  ...
  if spedMsrDurationDays.Focused or spedMsrDurationHours.Focused or spedMsrDurationMinutes.Focused
  then
    if ((spedMsrDurationDays.Text <> '') and (spedMsrDurationHours.Text <> '') and
      (spedMsrDurationMinutes.Text <> '')) then
    begin
      t1 := DateTimeToTimeStamp(dtpMsrStartDate.DateTime);
      t1.Date := t1.Date + spedMsrDurationDays.Value;
      a1 := TimeStampToMSecs(t1) + (spedMsrDurationMinutes.Value * 60000) +
        (spedMsrDurationHours.Value * 3600000);
...
begin
  ...
  begin
    if Trim(edbxMsrOrganizer.Text) <> '' then
      ShowEditBaloonTip(edbxMsrOrganizer, lblMsrOrganizer.Caption,
        Trim(edbxMsrOrganizer.Text), TTI_NONE)
    else
...
begin
  ...
  begin
    if Trim(edbxMsrPerformer.Text) <> '' then
      ShowEditBaloonTip(edbxMsrPerformer, lblMsrPerformer.Caption,
        Trim(edbxMsrPerformer.Text), TTI_NONE)
    else
...
begin
  ...
  begin
    if Trim(edbxMsrProducer.Text) <> '' then
      ShowEditBaloonTip(edbxMsrProducer, lblMsrProducer.Caption,
        Trim(edbxMsrProducer.Text), TTI_NONE)
    else
...
begin
  if not Configuration.bHideEditBoxToolTips then
    ShowEditBaloonTip(edbxSearchAddress, 'Подсказка',
      'Введите адресную часть строк искомой информации ' +
...
begin
  if not Configuration.bHideEditBoxToolTips then
    ShowEditBaloonTip(edbxSearchName, 'Подсказка',
      'Введите фрагменты строк наименования искомой информации ' +
      'и нажмите клавишу ENTER для выполнения запроса. Для отображения полного списка очистите поле '
      + 'и нажмите клавишу ENTER.', TTI_INFO);
end;
...
begin
  if not Configuration.bHideEditBoxToolTips then
    ShowEditBaloonTip(edbxSearchOther, 'Подсказка',
      'Введите фрагменты строк прочей искомой информации ' +
      'и нажмите клавишу ENTER для выполнения запроса. Для отображения полного списка очистите поле '
      + 'и нажмите клавишу ENTER.', TTI_INFO);
end;
...
begin
  ...
  spedMsrDurationDays.Enabled := chkbxMsrUnknownDuration.Enabled and
    not chkbxMsrUnknownDuration.Checked and chkbxMsrStart.Checked and chkbxMsrStop.Checked;
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        Action_DB_RecoverNode.Enabled := CurrentUser.bDBEdition and
          (lvOrg.Selected.ImageIndex = DELETEDICON_ID);
        Action_DB_DeleteNode.Enabled := CurrentUser.bDBEdition and
          (lvOrg.Selected.ImageIndex = EXISTINGICON_ID);
      end
      else
      begin
    ...
    begin
      ...
      begin
        Action_DB_AddMeasure.Enabled := CurrentUser.bDBEdition;
        Action_DB_RecoverNode.Enabled := CurrentUser.bDBEdition and
          (lvMsr.Selected.ImageIndex = DELETEDICON_ID);
        Action_DB_DeleteNode.Enabled := CurrentUser.bDBEdition and
          (lvMsr.Selected.ImageIndex = EXISTINGICON_ID);
      end
      else
      begin
        Action_DB_AddMeasure.Enabled := False;
  ...
  ActionToolBar1.ActionControls[4].Visible := Action_DB_Connection.Visible or
    Action_DB_Disconnection.Visible;
  ActionToolBar1.ActionControls[9].Visible := Action_DB_AddMeasure.Visible or
    Action_DB_DeleteNode.Visible or Action_DB_RecoverNode.Visible or Action_DB_RefreshList.Visible;
  ActionToolBar1.ActionControls[14].Visible := Action_DB_Cleaning.Visible or
    Action_DB_Defragmentation.Visible or Action_DB_Maintenance.Visible or Action_DB_Backup.Visible;
  ActionToolBar1.ActionControls[17].Visible := Action_DB_Users.Visible or
    Action_DB_Statistic.Visible;
  ActionToolBar1.ActionControls[20].Visible := Action_PostMessage.Visible or
    Action_ViewMessages.Visible;
  ActionToolBar1.ActionControls[22].Visible := Action_Configuration.Visible;
  miActions.Visible := Action_DB_Backup.Visible or Action_DB_Cleaning.Visible or
    Action_DB_Defragmentation.Visible or Action_DB_Maintenance.Visible or
    Action_DB_AddMeasure.Visible or Action_DB_DeleteNode.Visible or Action_DB_RecoverNode.Visible or
...
var
  ...
  pwcGroupName: PWideChar;
  GroupId: Integer;
  sExternalLog_All, sExternalLog_Dates, sExternalLog_Dates_Start, sExternalLog_Dates_Stop,
    sExternalLog_Types, sExternalLog_Types_1, sExternalLog_Types_2, sExternalLog_Types_3,
    sExternalLog_Types_4, sExternalLog_Types_5, sExternalLog_Names: string;
  sSym: string;
  iNum: Integer;
begin
  bError := False;
  ProcedureHeader('Процедура заполнения списка организаций в соответствии с параметрами поиска',
    LogGroupGUID, True, True);
  ...
  BroadcastSystemMessage(BSF_IGNORECURRENTTASK or BSF_POSTMESSAGE, @Recipients, msgBaseInfoClient,
    Handle, 0);
  ...
  if not bError then
  begin
    LogThis('Преобразование строки поиска по наименованию - <' + edbxSearchName.Text + '>.',
      LogGroupGUID, lmtInfo);
    try
      CutStringByLimiterToStringList(edbxSearchName.Text, slLastSearchedName, ' ');
    except
      GenerateError('Возникла ошибка в процессе преобразования строки поиска по наименованию!',
        s, bError);
    end;
  end;
  if not bError then
  begin
    LogThis('Преобразование строки поиска по адресу - <' + edbxSearchAddress.Text + '>.',
      LogGroupGUID, lmtInfo);
    try
      CutStringByLimiterToStringList(edbxSearchAddress.Text, slLastSearchedAddress, ' ');
    except
      GenerateError('Возникла ошибка в процессе преобразования строки поиска по адресным данным!',
        s, bError);
    end;
  end;
  if not bError then
  begin
    LogThis('Преобразование строки поиска по прочим данным - <' + edbxSearchOther.Text + '>.',
      LogGroupGUID, lmtInfo);
    try
      CutStringByLimiterToStringList(edbxSearchOther.Text, slLastSearchedOther, ' ');
    except
      GenerateError('Возникла ошибка в процессе преобразования строки поиска по прочим данным!',
        s, bError);
    end;
  end;
  ...
  begin
    q1 := '';
    if (chkbxHideErased.Visible and chkbxHideErased.Enabled and chkbxHideErased.Checked) or
      (not chkbxHideErased.Visible) then
      q1 := 'r_o.org_erased="0" ';
    ...
    begin
      if q1 > '' then
        q1 := q1 + 'AND ';
      q1 := q1 + '(ISNULL(r_m.msr_stop_datetime) OR (r_m.msr_stop_datetime>="' +
        FormatDateTime('yyyy-mm-dd hh:nn:ss', dtpStartDate.DateTime) + '")) ';
    end;
    ...
    begin
      if q1 > '' then
        q1 := q1 + 'AND ';
      q1 := q1 + '(ISNULL(r_m.msr_start_datetime) OR (r_m.msr_start_datetime<="' +
        FormatDateTime('yyyy-mm-dd hh:nn:ss', dtpStopDate.DateTime) + '")) ';
    end;
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            q1 := q1 + '(' + '(m_f.name LIKE "% ' + s + '%") OR ' + '(m_f.name LIKE "% ' + s +
              ' %") OR ' + '(m_f.name LIKE "% ' + s + '.%") OR ' + '(m_f.name LIKE "% ' + s +
              ',%") OR ' + '(m_f.name LIKE "% ' + s + '-%") OR ' + '(m_f.name LIKE "% ' + s +
              '+%") OR ' + '(m_f.name LIKE "% ' + s + '!%") OR ' + '(m_f.name LIKE "% ' + s +
              '?%") OR ' + '(m_f.name LIKE "% ' + s + ':%") OR ' + '(m_f.name LIKE "% ' + s +
              ';%") OR ' +

              '(r_o._org_address LIKE "% ' + s + '%") OR ' + '(r_o._org_address LIKE "% ' + s +
              ' %") OR ' + '(r_o._org_address LIKE "% ' + s + '.%") OR ' +
              '(r_o._org_address LIKE "% ' + s + ',%") OR ' + '(r_o._org_address LIKE "% ' + s +
              '-%") OR ' + '(r_o._org_address LIKE "% ' + s + '+%") OR ' +
              '(r_o._org_address LIKE "% ' + s + '!%") OR ' + '(r_o._org_address LIKE "% ' + s +
              '?%") OR ' + '(r_o._org_address LIKE "% ' + s + ':%") OR ' +
              ...
              '(m_c.name LIKE "% ' + s + '%") OR ' + '(m_c.name LIKE "% ' + s + ' %") OR ' +
              '(m_c.name LIKE "% ' + s + '.%") OR ' + '(m_c.name LIKE "% ' + s + ',%") OR ' +
              '(m_c.name LIKE "% ' + s + '-%") OR ' + '(m_c.name LIKE "% ' + s + '+%") OR ' +
              '(m_c.name LIKE "% ' + s + '!%") OR ' + '(m_c.name LIKE "% ' + s + '?%") OR ' +
              '(m_c.name LIKE "% ' + s + ':%") OR ' + '(m_c.name LIKE "% ' + s + ';%") OR ' +

              '(m_s.name LIKE "% ' + s + '%") OR ' + '(m_s.name LIKE "% ' + s + ' %") OR ' +
              '(m_s.name LIKE "% ' + s + '.%") OR ' + '(m_s.name LIKE "% ' + s + ',%") OR ' +
              '(m_s.name LIKE "% ' + s + '-%") OR ' + '(m_s.name LIKE "% ' + s + '+%") OR ' +
              '(m_s.name LIKE "% ' + s + '!%") OR ' + '(m_s.name LIKE "% ' + s + '?%") OR ' +
              '(m_s.name LIKE "% ' + s + ':%") OR ' + '(m_s.name LIKE "% ' + s + ';%") OR ' +
              ...
              '(m_f.subhouse_num LIKE "' + s + '%") OR ' + '(m_f.subhouse_sym LIKE "' + s +
              '%") OR ' +
              ...
              '(r_o.org_how_to_reach LIKE "% ' + s + '%") OR ' + '(r_o.org_how_to_reach LIKE "% ' +
              s + ' %") OR ' + '(r_o.org_how_to_reach LIKE "% ' + s + '.%") OR ' +
              '(r_o.org_how_to_reach LIKE "% ' + s + ',%") OR ' + '(r_o.org_how_to_reach LIKE "% ' +
              s + '-%") OR ' + '(r_o.org_how_to_reach LIKE "% ' + s + '+%") OR ' +
              '(r_o.org_how_to_reach LIKE "% ' + s + '!%") OR ' + '(r_o.org_how_to_reach LIKE "% ' +
              s + '?%") OR ' + '(r_o.org_how_to_reach LIKE "% ' + s + ':%") OR ' +
              ...
              '((r_o.org_work_time LIKE "% ' + s + '%") OR (m_f.work LIKE "% ' + s + '%")) OR ' +
              '((r_o.org_work_time LIKE "% ' + s + ' %") OR (m_f.work LIKE "% ' + s + ' %")) OR ' +
              '((r_o.org_work_time LIKE "% ' + s + '.%") OR (m_f.work LIKE "% ' + s + '.%")) OR ' +
              '((r_o.org_work_time LIKE "% ' + s + ',%") OR (m_f.work LIKE "% ' + s + ',%")) OR ' +
              '((r_o.org_work_time LIKE "% ' + s + '-%") OR (m_f.work LIKE "% ' + s + '-%")) OR ' +
              ...
              '(r_o.org_other_info LIKE "% ' + s + '%") OR ' + '(r_o.org_other_info LIKE "% ' + s +
              ' %") OR ' + '(r_o.org_other_info LIKE "% ' + s + '.%") OR ' +
              '(r_o.org_other_info LIKE "% ' + s + ',%") OR ' + '(r_o.org_other_info LIKE "% ' + s +
              '-%") OR ' + '(r_o.org_other_info LIKE "% ' + s + '+%") OR ' +
              '(r_o.org_other_info LIKE "% ' + s + '!%") OR ' + '(r_o.org_other_info LIKE "% ' + s +
              '?%") OR ' + '(r_o.org_other_info LIKE "% ' + s + ':%") OR ' +
              '(r_o.org_other_info LIKE "% ' + s + ';%") OR ' +

              '(r_m.msr_type LIKE "% ' + s + '%") OR ' + '(r_m.msr_type LIKE "% ' + s + ' %") OR ' +
              '(r_m.msr_type LIKE "% ' + s + '.%") OR ' + '(r_m.msr_type LIKE "% ' + s + ',%") OR '
              + '(r_m.msr_type LIKE "% ' + s + '-%") OR ' + '(r_m.msr_type LIKE "% ' + s +
              '+%") OR ' + '(r_m.msr_type LIKE "% ' + s + '!%") OR ' + '(r_m.msr_type LIKE "% ' + s
              + '?%") OR ' + '(r_m.msr_type LIKE "% ' + s + ':%") OR ' + '(r_m.msr_type LIKE "% ' +
              s + ';%") OR ' +

              '(r_m.msr_name LIKE "% ' + s + '%") OR ' + '(r_m.msr_name LIKE "% ' + s + ' %") OR ' +
              '(r_m.msr_name LIKE "% ' + s + '.%") OR ' + '(r_m.msr_name LIKE "% ' + s + ',%") OR '
              + '(r_m.msr_name LIKE "% ' + s + '-%") OR ' + '(r_m.msr_name LIKE "% ' + s +
              '+%") OR ' + '(r_m.msr_name LIKE "% ' + s + '!%") OR ' + '(r_m.msr_name LIKE "% ' + s
              + '?%") OR ' + '(r_m.msr_name LIKE "% ' + s + ':%") OR ' + '(r_m.msr_name LIKE "% ' +
              s + ';%") OR ' +

              '(r_m.msr_author LIKE "% ' + s + '%") OR ' + '(r_m.msr_author LIKE "% ' + s +
              ' %") OR ' + '(r_m.msr_author LIKE "% ' + s + '.%") OR ' + '(r_m.msr_author LIKE "% '
              + s + ',%") OR ' + '(r_m.msr_author LIKE "% ' + s + '-%") OR ' +
              '(r_m.msr_author LIKE "% ' + s + '+%") OR ' + '(r_m.msr_author LIKE "% ' + s +
              '!%") OR ' + '(r_m.msr_author LIKE "% ' + s + '?%") OR ' + '(r_m.msr_author LIKE "% '
              + s + ':%") OR ' + '(r_m.msr_author LIKE "% ' + s + ';%") OR ' +

              '(r_m.msr_producer LIKE "% ' + s + '%") OR ' + '(r_m.msr_producer LIKE "% ' + s +
              ' %") OR ' + '(r_m.msr_producer LIKE "% ' + s + '.%") OR ' +
              '(r_m.msr_producer LIKE "% ' + s + ',%") OR ' + '(r_m.msr_producer LIKE "% ' + s +
              '-%") OR ' + '(r_m.msr_producer LIKE "% ' + s + '+%") OR ' +
              '(r_m.msr_producer LIKE "% ' + s + '!%") OR ' + '(r_m.msr_producer LIKE "% ' + s +
              '?%") OR ' + '(r_m.msr_producer LIKE "% ' + s + ':%") OR ' +
              '(r_m.msr_producer LIKE "% ' + s + ';%") OR ' +

              '(r_m.msr_performer LIKE "% ' + s + '%") OR ' + '(r_m.msr_performer LIKE "% ' + s +
              ' %") OR ' + '(r_m.msr_performer LIKE "% ' + s + '.%") OR ' +
              '(r_m.msr_performer LIKE "% ' + s + ',%") OR ' + '(r_m.msr_performer LIKE "% ' + s +
              '-%") OR ' + '(r_m.msr_performer LIKE "% ' + s + '+%") OR ' +
              '(r_m.msr_performer LIKE "% ' + s + '!%") OR ' + '(r_m.msr_performer LIKE "% ' + s +
              '?%") OR ' + '(r_m.msr_performer LIKE "% ' + s + ':%") OR ' +
              '(r_m.msr_performer LIKE "% ' + s + ';%") OR ' +

              '(r_m.msr_organizer LIKE "% ' + s + '%") OR ' + '(r_m.msr_organizer LIKE "% ' + s +
              ' %") OR ' + '(r_m.msr_organizer LIKE "% ' + s + '.%") OR ' +
              '(r_m.msr_organizer LIKE "% ' + s + ',%") OR ' + '(r_m.msr_organizer LIKE "% ' + s +
              '-%") OR ' + '(r_m.msr_organizer LIKE "% ' + s + '+%") OR ' +
              '(r_m.msr_organizer LIKE "% ' + s + '!%") OR ' + '(r_m.msr_organizer LIKE "% ' + s +
              '?%") OR ' + '(r_m.msr_organizer LIKE "% ' + s + ':%") OR ' +
              '(r_m.msr_organizer LIKE "% ' + s + ';%") OR ' +

              '(r_m.msr_ticket_price LIKE "% ' + s + '%") OR ' + '(r_m.msr_ticket_price LIKE "% ' +
              s + ' %") OR ' + '(r_m.msr_ticket_price LIKE "% ' + s + '.%") OR ' +
              '(r_m.msr_ticket_price LIKE "% ' + s + ',%") OR ' + '(r_m.msr_ticket_price LIKE "% ' +
              s + '-%") OR ' + '(r_m.msr_ticket_price LIKE "% ' + s + '+%") OR ' +
              '(r_m.msr_ticket_price LIKE "% ' + s + '!%") OR ' + '(r_m.msr_ticket_price LIKE "% ' +
              s + '?%") OR ' + '(r_m.msr_ticket_price LIKE "% ' + s + ':%") OR ' +
              ...
              '(r_m.msr_other_info LIKE "% ' + s + '%") OR ' + '(r_m.msr_other_info LIKE "% ' + s +
              ' %") OR ' + '(r_m.msr_other_info LIKE "% ' + s + '.%") OR ' +
              '(r_m.msr_other_info LIKE "% ' + s + ',%") OR ' + '(r_m.msr_other_info LIKE "% ' + s +
              '-%") OR ' + '(r_m.msr_other_info LIKE "% ' + s + '+%") OR ' +
              '(r_m.msr_other_info LIKE "% ' + s + '!%") OR ' + '(r_m.msr_other_info LIKE "% ' + s +
              '?%") OR ' + '(r_m.msr_other_info LIKE "% ' + s + ':%") OR ' +
              '(r_m.msr_other_info LIKE "% ' + s + ';%") OR ' +

              '(r_p.phn_comments LIKE "% ' + s + '%") OR ' + '(r_p.phn_comments LIKE "% ' + s +
              ' %") OR ' + '(r_p.phn_comments LIKE "% ' + s + '.%") OR ' +
              '(r_p.phn_comments LIKE "% ' + s + ',%") OR ' + '(r_p.phn_comments LIKE "% ' + s +
              '-%") OR ' + '(r_p.phn_comments LIKE "% ' + s + '+%") OR ' +
              '(r_p.phn_comments LIKE "% ' + s + '!%") OR ' + '(r_p.phn_comments LIKE "% ' + s +
              '?%") OR ' + '(r_p.phn_comments LIKE "% ' + s + ':%") OR ' +
              '(r_p.phn_comments LIKE "% ' + s + ';%") OR ' +

              '(m_o.name LIKE "% ' + s + '%") OR ' + '(m_o.name LIKE "% ' + s + ' %") OR ' +
              '(m_o.name LIKE "% ' + s + '.%") OR ' + '(m_o.name LIKE "% ' + s + ',%") OR ' +
              '(m_o.name LIKE "% ' + s + '-%") OR ' + '(m_o.name LIKE "% ' + s + '+%") OR ' +
              '(m_o.name LIKE "% ' + s + '!%") OR ' + '(m_o.name LIKE "% ' + s + '?%") OR ' +
              '(m_o.name LIKE "% ' + s + ':%") OR ' + '(m_o.name LIKE "% ' + s + ';%") OR ' +

              '(m_o.comment LIKE "% ' + s + '%") OR ' + '(m_o.comment LIKE "% ' + s + ' %") OR ' +
              '(m_o.comment LIKE "% ' + s + '.%") OR ' + '(m_o.comment LIKE "% ' + s + ',%") OR ' +
              '(m_o.comment LIKE "% ' + s + '-%") OR ' + '(m_o.comment LIKE "% ' + s + '+%") OR ' +
              '(m_o.comment LIKE "% ' + s + '!%") OR ' + '(m_o.comment LIKE "% ' + s + '?%") OR ' +
              '(m_o.comment LIKE "% ' + s + ':%") OR ' + '(m_o.comment LIKE "% ' + s + ';%") OR ' +

              '(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(TRIM(m_o.phone),"-","")," ",""),"+",""),"(",""),")","") LIKE "'
              + s + '")) ';
          end
          else
          begin
            q1 := q1 + '(' + '(m_f.name LIKE "% ' + s + '%") OR ' + '(r_o._org_address LIKE "% ' + s
              + '%") OR ' + '(m_c.name LIKE "% ' + s + '%") OR ' + '(m_s.name LIKE "% ' + s +
              '%") OR ' + '(m_f.house_num LIKE "' + s + '%") OR ' + '(m_f.house_sym LIKE "' + s +
              '%") OR ' + '(UCASE(CONCAT(m_f.house_num,m_f.house_sym)) LIKE UCASE("%' + s +
              '%")) OR ' + '(m_f.subhouse_num LIKE "' + s + '%") OR ' + '(m_f.subhouse_sym LIKE "' +
              s + '%") OR ' + '(UCASE(CONCAT(m_f.subhouse_num,m_f.subhouse_sym)) LIKE UCASE("%' + s
              + '%")) OR ' + '(m_f.flat_num LIKE "' + s + '%") OR ' + '(m_f.flat_sym LIKE "' + s +
              '%") OR ' + '(UCASE(CONCAT(m_f.flat_num,m_f.flat_sym)) LIKE UCASE("%' + s + '%")) OR '
              + '(r_o.org_how_to_reach LIKE "% ' + s + '%") OR ' + '((r_o.org_work_time LIKE "% ' +
              s + '%") OR (m_f.work LIKE "% ' + s + '%")) OR ' + '(r_o.org_other_info LIKE "% ' + s
              + '%") OR ' + '(r_m.msr_type LIKE "% ' + s + '%") OR ' + '(r_m.msr_name LIKE "% ' + s
              + '%") OR ' + '(r_m.msr_author LIKE "% ' + s + '%") OR ' +
              '(r_m.msr_producer LIKE "% ' + s + '%") OR ' + '(r_m.msr_performer LIKE "% ' + s +
              '%") OR ' + '(r_m.msr_organizer LIKE "% ' + s + '%") OR ' +
              '(r_m.msr_ticket_price LIKE "% ' + s + '%") OR ' + '(r_m.msr_other_info LIKE "% ' + s
              + '%") OR ' + '(r_p.phn_comments LIKE "% ' + s + '%") OR ' + '(m_o.name LIKE "% ' + s
              + '%") OR ' + '(m_o.comment LIKE "% ' + s + '%") OR ' +
              '(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(TRIM(m_o.phone),"-","")," ",""),"+",""),"(",""),")","") LIKE "'
              + s + '%")) ';
          end;
        end;
    ...
    begin
        ...
        begin
          ...
          begin
            q1 := q1 + '(' + '(m_f.name LIKE "% ' + s + '%") OR ' + '(m_f.name LIKE "% ' + s +
              ...
              '(m_o.name LIKE "% ' + s + '-%") OR ' + '(m_o.name LIKE "% ' + s + '+%") OR ' +
              '(m_o.name LIKE "% ' + s + '!%") OR ' + '(m_o.name LIKE "% ' + s + '?%") OR ' +
              '(m_o.name LIKE "% ' + s + ':%") OR ' + '(m_o.name LIKE "% ' + s + ';%")' + ') ';
          end
          else
          begin
            q1 := q1 + '(' + '(m_f.name LIKE "% ' + s + '%") OR ' + '(r_m.msr_type LIKE "% ' + s +
              '%") OR ' + '(r_m.msr_name LIKE "% ' + s + '%") OR ' + '(m_o.name LIKE "% ' + s +
              '%")' + ') ';
          end;
        end;
      end;
        ...
        begin
          ...
          begin
            ...
            // поиск в данных номера корпуса
              '(m_f.subhouse_num LIKE "' + s + '%") OR ' + '(m_f.subhouse_sym LIKE "' + s +
              '%") OR ' + '(UCASE(CONCAT(m_f.subhouse_num,m_f.subhouse_sym)) LIKE UCASE("%' + s +
              '%")) OR ' +
          ...
          begin
            ...
            // поиск в данных "старой" строки адреса
              ...
              '(m_f.house_sym LIKE "' + s + '%") OR ' +
              '(UCASE(CONCAT(m_f.house_num,m_f.house_sym)) LIKE UCASE("%' + s + '%")) OR ' +
              '(m_f.subhouse_num LIKE "' + s + '%") OR ' + // поиск в данных номера корпуса
              '(m_f.subhouse_sym LIKE "' + s + '%") OR ' +
              '(UCASE(CONCAT(m_f.subhouse_num,m_f.subhouse_sym)) LIKE UCASE("%' + s + '%")) OR ' +
              '(m_f.flat_num LIKE "' + s + '%") OR ' + // поиск в данных номера квартиры/офиса
              '(m_f.flat_sym LIKE "' + s + '%") OR ' +
              '(UCASE(CONCAT(m_f.flat_num,m_f.flat_sym)) LIKE UCASE("%' + s + '%")) OR ' +
              '(r_o.org_how_to_reach LIKE "% ' + s + '%")' + ') ';
          end;
        end;
      ...
      begin
        ...
        begin
          ...
          begin
            q1 := q1 + '(' + '(m_f.name LIKE "% ' + s + '%") OR ' + '((r_o.org_work_time LIKE "% ' +
              s + '%") OR (m_f.work LIKE "% ' + s + '%")) OR ' + '(r_o.org_other_info LIKE "% ' + s
              + '%") OR ' + '(r_m.msr_type LIKE "% ' + s + '%") OR ' + '(r_m.msr_name LIKE "% ' + s
              + '%") OR ' + '(r_m.msr_author LIKE "% ' + s + '%") OR ' +
              '(r_m.msr_producer LIKE "% ' + s + '%") OR ' + '(r_m.msr_performer LIKE "% ' + s +
  ...
  begin
    ...
    // строка адреса в "старом" виде (поле БД подлежит удалению в будущем)
      'COUNT(r_m.msr_id) AS x3, ' +
    // если значение больше нуля, значит у данной организации есть мероприятия
    ...
    // таблица телефонов
      'LEFT JOIN ' + Configuration.sMySQLDatabase +
      '._msr r_m ON r_m.msr_organization_id=r_o.org_id ' + // таблица мероприятий
      GetConditionalMessage((chkbxHideErased.Visible and chkbxHideErased.Enabled and
      chkbxHideErased.Checked) or (not chkbxHideErased.Visible),
      'AND (r_m.msr_erased="0" OR ISNULL(r_m.msr_erased)) ', '') +
      GetConditionalMessage(chkbxHideOld.Enabled and chkbxHideOld.Checked,
      'AND ((r_m.msr_stop_datetime>="' + FormatDateTime('yyyy-mm-dd', Now) +
      ' 00:00:00") OR ISNULL(r_m.msr_stop_datetime)) ', '') +
    ...
    // если ранее подготовленая строка условий запроса не пуста, добавляем эту строку в запрос
            ...
            begin
              ...
              begin
                ...
                begin
                  ...
                  begin
                    if (Pos('ПРОСПЕКТ', Trim(string(ResultRow[7]))) > 1) or
                    // если название улицы не содержит данного фрагмента в начале или вообще не содержит
                      (Pos('БУЛЬВАР', Trim(string(ResultRow[7]))) > 1) or
                      (Pos('ПРОЕЗД', Trim(string(ResultRow[7]))) > 1) or
                      (Pos('ПЕРЕУЛОК', Trim(string(ResultRow[7]))) > 1) or
                      (Pos('ПОСЕЛОК', Trim(string(ResultRow[7]))) > 1) or
                      (Pos('ШОССЕ', Trim(string(ResultRow[7]))) > 1) or
                      (Pos('ТУПИК', Trim(string(ResultRow[7]))) > 1) or
                      (Pos('ТРАКТ', Trim(string(ResultRow[7]))) > 1) or
                      (Pos('МИКРОРАЙОН', Trim(string(ResultRow[7]))) > 1) or
                      (Pos('ПЛОЩАДЬ', Trim(string(ResultRow[7]))) > 1) or
                      (Pos('МЕТРО', Trim(string(ResultRow[7]))) > 1) or
                      (Pos('КОЛЬЦО', Trim(string(ResultRow[7]))) > 1) or
                      (Pos('ДЕРЕВНЯ', Trim(string(ResultRow[7]))) > 1) then
                    begin
                  ...
                  if ((Trim(string(ResultRow[8])) > '') and (Trim(string(ResultRow[8])) <> '0')) or
                    (Trim(string(ResultRow[9])) > '') then
                  begin
                    if s1 > '' then
                      s1 := s1 + ', д. ';
                    s1 := s1 + Trim(string(ResultRow[8])) + Trim(string(ResultRow[9]));
                  end;
                  if ((Trim(string(ResultRow[10])) > '') and (Trim(string(ResultRow[10])) <> '0'))
                    or (Trim(string(ResultRow[11])) > '') then
                  begin
                    if s1 > '' then
                      s1 := s1 + ', корп. ';
                    s1 := s1 + Trim(string(ResultRow[10])) + Trim(string(ResultRow[11]));
                  end;
                  if ((Trim(string(ResultRow[12])) > '') and (Trim(string(ResultRow[12])) <> '0'))
                    or (Trim(string(ResultRow[13])) > '') then
                  begin
                    if s1 > '' then
                      s1 := s1 + ', кв. ';
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        // !!! тут производится Update_PhonesMark;
  ...
  // флаги
  sExternalLog_Types_1 := GetConditionalMessage(chkbxPremier.Visible and chkbxPremier.Enabled and
    chkbxPremier.Checked, 'премьеры', '');
  sExternalLog_Types_2 := GetConditionalMessage(chkbxTour.Visible and chkbxTour.Enabled and
    chkbxTour.Checked, 'гастроли', '');
  sExternalLog_Types_3 := GetConditionalMessage(chkbxChildren.Visible and chkbxChildren.Enabled and
    chkbxChildren.Checked, 'детские', '');
  sExternalLog_Types_4 := GetConditionalMessage(chkbxTeenagers.Visible and
    chkbxTeenagers.Enabled and chkbxTeenagers.Checked, 'подростковые', '');
  sExternalLog_Types_5 := GetConditionalMessage(chkbxAdultsOnly.Visible and
    chkbxAdultsOnly.Enabled and chkbxAdultsOnly.Checked, 'взрослые', '');
  sExternalLog_Types := sExternalLog_Types_1;
  sExternalLog_Types := sExternalLog_Types + GetConditionalMessage((sExternalLog_Types > '') and
    (sExternalLog_Types_2 > ''), ', ', '') + sExternalLog_Types_2;
  sExternalLog_Types := sExternalLog_Types + GetConditionalMessage((sExternalLog_Types > '') and
    (sExternalLog_Types_3 > ''), ', ', '') + sExternalLog_Types_3;
  sExternalLog_Types := sExternalLog_Types + GetConditionalMessage((sExternalLog_Types > '') and
    (sExternalLog_Types_4 > ''), ', ', '') + sExternalLog_Types_4;
  sExternalLog_Types := sExternalLog_Types + GetConditionalMessage((sExternalLog_Types > '') and
    (sExternalLog_Types_5 > ''), ', ', '') + sExternalLog_Types_5;
  // даты
  sExternalLog_Dates_Start := GetConditionalMessage(chkbxStartDate.Visible and
    chkbxStartDate.Enabled and chkbxStartDate.Checked, 'с ' + FormatDateTime('dd.mm.yyyy',
    dtpStartDate.DateTime), '');
  sExternalLog_Dates_Stop := GetConditionalMessage(chkbxStopDate.Visible and
    chkbxStopDate.Enabled and chkbxStopDate.Checked, 'по ' + FormatDateTime('dd.mm.yyyy',
    dtpStopDate.DateTime), '');
  sExternalLog_Dates := GetConditionalMessage((sExternalLog_Dates_Start + sExternalLog_Dates_Stop) >
    '', 'Период времени: ', '') + sExternalLog_Dates_Start +
    GetConditionalMessage((sExternalLog_Dates_Start > '') and (sExternalLog_Dates_Stop > ''), ' ',
    '') + sExternalLog_Dates_Stop;
  // строки поиска
  sExternalLog_Names := GetConditionalMessage( // получение строки для первого поля поиска
    edbxSearchName.Text > '',
    GetConditionalMessage(not Configuration.bShowSearchAddressOtherEditBox, 'Поиск по всем полям: ',
    'Поиск по типу и наименованию: ') + edbxSearchName.Text, '');
  if Configuration.bShowSearchAddressOtherEditBox then // если видны все три поля поиска
  begin
    sExternalLog_Names := sExternalLog_Names + GetConditionalMessage((sExternalLog_Names > '') and
      (edbxSearchAddress.Text > ''), sLineBreak, '');
    // добавляем символ перевода строки если есть первое поле и второе
    sExternalLog_Names := sExternalLog_Names + GetConditionalMessage(edbxSearchAddress.Text > '',
      'Поиск по местоположению: ' + edbxSearchAddress.Text, '');
    // добавляем второе поле если оно не пустое
    sExternalLog_Names := sExternalLog_Names + GetConditionalMessage((sExternalLog_Names > '') and
      (edbxSearchOther.Text > ''), sLineBreak, '');
    // добавляем символ перевода строки если есть одно из первых полей и третье
    sExternalLog_Names := sExternalLog_Names + GetConditionalMessage(edbxSearchOther.Text > '',
      'Поиск по "прочим данным" и наименованию: ' + edbxSearchOther.Text, '');
    // добавляем третье поле если оно не пустое
  end;

  sExternalLog_All := sExternalLog_Names;
  sExternalLog_All := sExternalLog_All + GetConditionalMessage((sExternalLog_All > '') and
    (sExternalLog_Dates > ''), sLineBreak, '');
  sExternalLog_All := sExternalLog_All + GetConditionalMessage(sExternalLog_Dates > '',
    sExternalLog_Dates, '');
  sExternalLog_All := sExternalLog_All + GetConditionalMessage((sExternalLog_All > '') and
    (sExternalLog_Types > ''), sLineBreak, '');
  sExternalLog_All := sExternalLog_All + GetConditionalMessage(sExternalLog_Types > '',
    sExternalLog_Types, '');

  sExternalLog_All := sExternalLog_All + GetConditionalMessage(sExternalLog_All > '',
    sLineBreak, '');
...
var
  ...
  i: Integer;
  i_msr_id: Integer;
  sMsrType, sMsrName, sMsrAuthor, sMsrProducer, sMsrPerformer, sMsrOrganizer,
    sMsrTicketPrice: string;
  bMsrIsPremier, bMsrIsTour, bMsrForChildren, bMsrForTeenagers, bMsrForAdultsOnly,
    bMsrUnknownDuration: boolean;
...
begin
  ...
  begin
    ...
    begin
      ...
      if i_msr_id > -1 then
        LogThis('Попытка получения данных мероприятия (ID#' + IntToStr(i_msr_id) + ').',
          LogGroupGUID, lmtInfo)
      else
      ...
      begin
        ...
        q := 'SELECT msr_type, msr_name, msr_author, msr_producer, msr_performer, msr_organizer, ' +
          ...
          + 'FROM ' + Configuration.sMySQLDatabase + '._msr LEFT JOIN ' +
          Configuration.sMySQLDatabase + '._usr uc ON uc.usr_id=msr_created_by_user_id LEFT JOIN ' +
          Configuration.sMySQLDatabase + '._usr um ON um.usr_id=msr_modified_by_user_id ' +
          ...
          begin
            ...
            begin
              ...
              begin
                ...
                sMsrTicketPrice := StringReplace(string(ResultRow[15]), '<BR>', sLineBreak,
                  [rfReplaceAll]);
                ...
                chkbxMsrStart.Checked := (Length(string(ResultRow[6])) > 0) and
                  (Length(string(ResultRow[7])) > 0);
                ...
                chkbxMsrStop.Checked := (Length(string(ResultRow[8])) > 0) and
                  (Length(string(ResultRow[9])) > 0);
                ...
                begin
                  ...
                  try
                    i := System.AnsiStrings.StrLen(ResultRow[17]);
                    msA.SetSize(i);
                ...
                begin
                  ...
                  begin
                    if sDuration > '' then
                      sDuration := sDuration + ', ';
                    sDuration := sDuration + IntToStr(spedMsrDurationDays.Value) + ' ' +
                      lblMsrDurationDays.Caption;
                  end;
                  if spedMsrDurationHours.Value > 0 then
                  begin
                    if sDuration > '' then
                      sDuration := sDuration + ', ';
                    sDuration := sDuration + IntToStr(spedMsrDurationHours.Value) + ' ' +
                      lblMsrDurationHours.Caption;
                  end;
                  if spedMsrDurationMinutes.Value > 0 then
                  begin
                    if sDuration > '' then
                      sDuration := sDuration + ', ';
                    sDuration := sDuration + IntToStr(spedMsrDurationMinutes.Value) + ' ' +
                      lblMsrDurationMinutes.Caption;
                  end;
...
begin
  ProcedureHeader('Процедура обработки нажатия кнопки "' + btnSave.Caption + '"', LogGroupGUID,
    True, True);
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          if i_org_id > -1 then
            LogThis('Попытка получения данных организации (ID#' + IntToStr(i_org_id) + ').',
              LogGroupGUID, lmtInfo)
          else
            GenerateError('Возникла ошибка в процессе получения ID организации-владельца!',
              s, bError);
          if bError then
            ShowErrorBox(s, LogGroupGUID)
          else
        ...
        if not bError then
        begin
          q := 'SELECT org_firm_code FROM ' + Configuration.sMySQLDatabase + '._org WHERE org_id=' +
            IntToStr(i_org_id) + ';';
          LogThis(q, LogGroupGUID, lmtSQL);
          i := mysql_real_query(MySQLConnectionHandler, PAnsiChar(AnsiString(q)), Length(q));
          if i = 0 then
        ...
        begin
          ...
          begin
            ...
            begin
              ...
              LogThis('Произведена отправка во внешнюю программу следующей строки данных: [' +
                sParameter + ']', LogGroupGUID, lmtInfo);
            end
            else
...
begin
  ...
  if Configuration.bOrgPanelHalfHeight then
    pnlOrgLV.Height := (pnlSearchResultsCreationModification.Height - pnlCreationModification.Height
      - Splitter2.Height) div 2;
  if Configuration.bDataPanelHalfWidth then
    pnlData.Width := (pnlConnectedMode.Width - Splitter1.Width) div 2;
end;
...
begin
  ...
  begin
    lvOrg.Column[1].Width := (lvOrg.Width - lvOrg.Column[2].Width - (lvOrg.BevelWidth * 2) - 2 -
      GetSystemMetrics(SM_CXVSCROLL)) div 2;
    lvOrg.Column[0].Width := lvOrg.Width - lvOrg.Column[2].Width - (lvOrg.BevelWidth * 2) - 2 -
      GetSystemMetrics(SM_CXVSCROLL) - lvOrg.Column[1].Width;
  end
  else
  begin
    lvOrg.Column[1].Width := (lvOrg.Width - lvOrg.Column[2].Width - (lvOrg.BevelWidth * 2)
      - 2) div 2;
    lvOrg.Column[0].Width := lvOrg.Width - lvOrg.Column[2].Width - (lvOrg.BevelWidth * 2) - 2 -
...
begin
  ...
  if (GetWindowLong(h, GWL_STYLE) and WS_VSCROLL) = WS_VSCROLL then
  begin
    lvMsr.Column[0].Width := lvMsr.Width - lvMsr.Column[2].Width - lvMsr.Column[1].Width -
      (lvMsr.BevelWidth * 2) - 2 - GetSystemMetrics(SM_CXVSCROLL)
  end
  else
  begin
    lvMsr.Column[0].Width := lvMsr.Width - lvMsr.Column[2].Width - lvMsr.Column[1].Width -
      (lvMsr.BevelWidth * 2) - 2;
  end;
...
var
  ...
  bError: boolean;
begin
  LogThis('[Процедура формирования списка строк для выпадающего списка поля "' + lblMsrType.Caption
    + '"]', LogGroupGUID, lmtInfo);
  ...
  if (cmbbxMsrType.Text <> '') then
    q := 'SELECT DISTINCT msr_type FROM ' + Configuration.sMySQLDatabase +
      '._msr WHERE UCASE(msr_type) LIKE UCASE("%' + cmbbxMsrType.Text +
      '%") AND msr_erased="0" AND (!ISNULL(msr_type)) ORDER BY msr_type;'
  else
    q := 'SELECT DISTINCT msr_type FROM ' + Configuration.sMySQLDatabase +
      '._msr WHERE msr_erased="0" AND (!ISNULL(msr_type)) ORDER BY msr_type;';
    ...
    begin
      ...
      begin
        ...
        begin
          if not bError then
          begin
            LogThis('Попытка получения очередной строки выборки (#' + IntToStr(j) + ').',
              LogGroupGUID, lmtInfo);
            ResultRow := mysql_fetch_row(ResultSet);
            if ResultRow <> nil then
            begin
              LogThis('Операция получения очередной строки выборки прошла успешно.',
                LogGroupGUID, lmtInfo);
        ...
        GenerateError('Получено недопустимое количество строк результирующей выборки (' +
          IntToStr(i) + ')!', s, bError);
    end
    else
      GenerateError('Не удалось получить результирующую выборку по последнему SQL-запросу!',
...
var
  ...
  bError: boolean;
begin
  LogThis('[Процедура формирования списка строк для выпадающего списка поля "' +
    lblMsrAuthor.Caption + '"]', LogGroupGUID, lmtInfo);
  LogThis('Начало процедуры.', LogGroupGUID, lmtInfo);
  Set_Busy(LogGroupGUID);
  Screen.Cursor := crHourGlass;
  Application.ProcessMessages;
  ...
  if (cmbbxMsrAuthor.Text <> '') then
    q := 'SELECT DISTINCT msr_author FROM ' + Configuration.sMySQLDatabase +
      '._msr WHERE UCASE(msr_author) LIKE UCASE("%' + cmbbxMsrAuthor.Text +
      '%") AND msr_erased="0" AND (!ISNULL(msr_author)) ORDER BY msr_author;'
  else
    q := 'SELECT DISTINCT msr_author FROM ' + Configuration.sMySQLDatabase +
      '._msr WHERE msr_erased="0" AND (!ISNULL(msr_author)) ORDER BY msr_author;';
...
begin
  LogThis('[Процедура формирования списка строк для выпадающего списка поля "' +
    lblMsrProducer.Caption + '"]', LogGroupGUID, lmtInfo);
  LogThis('Начало процедуры.', LogGroupGUID, lmtInfo);
  Set_Busy(LogGroupGUID);
  Screen.Cursor := crHourGlass;
  Application.ProcessMessages;
  ...
  if (cmbbxMsrProducer.Text <> '') then
    q := 'SELECT DISTINCT msr_producer FROM ' + Configuration.sMySQLDatabase +
      '._msr WHERE UCASE(msr_producer) LIKE UCASE("%' + cmbbxMsrProducer.Text +
      '%") AND msr_erased="0" AND (!ISNULL(msr_producer)) ORDER BY msr_producer;'
  else
    q := 'SELECT DISTINCT msr_producer FROM ' + Configuration.sMySQLDatabase +
      '._msr WHERE msr_erased="0" AND (!ISNULL(msr_producer)) ORDER BY msr_producer;';
...
begin
  LogThis('[Процедура формирования списка строк для выпадающего списка поля "' +
    lblMsrPerformer.Caption + '"]', LogGroupGUID, lmtInfo);
  LogThis('Начало процедуры.', LogGroupGUID, lmtInfo);
  Set_Busy(LogGroupGUID);
  Screen.Cursor := crHourGlass;
  Application.ProcessMessages;
  ...
  LogThis('Преобразование строки поиска - <' + cmbbxMsrPerformer.Text + '>.', LogGroupGUID,
    lmtInfo);
  cmbbxMsrPerformer.Text := ValidateString(cmbbxMsrPerformer.Text);
  ...
  if (cmbbxMsrPerformer.Text <> '') then
    q := 'SELECT DISTINCT msr_performer FROM ' + Configuration.sMySQLDatabase +
      '._msr WHERE UCASE(msr_performer) LIKE UCASE("%' + cmbbxMsrPerformer.Text +
      '%") AND msr_erased="0" AND (!ISNULL(msr_performer)) ORDER BY msr_performer;'
  else
    q := 'SELECT DISTINCT msr_performer FROM ' + Configuration.sMySQLDatabase +
      '._msr WHERE msr_erased="0" AND (!ISNULL(msr_performer)) ORDER BY msr_performer;';
...
begin
  LogThis('[Процедура формирования списка строк для выпадающего списка поля "' +
    lblMsrOrganizer.Caption + '"]', LogGroupGUID, lmtInfo);
  LogThis('Начало процедуры.', LogGroupGUID, lmtInfo);
  Set_Busy(LogGroupGUID);
  Screen.Cursor := crHourGlass;
  Application.ProcessMessages;
  ...
  LogThis('Преобразование строки поиска - <' + cmbbxMsrOrganizer.Text + '>.', LogGroupGUID,
    lmtInfo);
  cmbbxMsrOrganizer.Text := ValidateString(cmbbxMsrOrganizer.Text);
  ...
  if (cmbbxMsrOrganizer.Text <> '') then
    q := 'SELECT DISTINCT msr_organizer FROM ' + Configuration.sMySQLDatabase +
      '._msr WHERE UCASE(msr_organizer) LIKE UCASE("%' + cmbbxMsrOrganizer.Text +
      '%") AND msr_erased="0" AND (!ISNULL(msr_organizer)) ORDER BY msr_organizer;'
  else
    q := 'SELECT DISTINCT msr_organizer FROM ' + Configuration.sMySQLDatabase +
      '._msr WHERE msr_erased="0" AND (!ISNULL(msr_organizer)) ORDER BY msr_organizer;';
...
begin
  ProcedureHeader('Процедура обработки нажатия кнопки "' + btnClear.Caption + '"', LogGroupGUID,
    True, True);

  if PageControl1.ActivePage = tsMsr then
    if MessageBox(Handle, PChar('Вы действительно хотите очистить все поля панели данных?'),
      PChar('OA4 - Подтверждение очистки'), MB_OKCANCEL + MB_ICONQUESTION + MB_DEFBUTTON2) = IDOK
    then
    begin
      LogThis('Очистка полей ввода панели данных была подтверждена пользователем.',
        LogGroupGUID, lmtInfo);
      ...
      LogThis('Очистка полей ввоа панели данных была отменена пользователем.',
        LogGroupGUID, lmtInfo);
  ProcedureFooter(LogGroupGUID, True, True);
end;
...
begin
  ProcedureHeader('Процедура установки фокуса ввода на строку поиска по наименованию', LogGroupGUID,
    False, False);
...
begin
  ProcedureHeader('Процедура установки фокуса ввода на строку поиска по местоположению',
    LogGroupGUID, False, False);
...
begin
  ProcedureHeader('Процедура установки фокуса ввода на строку поиска по прочей информации',
    LogGroupGUID, False, False);
...
begin
  ProcedureHeader('Процедура установки фокуса ввода на список организаций', LogGroupGUID,
    False, False);
...
begin
  ProcedureHeader('Процедура установки фокуса ввода на список мероприятий', LogGroupGUID,
    False, False);
...
begin
  ProcedureHeader('Процедура установки фокуса ввода на развёрнутый список мероприятий',
    LogGroupGUID, False, False);
...
begin
  ProcedureHeader('Процедура установки фокуса ввода на поле ввода прочей информации об организации',
    LogGroupGUID, False, False);
...
begin
  ProcedureHeader('Процедура установки фокуса ввода на поле ввода прочей информации о мероприятии',
    LogGroupGUID, False, False);
...
begin
  ProcedureHeader('Процедура установки фокуса ввода на строку выпадающего списка типов мероприятий',
    LogGroupGUID, False, False);
...
begin
  ProcedureHeader('Процедура установки фокуса ввода на строку типов мероприятий', LogGroupGUID,
    False, False);
...
begin
  ProcedureHeader('Процедура передачи фокуса ввода в панель организаций главной формы',
    LogGroupGUID, True, True);
...
begin
  ProcedureHeader('Процедура передачи фокуса ввода в панель мероприятий главной формы',
    LogGroupGUID, True, True);
...
begin
  ProcedureHeader('Процедура передачи фокуса ввода в панель данных главной формы', LogGroupGUID,
    True, True);
...
begin
  ProcedureHeader
    ('Процедура обновления количества элементов и порядкового номера текущего элемента списка организаций',
    LogGroupGUID, False, False);

  if (lvOrg.Items.Count > 0) and (lvOrg.Selected <> nil) then
  begin
    lblOrgListCaption.Caption := ' Список организаций (' + IntToStr(iOrgCount) + '/' +
      IntToStr(lvOrg.Selected.index + 1) + ') ';
    LogThis('Полученная строка - <' + IntToStr(iOrgCount) + '/' + IntToStr(lvOrg.Selected.index + 1)
      + '>.', LogGroupGUID, lmtInfo);
  end
  else
  begin
...
begin
  ProcedureHeader
    ('Процедура обновления количества элементов и порядкового номера текущего элемента списка мероприятий',
    LogGroupGUID, False, False);
  ...
  begin
    lblMsrListCaption.Caption := ' Список мероприятий (' + IntToStr(iMsrCount) + '/' +
      IntToStr(lvMsr.Selected.index + 1) + ') ';
    LogThis('Полученная строка - <' + IntToStr(iMsrCount) + '/' + IntToStr(lvMsr.Selected.index + 1)
      + '>.', LogGroupGUID, lmtInfo);
  end
  else
  begin
...
begin
  ProcedureHeader('Процедура отображения всплывающей подсказки поля ввода', LogGroupGUID,
    False, False);
...
begin
  ProcedureHeader('Процедура обработки нажатия клавиши клавиатуры в основном окне приложения',
    LogGroupGUID, False, False);
...
begin
  ProcedureHeader('Процедура обновления элементов интерфейса панели поиска', LogGroupGUID,
    False, False);
...
begin
  ...
  with IniFile do
    try
      ...
      Configuration.bShowDataInOtherInfoPanel :=
        ReadBool('Интерфейс', 'bShowDataInOtherInfoPanel', False);
      Configuration.bShowMeasuresListAsRichEdit :=
        ReadBool('Интерфейс', 'bShowMeasuresListAsRichEdit', False);
      ...
      Configuration.bShowSearchAddressOtherEditBox :=
        ReadBool('Интерфейс', 'bShowSearchAddressOtherEditBox', False);
      ...
      Configuration.iGetMessagesCycleDuration :=
        ReadInteger('Интерфейс', 'iGetMessagesCycleDuration', 0);
      ...
      Configuration.iMainFormWidth := ReadInteger('Интерфейс', 'iMainFormWidth',
        DEFAULT_MAINFORMWIDTH);
      Configuration.iMainFormHeight := ReadInteger('Интерфейс', 'iMainFormHeight',
        DEFAULT_MAINFORMHEIGHT);
      ...
      Configuration.iDataPanelWidth := ReadInteger('Интерфейс', 'iDataPanelWidth',
        DEFAULT_DATAPANELWIDTH);
      ...
      Configuration.fpSetPasswordForm.bCenter :=
        ReadBool('Интерфейс', 'bSetPasswordFormCenter', True);
      ...
      Configuration.fpMaintenanceForm.bCenter :=
        ReadBool('Интерфейс', 'bMaintenanceFormCenter', True);
      ...
      Configuration.fpViewPostListForm.bCenter :=
        ReadBool('Интерфейс', 'bViewPostListFormCenter', True);
      ...
      Configuration.fpCreateViewPostForm.bCenter :=
        ReadBool('Интерфейс', 'bCreateViewPostFormCenter', True);
      ...
      Configuration.fpAddEditPhoneForm.bCenter :=
        ReadBool('Интерфейс', 'bAddEditPhoneFormCenter', True);
      ...
      Configuration.fpAddMassMsrForm.bCenter :=
        ReadBool('Интерфейс', 'bAddMassMsrFormCenter', True);
      ...
      Configuration.bDontShowQuitConfirmation :=
        ReadBool('Прочее', 'bDontShowQuitConfirmation', False);
  ...
  with IniFile do
    try
      try
        ...
        WriteBool('Интерфейс', 'bShowDataInOtherInfoPanel',
          Configuration.bShowDataInOtherInfoPanel);
        WriteBool('Интерфейс', 'bShowMeasuresListAsRichEdit',
          Configuration.bShowMeasuresListAsRichEdit);
        ...
        WriteBool('Интерфейс', 'bShowSearchAddressOtherEditBox',
          Configuration.bShowSearchAddressOtherEditBox);
        ...
        WriteInteger('Интерфейс', 'iGetMessagesCycleDuration',
          Configuration.iGetMessagesCycleDuration);
        ...
        WriteBool('Интерфейс', 'bCreateViewPostFormCenter',
          Configuration.fpCreateViewPostForm.bCenter);
...
begin
  ...
  begin
    ...
    if Configuration.bDontShowQuitConfirmation then
      ...
      b := MessageBox(Handle, PChar('Вы действительно хотите завершить работу программы?'),
        PChar('OA4 - Подтверждение выхода'), MB_OKCANCEL + MB_ICONQUESTION + MB_DEFBUTTON2) = IDOK;
    Screen.Cursor := crHourGlass;
    if Configuration.bDontShowQuitConfirmation then
      LogThis('Завершение работы приложения было подтверждено программой (установлен флаг отмены подтверждения при выходе в настройках программы).',
        LogGroupGUID, lmtInfo)
    else
    begin
      if b then
    ...
    LogThis('Завершение работы приложения было подтверждено программой (установлен флаг немедленного выхода).',
      LogGroupGUID, lmtInfo);
...
begin
  ...
  begin
    ...
    case ch0 of
      '1':
        LogThis(PChar('Получена команда получения информации об организации с кодом фирмы равным ['
          + sData + '].'), LogGroupGUID, lmtInfo); // INFO_GET_FIRMA_BY_CODE		=1;
      '2':
        LogThis(PChar('Получена команда получения информации о мероприятии с кодом отдела равным ['
          + sData + '].'), LogGroupGUID, lmtInfo); // INFO_GET_OTDEL_BY_CODE		=2;
      '3':
        LogThis(PChar('Получена команда получения информации о фирме по номеру телефона [' + sData +
          '].'), LogGroupGUID, lmtInfo); // INFO_GET_FIRMA_BY_PHONE=3;
      '4':
        LogThis(PChar('Получена команда получения информации об абоненте по номеру телефона [' +
          sData + '].'), LogGroupGUID, lmtInfo); // INFO_GET_ABON_BY_PHONE=4;
      '5':
        LogThis(PChar('Получена команда получения информации о фирме по наименованию [' + sData +
          '].'), LogGroupGUID, lmtInfo); // INFO_GET_FIRMA_BY_NAME=5;
      '6':
        LogThis(PChar('Получена команда отправки номера телефона [' + sData +
          '] на речевую карту.'), LogGroupGUID, lmtInfo); // INFO_GET_PRONOUNCE_PHONE=6;
      '7':
        LogThis(PChar('Получена команда соединения входящего соединения с номером телефона [' +
          sData + '].'), LogGroupGUID, lmtInfo); // INFO_GET_CONNECT_PHONE=7;
    end;
  end
  else
...
begin
  ProcedureHeader('Процедура очистки полного списка мероприятий (RichEdit)', LogGroupGUID,
    False, False);
...
begin
  ProcedureHeader('Процедура передачи фокуса ввода в флажков панели поиска главной формы',
    LogGroupGUID, True, True);

  if pnlConnectedMode.Visible then
  begin
    if ((ActiveControl = chkbxPremier) or (ActiveControl = chkbxChildren) or
      (ActiveControl = chkbxTeenagers) or (ActiveControl = chkbxTour) or
      (ActiveControl = chkbxAdultsOnly) or (ActiveControl = chkbxHideOld) or
      (ActiveControl = chkbxHideErased)) then
    begin
...
begin
  ProcedureHeader('Процедура передачи фокуса ввода в панель поиска главной формы', LogGroupGUID,
    True, True);

  if pnlConnectedMode.Visible then
  begin
    if ((ActiveControl = edbxSearchName) or (ActiveControl = edbxSearchAddress) or
      (ActiveControl = edbxSearchOther)) then
    begin
...
begin
  ProcedureHeader
    ('Процедура реакции на нажатие кнопки копирования данных о мероприятии во временный буфер',
    LogGroupGUID, True, True);
  ...
  begin
    ...
    raMultibuffer[j].MsrDurationDays := StrToIntDef(spedMsrDurationDays.Text,
      spedMsrDurationDays.Value);
    raMultibuffer[j].MsrDurationHours := StrToIntDef(spedMsrDurationHours.Text,
      spedMsrDurationHours.Value);
    raMultibuffer[j].MsrDurationMinutes := StrToIntDef(spedMsrDurationMinutes.Text,
      spedMsrDurationMinutes.Value);
    ...
    btbtnPasteFromMsrBuffer.Enabled := (PageControl1.ActivePage = tsMsr) and
      raMultibuffer[j].bStoredDataExists and CurrentUser.bDBEdition;
    ...
    if j > -1 then
      ...
      begin
        ...
        MessageBox(Handle, PChar(sMessage), PChar('Операция выполнена успешно'),
          MB_OK + MB_ICONWARNING);
      end
      else
        ShowErrorBox('Произошла ошибка при попытке сохранения данных о мероприятии в мультибуфере!',
          LogGroupGUID)
    else
      ShowErrorBox
        ('Произошла ошибка при попытке сохранения данных о мероприятии в мультибуфере! Буфер заполнен. Выполните предварительную очистку!',
        LogGroupGUID);
  end
  else
  begin
    ...
    MsrBufferRec.MsrDurationDays := StrToIntDef(spedMsrDurationDays.Text,
      spedMsrDurationDays.Value);
    MsrBufferRec.MsrDurationHours := StrToIntDef(spedMsrDurationHours.Text,
      spedMsrDurationHours.Value);
    MsrBufferRec.MsrDurationMinutes := StrToIntDef(spedMsrDurationMinutes.Text,
      spedMsrDurationMinutes.Value);
    ...
    btbtnPasteFromMsrBuffer.Enabled := (PageControl1.ActivePage = tsMsr) and
      MsrBufferRec.bStoredDataExists and CurrentUser.bDBEdition;
    ...
    begin
      ...
      ShowErrorBox
        ('Произошла ошибка при попытке сохранения данных о мероприятии во временном буфере!',
        LogGroupGUID);
  end;
...
begin
  ProcedureHeader('Процедура обновления вида индикатора наличия телефонных номеров', LogGroupGUID,
    False, False);
  ...
  if (((PageControl1.ActivePage = tsOrg) and (lvOrg.Selected <> nil)) or
    ((PageControl1.ActivePage = tsMsr) and (lvMsr.Selected <> nil))) then
  begin
    ...
    begin
      ...
      begin
        LogThis('Попытка получения данных организации (ID#' + IntToStr(i_org_id) + ').',
          LogGroupGUID, lmtInfo);
        q := 'SELECT COUNT(*) FROM minsk.otdel ' +
          'LEFT JOIN minsk.firma ON firma.firm_code=otdel.firm_code ' + 'LEFT JOIN ' +
          Configuration.sMySQLDatabase + '._org ON _org.org_firm_code=firma.firm_code ' +
          'LEFT JOIN ' + Configuration.sMySQLDatabase + '._phn ON _phn.phn_otdel_id=otdel.otdel_id '
          + 'WHERE  ((!(_phn.phn_owner_is_measure="1")) OR ISNULL(_phn.phn_owner_is_measure)) AND _org.org_id='
          + IntToStr(i_org_id) + ';';
    ...
    begin
      ...
      begin
        LogThis('Попытка получения данных мероприятия (ID#' + IntToStr(i_msr_id) + ').',
          LogGroupGUID, lmtInfo);
        q := 'SELECT COUNT(*) FROM ' + Configuration.sMySQLDatabase +
          '._phn LEFT JOIN minsk.otdel ON otdel_id=phn_otdel_id ' +
...
var
  ...
  sOrgWorkTime, sOrgHowToReach, sOrgHouse, sOrgSubHouse, sOrgFlat, sOrgStreet, sOrgTown,
    sOrgAddress, sOrgName, sOrgType: string;
  s1: string;

  sMsrID, sMsrCreation, sMsrModification, sMsrType, sMsrName, sMsrAuthor, sMsrProducer,
    sMsrPerformer, sMsrOrganizer, sMsrTicketPrice: string;
  ...
  begin
    ...
    reMsrOtherInfo.SelStart := 0;
    reMsrOtherInfo.SelLength := 0;
    reMsrOtherInfo.Lines.Insert(0, DupeString('-', 20) + ' ' + IntToStr(iMsrCount) + '/~~~~~~~~~~ '
      + DupeString('-', 20));
    reMsrOtherInfo.SelStart := 0;
    reMsrOtherInfo.SelLength := Length(DupeString('-', 20) + ' ' + IntToStr(iMsrCount) +
      '/~~~~~~~~~~ ' + DupeString('-', 20));
...
begin
  ProcedureHeader('Процедура реакции на выделение элемента списка организаций', LogGroupGUID,
    True, True);
    ...
    begin
      ...
      begin
        ...
        // дата и время окончания мероприятия
          ...
          GetConditionalMessage(Configuration.bShowMeasuresListAsRichEdit and
          (not CurrentUser.bDBEdition), // в режиме просмотра полного списка мероприятий
          ...
          'LEFT JOIN ' + Configuration.sMySQLDatabase +
          '._org r_o ON r_o.org_id=r_m.msr_organization_id ' + // таблица организаций
          'LEFT JOIN minsk.firma m_f ON m_f.firm_code=r_o.org_firm_code ' + // таблица фирм
        ...
        // таблица городов
          'LEFT JOIN minsk.street m_s ON m_s.street_id=m_f.street_id ' + // таблица улиц
          'LEFT JOIN ' + Configuration.sMySQLDatabase +
          '._usr r_uc ON r_uc.usr_id=r_m.msr_created_by_user_id ' + // таблица пользователей
          'LEFT JOIN ' + Configuration.sMySQLDatabase +
          '._usr r_um ON r_um.usr_id=r_m.msr_modified_by_user_id ' + // таблица пользователей
          'LEFT JOIN ' + Configuration.sMySQLDatabase +
          '._phn r_p ON r_p.phn_otdel_id=m_o.otdel_id AND r_p.phn_owner_is_measure="1" AND r_p.phn_owner_id=r_m.msr_id ' +
        // таблица телефонов
          GetConditionalMessage(Configuration.bShowMeasuresListAsRichEdit and
          (not CurrentUser.bDBEdition),
          'LEFT JOIN minsk.otdel m_o2 ON m_o2.otdel_id=r_p.phn_otdel_id ', '') + // таблица отделов
          'WHERE r_m.msr_organization_id=' + IntToStr(i_org_id) + ' ' +
          GetConditionalMessage((chkbxHideErased.Visible and chkbxHideErased.Enabled and
          chkbxHideErased.Checked) or (not chkbxHideErased.Visible), 'AND r_m.msr_erased="0" ', '')
          + GetConditionalMessage(chkbxHideOld.Enabled and chkbxHideOld.Checked,
          'AND ((r_m.msr_stop_datetime>="' + FormatDateTime('yyyy-mm-dd', Now) +
          ' 00:00:00") OR ISNULL(r_m.msr_stop_datetime)) ', '') +
          GetConditionalMessage(chkbxStartDate.Visible and chkbxStartDate.Enabled and
          chkbxStartDate.Checked, 'AND (ISNULL(r_m.msr_stop_datetime) OR (r_m.msr_stop_datetime>="'
          + FormatDateTime('yyyy-mm-dd hh:nn:ss', dtpStartDate.DateTime) + '")) ', '') +
        // будут отобраны все мероприятия не окончившиеся на указанную дату
          GetConditionalMessage(chkbxStopDate.Visible and chkbxStopDate.Enabled and
          chkbxStopDate.Checked, 'AND (ISNULL(r_m.msr_start_datetime) OR (r_m.msr_start_datetime<="'
          + FormatDateTime('yyyy-mm-dd hh:nn:ss', dtpStopDate.DateTime) + '")) ', '') +
        // будут указаны все мероприятия начинающиеся не позднее указанной даты
          GetConditionalMessage(chkbxPremier.Visible and chkbxPremier.Enabled and
          chkbxPremier.Checked, 'AND r_m.msr_is_premier="1" ', '') +
          GetConditionalMessage(chkbxTour.Visible and chkbxTour.Enabled and chkbxTour.Checked,
          'AND r_m.msr_is_tour="1" ', '') + GetConditionalMessage(chkbxChildren.Visible and
          chkbxChildren.Enabled and chkbxChildren.Checked, 'AND r_m.msr_for_children="1" ', '') +
          GetConditionalMessage(chkbxTeenagers.Visible and chkbxTeenagers.Enabled and
          chkbxTeenagers.Checked, 'AND r_m.msr_for_teenagers="1" ', '') +
          GetConditionalMessage(chkbxAdultsOnly.Visible and chkbxAdultsOnly.Enabled and
          chkbxAdultsOnly.Checked, 'AND r_m.msr_for_adults_only="1" ', '');
          ...
          begin
            ...
            begin
              ...
              q := q + GetConditionalMessage(q > '', 'AND ', '') +
                ...
                '(' + '(m_f.name LIKE "% ' + s + '%") OR ' + '(m_f.name LIKE "% ' + s + ' %") OR ' +
                '(m_f.name LIKE "% ' + s + '.%") OR ' + '(m_f.name LIKE "% ' + s + ',%") OR ' +
                '(m_f.name LIKE "% ' + s + '-%") OR ' + '(m_f.name LIKE "% ' + s + '+%") OR ' +
                '(m_f.name LIKE "% ' + s + '!%") OR ' + '(m_f.name LIKE "% ' + s + '?%") OR ' +
                '(m_f.name LIKE "% ' + s + ':%") OR ' + '(m_f.name LIKE "% ' + s + ';%") OR ' +
                ...
                '(r_o._org_address LIKE "% ' + s + '!%") OR ' + '(r_o._org_address LIKE "% ' + s +
                '?%") OR ' + '(r_o._org_address LIKE "% ' + s + ':%") OR ' +
                '(r_o._org_address LIKE "% ' + s + ';%") OR ' + '(m_c.name LIKE "% ' + s + '%") OR '
                + '(m_c.name LIKE "% ' + s + ' %") OR ' + '(m_c.name LIKE "% ' + s + '.%") OR ' +
                '(m_c.name LIKE "% ' + s + ',%") OR ' + '(m_c.name LIKE "% ' + s + '-%") OR ' +
                '(m_c.name LIKE "% ' + s + '+%") OR ' + '(m_c.name LIKE "% ' + s + '!%") OR ' +
                '(m_c.name LIKE "% ' + s + '?%") OR ' + '(m_c.name LIKE "% ' + s + ':%") OR ' +
                '(m_c.name LIKE "% ' + s + ';%") OR ' + '(m_s.name LIKE "% ' + s + '%") OR ' +
                '(m_s.name LIKE "% ' + s + ' %") OR ' + '(m_s.name LIKE "% ' + s + '.%") OR ' +
                '(m_s.name LIKE "% ' + s + ',%") OR ' + '(m_s.name LIKE "% ' + s + '-%") OR ' +
                '(m_s.name LIKE "% ' + s + '+%") OR ' + '(m_s.name LIKE "% ' + s + '!%") OR ' +
                '(m_s.name LIKE "% ' + s + '?%") OR ' + '(m_s.name LIKE "% ' + s + ':%") OR ' +
                '(m_s.name LIKE "% ' + s + ';%") OR ' + '(m_f.house_num LIKE "' + s + '%") OR ' +
                '(m_f.house_sym LIKE "' + s + '%") OR ' +
                '(UCASE(CONCAT(m_f.house_num,m_f.house_sym)) LIKE UCASE("%' + s + '%")) OR ' +
                '(m_f.subhouse_num LIKE "' + s + '%") OR ' + '(m_f.subhouse_sym LIKE "' + s +
                '%") OR ' + '(UCASE(CONCAT(m_f.subhouse_num,m_f.subhouse_sym)) LIKE UCASE("%' + s +
                '%")) OR ' + '(m_f.flat_num LIKE "' + s + '%") OR ' + '(m_f.flat_sym LIKE "' + s +
                '%") OR ' + '(UCASE(CONCAT(m_f.flat_num,m_f.flat_sym)) LIKE UCASE("%' + s +
                '%")) OR ' + '(r_o.org_how_to_reach LIKE "% ' + s + '%") OR ' +
                '(r_o.org_how_to_reach LIKE "% ' + s + ' %") OR ' + '(r_o.org_how_to_reach LIKE "% '
                + s + '.%") OR ' + '(r_o.org_how_to_reach LIKE "% ' + s + ',%") OR ' +
                '(r_o.org_how_to_reach LIKE "% ' + s + '-%") OR ' + '(r_o.org_how_to_reach LIKE "% '
                + s + '+%") OR ' + '(r_o.org_how_to_reach LIKE "% ' + s + '!%") OR ' +
                ...
                '((r_o.org_work_time LIKE "% ' + s + '%") OR (m_f.work LIKE "% ' + s + '%")) OR ' +
                '((r_o.org_work_time LIKE "% ' + s + ' %") OR (m_f.work LIKE "% ' + s + ' %")) OR '
                + '((r_o.org_work_time LIKE "% ' + s + '.%") OR (m_f.work LIKE "% ' + s +
                '.%")) OR ' + '((r_o.org_work_time LIKE "% ' + s + ',%") OR (m_f.work LIKE "% ' + s
                + ',%")) OR ' + '((r_o.org_work_time LIKE "% ' + s + '-%") OR (m_f.work LIKE "% ' +
                s + '-%")) OR ' + '((r_o.org_work_time LIKE "% ' + s + '+%") OR (m_f.work LIKE "% '
                + s + '+%")) OR ' + '((r_o.org_work_time LIKE "% ' + s +
                '!%") OR (m_f.work LIKE "% ' + s + '!%")) OR ' + '((r_o.org_work_time LIKE "% ' + s
                + '?%") OR (m_f.work LIKE "% ' + s + '?%")) OR ' + '((r_o.org_work_time LIKE "% ' +
                s + ':%") OR (m_f.work LIKE "% ' + s + ':%")) OR ' + '((r_o.org_work_time LIKE "% '
                + s + ';%") OR (m_f.work LIKE "% ' + s + ';%")) OR ' +
                '(r_o.org_other_info LIKE "% ' + s + '%") OR ' + '(r_o.org_other_info LIKE "% ' + s
                + ' %") OR ' + '(r_o.org_other_info LIKE "% ' + s + '.%") OR ' +
                '(r_o.org_other_info LIKE "% ' + s + ',%") OR ' + '(r_o.org_other_info LIKE "% ' + s
                + '-%") OR ' + '(r_o.org_other_info LIKE "% ' + s + '+%") OR ' +
                '(r_o.org_other_info LIKE "% ' + s + '!%") OR ' + '(r_o.org_other_info LIKE "% ' + s
                + '?%") OR ' + '(r_o.org_other_info LIKE "% ' + s + ':%") OR ' +
                '(r_o.org_other_info LIKE "% ' + s + ';%") OR ' + '(r_m.msr_type LIKE "% ' + s +
                '%") OR ' + '(r_m.msr_type LIKE "% ' + s + ' %") OR ' + '(r_m.msr_type LIKE "% ' + s
                + '.%") OR ' + '(r_m.msr_type LIKE "% ' + s + ',%") OR ' + '(r_m.msr_type LIKE "% '
                + s + '-%") OR ' + '(r_m.msr_type LIKE "% ' + s + '+%") OR ' +
                '(r_m.msr_type LIKE "% ' + s + '!%") OR ' + '(r_m.msr_type LIKE "% ' + s +
                '?%") OR ' + '(r_m.msr_type LIKE "% ' + s + ':%") OR ' + '(r_m.msr_type LIKE "% ' +
                s + ';%") OR ' + '(r_m.msr_name LIKE "% ' + s + '%") OR ' + '(r_m.msr_name LIKE "% '
                + s + ' %") OR ' + '(r_m.msr_name LIKE "% ' + s + '.%") OR ' +
                '(r_m.msr_name LIKE "% ' + s + ',%") OR ' + '(r_m.msr_name LIKE "% ' + s +
                '-%") OR ' + '(r_m.msr_name LIKE "% ' + s + '+%") OR ' + '(r_m.msr_name LIKE "% ' +
                s + '!%") OR ' + '(r_m.msr_name LIKE "% ' + s + '?%") OR ' +
                '(r_m.msr_name LIKE "% ' + s + ':%") OR ' + '(r_m.msr_name LIKE "% ' + s +
                ...
                '(r_m.msr_author LIKE "% ' + s + '!%") OR ' + '(r_m.msr_author LIKE "% ' + s +
                '?%") OR ' + '(r_m.msr_author LIKE "% ' + s + ':%") OR ' +
                '(r_m.msr_author LIKE "% ' + s + ';%") OR ' + '(r_m.msr_producer LIKE "% ' + s +
                '%") OR ' + '(r_m.msr_producer LIKE "% ' + s + ' %") OR ' +
                '(r_m.msr_producer LIKE "% ' + s + '.%") OR ' + '(r_m.msr_producer LIKE "% ' + s +
                ',%") OR ' + '(r_m.msr_producer LIKE "% ' + s + '-%") OR ' +
                '(r_m.msr_producer LIKE "% ' + s + '+%") OR ' + '(r_m.msr_producer LIKE "% ' + s +
                ...
                '(r_m.msr_organizer LIKE "% ' + s + '!%") OR ' + '(r_m.msr_organizer LIKE "% ' + s +
                '?%") OR ' + '(r_m.msr_organizer LIKE "% ' + s + ':%") OR ' +
                '(r_m.msr_organizer LIKE "% ' + s + ';%") OR ' + '(r_m.msr_ticket_price LIKE "% ' +
                s + '%") OR ' + '(r_m.msr_ticket_price LIKE "% ' + s + ' %") OR ' +
                '(r_m.msr_ticket_price LIKE "% ' + s + '.%") OR ' + '(r_m.msr_ticket_price LIKE "% '
                + s + ',%") OR ' + '(r_m.msr_ticket_price LIKE "% ' + s + '-%") OR ' +
                '(r_m.msr_ticket_price LIKE "% ' + s + '+%") OR ' + '(r_m.msr_ticket_price LIKE "% '
                + s + '!%") OR ' + '(r_m.msr_ticket_price LIKE "% ' + s + '?%") OR ' +
                '(r_m.msr_ticket_price LIKE "% ' + s + ':%") OR ' + '(r_m.msr_ticket_price LIKE "% '
                + s + ';%") OR ' + '(r_m.msr_other_info LIKE "% ' + s + '%") OR ' +
                '(r_m.msr_other_info LIKE "% ' + s + ' %") OR ' + '(r_m.msr_other_info LIKE "% ' + s
                + '.%") OR ' + '(r_m.msr_other_info LIKE "% ' + s + ',%") OR ' +
                '(r_m.msr_other_info LIKE "% ' + s + '-%") OR ' + '(r_m.msr_other_info LIKE "% ' + s
                + '+%") OR ' + '(r_m.msr_other_info LIKE "% ' + s + '!%") OR ' +
                ...
                '(r_p.phn_comments LIKE "% ' + s + '!%") OR ' + '(r_p.phn_comments LIKE "% ' + s +
                '?%") OR ' + '(r_p.phn_comments LIKE "% ' + s + ':%") OR ' +
                '(r_p.phn_comments LIKE "% ' + s + ';%") OR ' + '(m_o.name LIKE "% ' + s + '%") OR '
                + '(m_o.name LIKE "% ' + s + ' %") OR ' + '(m_o.name LIKE "% ' + s + '.%") OR ' +
                '(m_o.name LIKE "% ' + s + ',%") OR ' + '(m_o.name LIKE "% ' + s + '-%") OR ' +
                '(m_o.name LIKE "% ' + s + '+%") OR ' + '(m_o.name LIKE "% ' + s + '!%") OR ' +
                '(m_o.name LIKE "% ' + s + '?%") OR ' + '(m_o.name LIKE "% ' + s + ':%") OR ' +
                '(m_o.name LIKE "% ' + s + ';%") OR ' + '(m_o.comment LIKE "% ' + s + '%") OR ' +
                '(m_o.comment LIKE "% ' + s + ' %") OR ' + '(m_o.comment LIKE "% ' + s + '.%") OR '
                + '(m_o.comment LIKE "% ' + s + ',%") OR ' + '(m_o.comment LIKE "% ' + s +
                '-%") OR ' + '(m_o.comment LIKE "% ' + s + '+%") OR ' + '(m_o.comment LIKE "% ' + s
                + '!%") OR ' + '(m_o.comment LIKE "% ' + s + '?%") OR ' + '(m_o.comment LIKE "% ' +
                s + ':%") OR ' + '(m_o.comment LIKE "% ' + s + ';%") OR ' +
                '(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(TRIM(m_o.phone),"-","")," ",""),"+",""),"(",""),")","") LIKE "'
                + s + '")) ',

                '(' + '(m_f.name LIKE "% ' + s + '%") OR ' + '(r_o._org_address LIKE "% ' + s +
                '%") OR ' + '(m_c.name LIKE "% ' + s + '%") OR ' + '(m_s.name LIKE "% ' + s +
                '%") OR ' + '(m_f.house_num LIKE "' + s + '%") OR ' + '(m_f.house_sym LIKE "' + s +
                '%") OR ' + '(UCASE(CONCAT(m_f.house_num,m_f.house_sym)) LIKE UCASE("%' + s +
                '%")) OR ' + '(m_f.subhouse_num LIKE "' + s + '%") OR ' + '(m_f.subhouse_sym LIKE "'
                + s + '%") OR ' + '(UCASE(CONCAT(m_f.subhouse_num,m_f.subhouse_sym)) LIKE UCASE("%'
                + s + '%")) OR ' + '(m_f.flat_num LIKE "' + s + '%") OR ' + '(m_f.flat_sym LIKE "' +
                s + '%") OR ' + '(UCASE(CONCAT(m_f.flat_num,m_f.flat_sym)) LIKE UCASE("%' + s +
                '%")) OR ' + '(r_o.org_how_to_reach LIKE "% ' + s + '%") OR ' +
                '((r_o.org_work_time LIKE "% ' + s + '%") OR (m_f.work LIKE "% ' + s + '%")) OR ' +
                '(r_o.org_other_info LIKE "% ' + s + '%") OR ' + '(r_m.msr_type LIKE "% ' + s +
                '%") OR ' + '(r_m.msr_name LIKE "% ' + s + '%") OR ' + '(r_m.msr_author LIKE "% ' +
                s + '%") OR ' + '(r_m.msr_producer LIKE "% ' + s + '%") OR ' +
                '(r_m.msr_performer LIKE "% ' + s + '%") OR ' + '(r_m.msr_organizer LIKE "% ' + s +
                '%") OR ' + '(r_m.msr_ticket_price LIKE "% ' + s + '%") OR ' +
                '(r_m.msr_other_info LIKE "% ' + s + '%") OR ' + '(r_p.phn_comments LIKE "% ' + s +
                '%") OR ' + '(m_o.name LIKE "% ' + s + '%") OR ' + '(m_o.comment LIKE "% ' + s +
                '%") OR ' +
                '(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(TRIM(m_o.phone),"-","")," ",""),"+",""),"(",""),")","") LIKE "'
                + s + '%")) ');
            end;
          ...
          begin
            ...
            begin
              ...
              q := q + GetConditionalMessage(q > '', 'AND ', '') +
                ...
                '(' + '(m_f.name LIKE "% ' + s + '%") OR ' + '(r_m.msr_type LIKE "% ' + s +
                '%") OR ' + '(r_m.msr_name LIKE "% ' + s + '%") OR ' + '(m_o.name LIKE "% ' + s +
                '%")' + ') ');
            end;
          end;
          ...
          begin
            ...
            begin
              ...
              q := q + GetConditionalMessage(q > '', 'AND ', '') +
                ...
                '(' + '(m_f.name LIKE "% ' + s + '%") OR ' + '((r_o.org_work_time LIKE "% ' + s +
                '%") OR (m_f.work LIKE "% ' + s + '%")) OR ' + '(r_o.org_other_info LIKE "% ' + s +
                '%") OR ' + '(r_m.msr_type LIKE "% ' + s + '%") OR ' + '(r_m.msr_name LIKE "% ' + s
                + '%") OR ' + '(r_m.msr_author LIKE "% ' + s + '%") OR ' +
                '(r_m.msr_producer LIKE "% ' + s + '%") OR ' + '(r_m.msr_performer LIKE "% ' + s +
                '%") OR ' + '(r_m.msr_organizer LIKE "% ' + s + '%") OR ' +
                '(r_m.msr_ticket_price LIKE "% ' + s + '%") OR ' + '(r_m.msr_other_info LIKE "% ' +
                s + '%") OR ' + '(r_p.phn_comments LIKE "% ' + s + '%") OR ' + '(m_o.name LIKE "% '
                + s + '%") OR ' + '(m_o.comment LIKE "% ' + s + '%") OR ' +
                '(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(TRIM(m_o.phone),"-","")," ",""),"+",""),"(",""),")","") LIKE "'
                + s + '%")) ');
            end;
        ...
        q := q + 'GROUP BY r_m.msr_id' + GetConditionalMessage
          (Configuration.bShowMeasuresListAsRichEdit and (not CurrentUser.bDBEdition),
          ', r_p.phn_id', '') + ' ORDER BY x0, ';
        ...
        q := q + GetConditionalMessage(Configuration.bShowMeasuresListAsRichEdit and
          (not CurrentUser.bDBEdition), ', r_p.phn_priority', '') + ';';
                  ...
                  begin
                    ...
                    begin
                      ...
                      begin
                        ...
                        bMsrUnknownDuration := string(ResultRow[22]) = '1';
                        sMsrTicketPrice := StringReplace(string(ResultRow[23]), '<BR>', sLineBreak,
                          [rfReplaceAll]);
                        ...
                        begin
                          ...
                          msA := TMemoryStream.Create;
                          try
                            k := System.AnsiStrings.StrLen(ResultRow[25]);
                            msA.SetSize(k);
                            msA.Seek(0, soFromBeginning);
                        ...
                        // если длительность не является неизвестной - попытаться высчитать длительность на основе начала и окончания
                        begin
                          dtStartDateTime := StrToDateTimeDef(Trim(sStartDateTime),
                            EncodeDate(1900, 1, 1));
                          if dtStartDateTime <> EncodeDate(1900, 1, 1) then
                          begin
                            dtStopDateTime := StrToDateTimeDef(Trim(sStopDateTime),
                              EncodeDate(1900, 1, 1));
                            ...
                            begin
                              ...
                              begin
                                if sDuration > '' then
                                  sDuration := sDuration + ', ';
                                sDuration := sDuration + IntToStr(iDays) + ' ' +
                                  lblMsrDurationDays.Caption;
                              end;
                              if iHours > 0 then
                              begin
                                if sDuration > '' then
                                  sDuration := sDuration + ', ';
                                sDuration := sDuration + IntToStr(iHours) + ' ' +
                                  lblMsrDurationHours.Caption;
                              end;
                              if iMinutes > 0 then
                              begin
                                if sDuration > '' then
                                  sDuration := sDuration + ', ';
                                sDuration := sDuration + IntToStr(iMinutes) + ' ' +
                                  lblMsrDurationMinutes.Caption;
                              end;
                            end;
                          end;
                        ...
                        { iPhonesCounter:=1; }
                        sPhones := '';
                        if Trim('' + string(ResultRow[30]) + string(ResultRow[31]) +
                          string(ResultRow[32])) > '' then
                          sPhones := { IntToStr(iPhonesCounter)+'.'+#09+ } string(ResultRow[30]) +
                            #09 + string(ResultRow[31]) + #09 + string(ResultRow[32]);
                      end
                      else
                      begin
                        // добавление нового телефона к значению переменной sPnones;
                        if (sPhones > '') and
                          (Trim('' + string(ResultRow[30]) + string(ResultRow[31]) +
                          string(ResultRow[32])) > '') then
                          sPhones := sPhones + sLineBreak;
                        sPhones := sPhones + string(ResultRow[30]) + #09 + string(ResultRow[31]) +
                          #09 + string(ResultRow[32]);
                      end;
                      if j = (i - 1) then
                        BuildMsrView(True); // вывод мероприятия
                    end
                    else
                      GenerateError('Возникла ошибка при загрузке данных строки выборки!',
                        s, bError);
                    ProgressBar1.StepIt;
                  end;
                end;
                ...
                try
                  ...
                  begin
                    ...
                    while Pos('/~~~~~~~~~~', reMsr.Lines[k]) > 0 do
                      reMsr.Lines[k] := StringReplace(reMsr.Lines[k], '/~~~~~~~~~~',
                        '/' + IntToStr(iMsrCount), [rfReplaceAll]);
                  end;
                  reMsr.Lines.EndUpdate;
                finally
                  TextStream.Free;
      ...
      // обновление вида окна в зависимости от количества найденых результатов
      ...
      begin
        ...
        SetDefaultSettings(reOrgOtherInfo);
        LogThis('Попытка получения данных организации (ID#' + lvOrg.Selected.SubItems[2] + ').',
          LogGroupGUID, lmtInfo);
      end;

      if not bError then
      begin
        q := 'SELECT ' + 'CONCAT(" ",TRIM(SUBSTRING_INDEX(m_f.name,"|",-1))), ' +
          'CONCAT(" ",TRIM(SUBSTRING_INDEX(m_f.name,"|",1))), ' + 'r_o._org_address, ' +
          'm_c.name, ' + 'm_s.name, ' + 'm_f.house_num, ' + 'm_f.house_sym, ' + 'm_f.subhouse_num, '
          + 'm_f.subhouse_sym, ' + 'm_f.flat_num, ' + 'm_f.flat_sym, ' + 'r_o.org_how_to_reach, ' +
          'IFNULL(r_o.org_work_time,m_f.work), ' + 'r_o.org_other_info, ' +
          'r_o.org_other_info_rtf, ' + 'CONCAT(r_uc.usr_fullname,", ",r_o.org_created_at), ' +
          'CONCAT(r_um.usr_fullname,", ",r_o.org_modified_at) ' + 'FROM ' +
          Configuration.sMySQLDatabase + '._org r_o ' +
          'LEFT JOIN minsk.firma m_f ON m_f.firm_code=r_o.org_firm_code ' +
          'LEFT JOIN minsk.city m_c ON m_c.city_id=m_f.city_id and m_c.alt="0" ' +
          'LEFT JOIN minsk.street m_s ON m_s.street_id=m_f.street_id ' + 'LEFT JOIN ' +
          Configuration.sMySQLDatabase + '._usr r_uc ON r_uc.usr_id=r_o.org_created_by_user_id ' +
          'LEFT JOIN ' + Configuration.sMySQLDatabase +
          '._usr r_um ON r_um.usr_id=r_o.org_modified_by_user_id ' + 'WHERE r_o.org_id=' +
          IntToStr(i_org_id) + ';';
        LogThis(q, LogGroupGUID, lmtSQL);
      end;
            ...
            begin
              ...
              begin
                ...
                begin
                  msA := TMemoryStream.Create;
                  try
                    k := System.AnsiStrings.StrLen(ResultRow[14]);
                    msA.SetSize(k);
                    msA.Seek(0, soFromBeginning);
                ...
                begin
                  reOrgOtherInfo.SelStart := 0;
                  reOrgOtherInfo.SelLength := 0;
                  reOrgOtherInfo.Lines.Insert(0, 'Как добраться / Местоположение:' + sLineBreak +
                    sOrgHowToReach);
                  iSelLenght := Length('Как добраться / Местоположение:' + sLineBreak +
                    sOrgHowToReach);
                  reOrgOtherInfo.SelStart := 0;
                  reOrgOtherInfo.SelLength := iSelLenght;
                  reOrgOtherInfo.SelAttributes.Color := clBlack;
                  ...
                  iSelLenght := Length(sOrgWorkTime);
                  reOrgOtherInfo.SelStart :=
                    Length('Как добраться / Местоположение:' + sLineBreak) - 1;
                  reOrgOtherInfo.SelLength := iSelLenght;
                  reOrgOtherInfo.SelAttributes.name := 'Lucida Console';
                ...
                begin
                  ...
                  begin
                    if (Pos('ПРОСПЕКТ', Trim(sOrgStreet)) > 0) or
                      ...
                      (Pos('ПОСЕЛОК', Trim(sOrgStreet)) > 0) or (Pos('ШОССЕ', Trim(sOrgStreet)) > 0)
                      or (Pos('ТУПИК', Trim(sOrgStreet)) > 0) or
                ...
                begin
                  reOrgOtherInfo.SelStart := 0;
                  reOrgOtherInfo.SelLength := 0;
                  reOrgOtherInfo.Lines.Insert(0, 'Наименование организации:' + sLineBreak +
                    sOrgName);
  ...
  begin
    ...
    for i := 0 to (SizeOf(raMultibuffer) div (SizeOf(TMsrBufferRec))) - 1 do
      if raMultibuffer[i].bStoredDataExists then
        b := True;
    btbtnPasteFromMsrBuffer.Enabled := b and (PageControl1.ActivePage = tsMsr) and
      CurrentUser.bDBEdition;;
  end
  else
    btbtnPasteFromMsrBuffer.Enabled := (PageControl1.ActivePage = tsMsr) and
...
begin
  ...
  with OptionsForm do
    try
      ...
      chkbxStoreLastPassword.Checked := chkbxStoreLastLogin.Enabled and
        Configuration.bStoreLastPassword;
      chkbxAutoLogon.Enabled := chkbxStoreLastLogin.Checked and chkbxStoreLastPassword.Checked;
      chkbxAutoLogon.Checked := chkbxStoreLastLogin.Enabled and chkbxStoreLastPassword.Checked and
        Configuration.bAutoLogon;
      ...
      begin
        ...
        q := 'SELECT arl_source, arl_destination FROM ' + Configuration.sMySQLDatabase +
          '._autoreplace ORDER BY arl_id;';
        LogThis(q, LogGroupGUID, lmtSQL);
        i := mysql_real_query(MySQLConnectionHandler, PAnsiChar(AnsiString(q)), Length(q));
        if i = 0 then
        begin
          ...
          begin
            ...
            begin
              ...
              for j := 0 to i - 1 do
              begin
                LogThis('Получение очередной строки выборки (' + IntToStr(j) + ').',
                  LogGroupGUID, lmtInfo);
                ResultRow := mysql_fetch_row(ResultSet);
                if ResultRow <> nil then
                begin
                  LogThis('Операция получения очередной строки выборки (' + IntToStr(j) +
                    ') прошла успешно.', LogGroupGUID, lmtInfo);
                  sgAutoReplaceList.Cells[1, j + 1] := string(ResultRow[0]);
                  sgAutoReplaceList.Cells[2, j + 1] := string(ResultRow[1]);
                end
                else
                  GenerateError
                    ('Возникла ошибка при загрузке данных строки выборки (порядковый номер строки равен '
                    + IntToStr(j) + ')!', s, bError);
                ProgressBar1.StepIt;
              end;
            end
            else
              GenerateError('Возникла ошибка при получении колчиества срок результирующей выборки!',
                s, bError);
          end
          else
            GenerateError('Не удалось получить результирующую выборку по последнему SQL-запросу!',
      ...
      edbxMainFormPositionX.Enabled :=
        not(Configuration.bFullScreen or Configuration.fpMainForm.bCenter);
      ...
      begin
        ...
        Configuration.fpMainForm.bCenter := chkbxMainFormPositionByCenter.Enabled and
          chkbxMainFormPositionByCenter.Checked;

        if edbxMainFormWidth.Enabled then
          Configuration.iMainFormWidth := StrToIntDef(edbxMainFormWidth.Text,
            DEFAULT_MAINFORMWIDTH);

        if edbxMainFormHeight.Enabled then
          Configuration.iMainFormHeight := StrToIntDef(edbxMainFormHeight.Text,
            DEFAULT_MAINFORMHEIGHT);

        Configuration.bFullScreen := chkbxStartupFullScreen.Checked and
          chkbxStartupFullScreen.Enabled;

        Configuration.bDataPanelHalfWidth := chkbxDataPanelHalfWidth.Checked;

        if edbxDataPanelWidth.Enabled then
          Configuration.iDataPanelWidth := StrToIntDef(edbxDataPanelWidth.Text,
            DEFAULT_DATAPANELWIDTH);

        Configuration.bOrgPanelHalfHeight := chkbxOrgPanelHalfHeight.Checked;

        if edbxOrgPanelHeight.Enabled then
          Configuration.iOrgPanelHeight := StrToIntDef(edbxOrgPanelHeight.Text,
            DEFAULT_ORGPANELHEIGHT);
        ...
        if Configuration.bOrgPanelHalfHeight then
          pnlOrgLV.Height := (pnlSearchResultsCreationModification.Height -
            pnlCreationModification.Height - Splitter2.Height) div 2
        else
          pnlOrgLV.Height := Configuration.iOrgPanelHeight;
        ...
        Configuration.bStoreLastLogin := chkbxStoreLastLogin.Checked and
          chkbxStoreLastLogin.Enabled;
        Configuration.bStoreLastPassword := chkbxStoreLastPassword.Checked and
          chkbxStoreLastPassword.Enabled;
        Configuration.bAutoLogon := chkbxAutoLogon.Checked and chkbxAutoLogon.Enabled;
        Configuration.bShowDataInOtherInfoPanel := chkbxShowDataInOtherInfoPanel.Checked and
          chkbxShowDataInOtherInfoPanel.Enabled;
        Configuration.bShowMeasuresListAsRichEdit := chkbxShowMeasuresListAsRichEdit.Checked and
          chkbxShowMeasuresListAsRichEdit.Enabled;
        Configuration.bMarkSearchedStrings := chkbxMarkSearchedStrings.Checked and
          chkbxMarkSearchedStrings.Enabled;
        Configuration.bPutTownAtTheEnd := chkbxPutTownAtTheEnd.Checked and
          chkbxPutTownAtTheEnd.Enabled;
        Configuration.bHideEditBoxToolTips := chkbxHideEditBoxToolTips.Checked and
          chkbxHideEditBoxToolTips.Enabled;
        Configuration.bShowSearchAddressOtherEditBox :=
          chkbxShowSearchAddressOtherEditBox.Checked and chkbxShowSearchAddressOtherEditBox.Enabled;
        Configuration.bUseMultibuffer := chkbxUseMultiBuffer.Checked and
          chkbxUseMultiBuffer.Enabled;

        Configuration.bGetMessages := chkbxGetMessages.Checked and chkbxGetMessages.Enabled;;
        Configuration.iGetMessagesCycleDuration :=
          StrToIntDef(edbxGetMessagesCycleDuration.Text, 1);
        Update_GetMessagesTimerState;
        ...
        Configuration.fpCreateViewPostForm.x :=
          StrToIntDef(edbxCreateViewPostFormPositionX.Text, 0);
        Configuration.fpCreateViewPostForm.y :=
          StrToIntDef(edbxCreateViewPostFormPositionY.Text, 0);
        Configuration.fpCreateViewPostForm.bCenter :=
          chkbxCreateViewPostFormPositionByCenter.Checked;
        ...
        begin
          ...
          begin
            ...
            begin
              if (sgAutoReplaceList.Cells[1, j] <> '') and (sgAutoReplaceList.Cells[2, j] <> '')
              then
              begin
                k := k + 1;
                q := 'INSERT INTO ' + Configuration.sMySQLDatabase + '._autoreplace VALUES (' +
                  IntToStr(k) + ', "' + sgAutoReplaceList.Cells[1, j] + '", "' +
                  sgAutoReplaceList.Cells[2, j] + '")';
                LogThis(q, LogGroupGUID, lmtSQL);
                i := mysql_real_query(MySQLConnectionHandler, PAnsiChar(AnsiString(q)), Length(q));
                if i = 0 then
                begin
                  ...
                  begin
                    LogThis('Количество строк выборки соответствует требуемому.',
                      LogGroupGUID, lmtInfo);
                    LogThis('Элемент списка автозамены номер ' + IntToStr(k) +
                      ' успешно сохранён в таблице БД.', LogGroupGUID, lmtInfo);
                  end
                  else
                    GenerateError('Количество обработанных строк (' + IntToStr(i) +
                      ') не соответствует требуемому (1)!', s, bError);
...
begin
  ...
  with CreateViewPostForm do
    try
      ...
      q := 'SELECT DISTINCT usr_fullname FROM ' + Configuration.sMySQLDatabase +
        '._usr WHERE usr_erased="0" ORDER BY usr_fullname;';
      LogThis(q, LogGroupGUID, lmtSQL);
      i := mysql_real_query(MySQLConnectionHandler, PAnsiChar(AnsiString(q)), Length(q));
      if i = 0 then
      begin
      ...
      begin
        ...
        try
          Screen.Cursor := crHourGlass;
          if ((cmbbxTo.Text = '[ группа администраторов ]') or
            (cmbbxTo.Text = '[ группа операторов ]') or (cmbbxTo.Text = '[ группа пользователей ]'))
          then
          begin
            if cmbbxTo.Text = '[ группа администраторов ]' then
            begin
              LogThis('Получение списка идентификаторов администраторов...', LogGroupGUID, lmtInfo);
              q := 'SELECT DISTINCT usr_id FROM ' + Configuration.sMySQLDatabase +
                '._usr WHERE usr_erased="0" and usr_is_admin="1" ORDER BY usr_id;';
            end;
            if cmbbxTo.Text = '[ группа операторов ]' then
            begin
              LogThis('Получение списка идентификаторов операторов...', LogGroupGUID, lmtInfo);
              q := 'SELECT DISTINCT usr_id FROM ' + Configuration.sMySQLDatabase +
                '._usr WHERE usr_erased="0" and usr_can_edit_db="1" ORDER BY usr_id;';
            end;
            if cmbbxTo.Text = '[ группа пользователей ]' then
            begin
              LogThis('Получение списка идентификаторов пользователей...', LogGroupGUID, lmtInfo);
              q := 'SELECT DISTINCT usr_id FROM ' + Configuration.sMySQLDatabase +
                '._usr WHERE usr_erased="0" and usr_is_admin="0" ORDER BY usr_id;';
            end;
          end
          else
          begin
            LogThis('Получение идентификатора указанного пользователя...', LogGroupGUID, lmtInfo);
            q := 'SELECT usr_id FROM ' + Configuration.sMySQLDatabase + '._usr WHERE usr_fullname="'
              + cmbbxTo.Text + '" ORDER BY usr_id;';
          end;
          LogThis(q, LogGroupGUID, lmtSQL);
          i := mysql_real_query(MySQLConnectionHandler, PAnsiChar(AnsiString(q)), Length(q));
          if i = 0 then
          begin
            ...
            begin
              ...
              if i > 0 then
              begin
                LogThis('Количество строк выборки равно ' + IntToStr(i) + '.',
                  LogGroupGUID, lmtInfo);
                ProgressBar1.Max := i;
                for j := 0 to i - 1 do
                begin
                ...
                GenerateError
                  ('В системе не зарегестрироывано ни одного пользователя с такими правами!',
                  s, bError);
            end
            else
              GenerateError('Не удалось получить результирующую выборку по последнему SQL-запросу!',
          ...
          begin
            ...
            try
              ...
              mysql_escape_string(pacCaption, PAnsiChar(AnsiString(edbxCaption.Text)),
                Length(edbxCaption.Text));
              ...
              begin
                ...
                begin
                  ...
                  q := 'INSERT INTO ' + Configuration.sMySQLDatabase +
                    '._msg (msg_id, msg_erased, msg_processed, msg_for_user_id, ' +
                    'msg_caption, msg_text, msg_created_at, msg_created_by_user_id) VALUES ' +
                    '(NULL, "0", "0", ' + TSL.Strings[j] + ', "' + string(pacCaption) + '", "' +
                    string(pacText) + '", Now(), ' + CurrentUser.sID + ');';
                  LogThis(q, LogGroupGUID, lmtSQL);
                  i := mysql_real_query(MySQLConnectionHandler, PAnsiChar(AnsiString(q)),
                    Length(q));
                  if i = 0 then
                  begin
                    LogThis('Запрос выполнен успешно.', LogGroupGUID, lmtInfo);
                    iLastID := mysql_insert_id(MySQLConnectionHandler);
                  ...
                  begin
                    LogThis('Внесение записи в таблицу событий.', LogGroupGUID, lmtInfo);
                    q := 'INSERT INTO ' + Configuration.sMySQLDatabase +
                      '._evn VALUES (NULL, NOW(),"INS","msg",' + IntToStr(iLastID) + ',' +
                      CurrentUser.sID + ',' + #39 + q + #39 + ');';
                    LogThis(q, LogGroupGUID, lmtSQL);
                    i := mysql_real_query(MySQLConnectionHandler, PAnsiChar(AnsiString(q)),
                      Length(q));
                    if i = 0 then
...
begin
  ProcedureHeader
    ('Процедура реакции на нажатие кнопки вставки данных о мероприятии из временного буфера',
    LogGroupGUID, True, True);
      ...
      try
        ...
        begin
          ...
          begin
            ...
            aListItem.SubItems.Add(raMultibuffer[i].MsrType + '|' + raMultibuffer[i].MsrName + '|' +
              raMultibuffer[i].MsrAuthor + '|' + raMultibuffer[i].MsrProducer + '|' +
              raMultibuffer[i].MsrPerformer + '|' + raMultibuffer[i].MsrOrganizer + '|' +
              raMultibuffer[i].MsrTicketPrice + '|' + string(raMultibuffer[i].MsrOtherInfo));
          end;
        end;
        PreShowModal(sModalWinName, LogGroupGUID);
        ShowModal;
  ...
  begin
    if (PageControl1.ActivePage = tsMsr) and MsrBufferRec.bStoredDataExists and CurrentUser.bDBEdition
    then
    begin
...
const
  ...
  sModalWinName: string = '"О программе..."';
begin
  ProcedureHeader('Процедура отображения окна ' + sModalWinName, LogGroupGUID, not aIsPopupState,
    not aIsPopupState);
...
begin
  ...
  with ViewPostListForm do
    try
      ...
      q := 'SELECT msg_created_at, usr_fullname, msg_caption, msg_id, msg_processed FROM ' +
        Configuration.sMySQLDatabase + '._msg LEFT JOIN ' + Configuration.sMySQLDatabase +
        '._usr ON usr_id=msg_created_by_user_id  WHERE msg_for_user_id="' + CurrentUser.sID +
      ...
      pnlID.Visible := Configuration.bShowID and (lvMessages.Items.Count > 0) and
...
var
  ...
  ResultRow: PMYSQL_ROW;
begin
  ProcedureHeader('Процедура обновления вида индикатора наличия новых сообщений', LogGroupGUID,
    True, False);

  if Configuration.bLogged then
  begin
    bError := False;
    q := 'SELECT COUNT(*) FROM ' + Configuration.sMySQLDatabase + '._msg ' + 'LEFT JOIN ' +
      Configuration.sMySQLDatabase + '._usr ON usr_id=msg_created_by_user_id ' +
    ...
    LogThis('Действие отменено, так как подключение к базе данных не было выполнено!', LogGroupGUID,
      lmtWarning);
    ...
    with StatisticForm do
      try
        ...
        begin
          ...
          begin
            ...
            begin
              ...
              q := 'SELECT DISTINCT LEFT(evn_datetime,4) FROM ' + Configuration.sMySQLDatabase +
                '._evn ORDER BY LEFT(evn_datetime,4);';
              LogThis(q, LogGroupGUID, lmtSQL);
              i := mysql_real_query(MySQLConnectionHandler, PAnsiChar(AnsiString(q)), Length(q));
              if i = 0 then
              begin
                ...
                begin
                  ...
                  begin
                    ...
                    GenerateError
                      ('Формирование отчётов невозможно, так как в таблице статистических данных не было найдено ни одной записи!',
                      s, bError);
                end
                else
                  GenerateError
        ...
        begin
          ...
          // список начинается с нуля, поэтому отнимаем единицу
          if cmbbxSelectedMonthYear.Items.Count > 0 then
            cmbbxSelectedMonthYear.ItemIndex := cmbbxSelectedMonthYear.Items.IndexOf
              (IntToStr(Year));
        ...
        begin
          ...
          begin
            sStartDate :=
              DateToStr(EncodeDate(StrToInt(cmbbxSelectedMonthYear.Items
              [cmbbxSelectedMonthYear.ItemIndex]), cmbbxSelectedMonthMonth.ItemIndex + 1, 1));
            // определение следующего месяца
            iNextMonth := cmbbxSelectedMonthMonth.ItemIndex + 2;
            iNextMonthYear :=
              StrToInt(cmbbxSelectedMonthYear.Items[cmbbxSelectedMonthYear.ItemIndex]);
            if iNextMonth = 13 then
            begin
              iNextMonth := 1;
              iNextMonthYear := iNextMonthYear + 1;
            end;
            sStopDate := DateToStr(EncodeDate(iNextMonthYear, iNextMonth, 1) -
              (EncodeDate(2004, 1, 2) - EncodeDate(2004, 1, 1)));
          end;
          if RadioButton3.Checked then
          begin
            sStartDate :=
              DateToStr(EncodeDate(StrToInt(cmbbxSelectedYear.Items
              [cmbbxSelectedYear.ItemIndex]), 1, 1));
            sStopDate :=
              DateToStr(EncodeDate(StrToInt(cmbbxSelectedYear.Items[cmbbxSelectedYear.ItemIndex]
              ), 12, 31));
          end;
          ...
          html := html +
            '    <TITLE>Статистика по работе пользователей АРМ услуги "Отдых и развлечения" за период с '
            + sStartDate + ' по ' + sStopDate + '</TITLE>' + sLineBreak;
          ...
          html := html +
            '      * {font-family: Verdana, Arial, Helvetica, sans-serif; font-size: 8pt; padding: 0; margin: 0; border-color: transparent; border-width: 0;}'
            + sLineBreak;
          ...
          begin
            ...
            html := html +
              ...
              + sLineBreak;
            html := html +
              '          <TD HEIGHT="1px" STYLE="border-color: white; border-width: 0;" WIDTH="120px"></TD>'
              + sLineBreak;
            html := html +
              '          <TD HEIGHT="1px" STYLE="border-color: white; border-width: 0;" WIDTH="386px"></TD>'
              + sLineBreak;
            html := html + '        </TR>' + sLineBreak;
            html := html +
              '        <TR><TD COLSPAN="4" STYLE="border-bottom-color: #CCCCCC; border-top-color: white; border-left-color: white; border-right-color: white; border-width: 0px 0px 1px 0px; font-size: 7pt; text-align: right;">';
            html := html +
              'Справочно-информационный цех<BR>филиала &laquo;Минская городская телефонная сеть&raquo;<BR>РУП &laquo;Белтелеком&raquo;</TD></TR>'
              + sLineBreak;
            html := html +
              '        <TR><TD COLSPAN="4" STYLE="border-color: white; border-width: 0; text-align: center; font-size: 13pt; font-weight: bold; padding: 20px 0px 3px 0px;">';
            html := html +
              'Статистика&nbsp;по&nbsp;работе&nbsp;пользователей АРМ&nbsp;&laquo;Отдых&nbsp;и&nbsp;развлечения&raquo; за период с '
              + sStartDate + ' по ' + sStopDate + '<BR></TD></TR>' + sLineBreak;
            ...
            begin
              ...
              begin
                ...
                q := 'SELECT LEFT(evn_datetime,10), RIGHT(evn_datetime,8), evn_type, evn_object, evn_object_id, evn_details FROM '
                  + Configuration.sMySQLDatabase + '._evn LEFT JOIN ' + Configuration.sMySQLDatabase
                  + '._usr ON usr_id=evn_by_user_id WHERE usr_fullname="' + s +
                  '" AND LEFT(evn_datetime,10) BETWEEN "' + Copy(sStartDate, 7, 4) + '-' +
                  Copy(sStartDate, 4, 2) + '-' + Copy(sStartDate, 0, 2) + '" AND "' +
                  Copy(sStopDate, 7, 4) + '-' + Copy(sStopDate, 4, 2) + '-' + Copy(sStopDate, 0, 2)
                  + '" ORDER BY LEFT(evn_datetime,10), RIGHT(evn_datetime,8);';
                LogThis(q, LogGroupGUID, lmtSQL);
                i := mysql_real_query(MySQLConnectionHandler, PAnsiChar(AnsiString(q)), Length(q));
                if i = 0 then
                begin
                  ...
                  begin
                    ...
                    begin
                      ...
                      html := html + '        <TR>' + sLineBreak;
                      html := html +
                        '          <TD VALIGN="TOP" COLSPAN="4" STYLE="text-align: left; font-size: 12pt; font-weight: bold; border-color: white; border-width: 0; padding: 20px 2px 2px 0px;">'
                        + s + '</TD>' + sLineBreak;
                      html := html + '        </TR>' + sLineBreak;
                      if i > 0 then
                      begin
                        html := html + '        <TR>' + sLineBreak;
                        html := html +
                          '          <TD STYLE="text-align: center; border-width: 2px 2px 2px 2px; border-color: black; font-weight: bold; font-size: 8pt; padding: 2px;">Дата</TD>'
                          + sLineBreak;
                        html := html +
                          '          <TD STYLE="text-align: center; border-width: 2px 2px 2px 0px; border-color: black; font-weight: bold; font-size: 8pt; padding: 2px;">Время</TD>'
                          + sLineBreak;
                        html := html + '          <TD ';
                        if not chkbxShowSQLQuery.Checked then
                          html := html + 'COLSPAN="2" ';
                        html := html +
                          'STYLE="text-align: center; border-width: 2px 2px 2px 0px; border-color: black; font-weight: bold; font-size: 8pt; padding: 2px;">Действие</TD>'
                          + sLineBreak;
                        if chkbxShowSQLQuery.Checked then
                          html := html +
                            '          <TD STYLE="text-align: center; border-width: 2px 2px 2px 0px; border-color: black; font-weight: bold; font-size: 8pt; padding: 2px;">Текст&nbsp;SQL-запроса</TD>'
                            + sLineBreak;
                        html := html + '        </TR>' + sLineBreak;
                        for j := 0 to i - 1 do
                        begin
                          ...
                          begin
                            ...
                            html := html + '        <TR>' + sLineBreak;
                            html := html +
                              '          <TD STYLE="text-align: center; border-width: 0px 2px ' +
                              IntToStr(1 + Integer(((i - 1) - j) = 0)) +
                              'px 2px; border-color: black; font-size: 8pt; padding: 2px;">' +
                              string(Copy(ResultRow[0], 9, 2)) + '.' +
                              string(Copy(ResultRow[0], 6, 2)) + '.' +
                              string(Copy(ResultRow[0], 0, 4)) + '</TD>' + sLineBreak;
                            html := html +
                              '          <TD STYLE="text-align: center; border-width: 0px 2px ' +
                              IntToStr(1 + Integer(((i - 1) - j) = 0)) +
                              'px 0px; border-color: black; font-size: 8pt; padding: 2px;">' +
                              string(ResultRow[1]) + '</TD>' + sLineBreak;
                            ...
                            q := q + GetConditionalMessage(string(ResultRow[3]) > '', ' ', '') +
                              ...
                              '') + GetConditionalMessage(string(ResultRow[3]) = 'org',
                              'организации', '') + GetConditionalMessage(string(ResultRow[3])
                              = 'msr', 'мероприятия', '') +
                              GetConditionalMessage(StrToIntDef(string(ResultRow[4]), -1) > -1,
                              ' ID #' + string(ResultRow[4]), ''));
                            html := html + '          <TD ';
                            if not chkbxShowSQLQuery.Checked then
                              html := html + 'COLSPAN="2" ';
                            html := html + 'STYLE="text-align: center; border-width: 0px 2px ' +
                              IntToStr(1 + Integer(((i - 1) - j) = 0)) +
                              'px 0px; border-color: black; font-size: 8pt; padding: 2px;">' + q +
                              '</TD>' + sLineBreak;
                            if chkbxShowSQLQuery.Checked then
                              html := html +
                                '          <TD STYLE="text-align: left; border-width: 0px 2px ' +
                                IntToStr(1 + Integer(((i - 1) - j) = 0)) +
                                'px 0px; border-color: black; font-size: 8pt; padding: 2px 2px 2px 5px;">'
                                + string(ResultRow[5]) + '</TD>' + sLineBreak;
                            html := html + '        </TR>' + sLineBreak;
                          end
                          else
                            GenerateError
                      ...
                      begin
                        html := html + '        <TR>' + sLineBreak;
                        html := html +
                          '          <TD COLSPAN="4" STYLE="text-align: left; border-width: 0px; border-color: white; font-size: 8pt; padding: 2px 2px 2px 0px;">Данные по работе указанного пользователя отсутствуют.</TD>'
                          + sLineBreak;
                        html := html + '        </TR>' + sLineBreak;
                      end;
                    end
                    ...
                    begin
                      LogThis('Данных по работе пользователя "' + s + '" в базе данных не найдено.',
                        LogGroupGUID, lmtInfo);
                    end;
                  end
                  else
            ...
            html := html + '        <TR>' + sLineBreak;
            html := html +
              '          <TD COLSPAN="4" STYLE="text-align: center; font-size: 5pt; border-bottom-color: white; border-top-color: #CCCCCC; border-left-color: white; border-right-color: white; border-width: 1px 0px 0px 0px; padding-top: 1px;">Отчёт подготовлен '
              + DateToStr(EncodeDate(Year, Month, Day)) + ' в ' +
              TimeToStr(EncodeTime(Hour, Min, Sec, MSec)) +
              ' при помощи АРМ услуги &laquo;Отдых и развлечения&raquo;, &copy;&nbsp;2008&nbsp;by&nbsp;Vlad&nbsp;Ivanov</TD>'
              + sLineBreak;
            html := html + '        </TR>' + sLineBreak;
          end
          else
          begin
            ...
            html := html +
              ...
              + sLineBreak;
            html := html +
              '          <TD HEIGHT="1px" STYLE="border-color: white; border-width: 0;" WIDTH="144px"></TD>'
              + sLineBreak;
            html := html +
              '          <TD HEIGHT="1px" STYLE="border-color: white; border-width: 0;" WIDTH="144px"></TD>'
              + sLineBreak;
            ...
            html := html + '        </TR>' + sLineBreak;
            html := html +
              '        <TR><TD COLSPAN="5" STYLE="border-bottom-color: #CCCCCC; border-top-color: white; border-left-color: white; border-right-color: white; border-width: 0px 0px 1px 0px; font-size: 7pt; text-align: right;">';
            html := html +
              'Справочно-информационный цех<BR>филиала &laquo;Минская городская телефонная сеть&raquo;<BR>РУП &laquo;Белтелеком&raquo;</TD></TR>'
              + sLineBreak;
            html := html +
              '        <TR><TD COLSPAN="5" STYLE="border-color: white; border-width: 0; text-align: center; font-size: 13pt; font-weight: bold; padding: 20px 0px 3px 0px;">';
            html := html +
              'Статистика&nbsp;по&nbsp;работе&nbsp;пользователей АРМ&nbsp;&laquo;Отдых&nbsp;и&nbsp;развлечения&raquo; за период с '
              + sStartDate + ' по ' + sStopDate + '<BR></TD></TR>' + sLineBreak;
            // перебираем список отмеченых пользователей
            ...
            begin
              ...
              begin
                ...
                q := 'SELECT DISTINCT LEFT(evn_datetime,10) FROM ' + Configuration.sMySQLDatabase +
                  '._evn LEFT JOIN ' + Configuration.sMySQLDatabase +
                  '._usr ON usr_id=evn_by_user_id WHERE usr_fullname="' + s +
                  '" AND LEFT(evn_datetime,10) BETWEEN "' + Copy(sStartDate, 7, 4) + '-' +
                  Copy(sStartDate, 4, 2) + '-' + Copy(sStartDate, 0, 2) + '" AND "' +
                  Copy(sStopDate, 7, 4) + '-' + Copy(sStopDate, 4, 2) + '-' + Copy(sStopDate, 0, 2)
                  + '" ORDER BY LEFT(evn_datetime,10);';
                LogThis(q, LogGroupGUID, lmtSQL);
                i := mysql_real_query(MySQLConnectionHandler, PAnsiChar(AnsiString(q)), Length(q));
                if i = 0 then
                begin
                ...
                begin
                  html := html + '        <TR>' + sLineBreak;
                  html := html +
                    '          <TD VALIGN="TOP" COLSPAN="5" STYLE="text-align: left; font-size: 12pt; font-weight: bold; border-color: white; border-width: 0; padding: 20px 2px 2px 0px;">'
                    + s + '</TD>' + sLineBreak;
                  html := html + '        </TR>' + sLineBreak;
                  if i > 0 then
                  begin
                    ...
                    html := html +
                      ...
                      + sLineBreak;
                    html := html +
                      '          <TD STYLE="text-align: center; border-width: 2px 2px 2px 0px; border-color: black; font-weight: bold; font-size: 8pt; padding: 2px;">Создано</TD>'
                      + sLineBreak;
                    html := html +
                      '          <TD STYLE="text-align: center; border-width: 2px 2px 2px 0px; border-color: black; font-weight: bold; font-size: 8pt; padding: 2px;">Исправлено</TD>'
                      + sLineBreak;
                    html := html +
                      '          <TD STYLE="text-align: center; border-width: 2px 2px 2px 0px; border-color: black; font-weight: bold; font-size: 8pt; padding: 2px;">Удалено</TD>'
                      + sLineBreak;
                    html := html +
                      '          <TD STYLE="text-align: center; border-width: 2px 2px 2px 0px; border-color: black; font-weight: bold; font-size: 8pt; padding: 2px;">Восстановлено</TD>'
                      + sLineBreak;
                    html := html + '        </TR>' + sLineBreak;
                    ...
                    begin
                      ...
                      sIns := '0';
                      s := chklbxUsers.Items[k];
                      q := 'SELECT evn_datetime, evn_type, evn_object, evn_object_id FROM ' +
                        Configuration.sMySQLDatabase + '._evn ' + 'LEFT JOIN ' +
                      ...
                      begin
                        ...
                        begin
                          ...
                          begin
                            LogThis('Данных по работе пользователя "' + s +
                              '" в базе данных не найдено.', LogGroupGUID, lmtInfo);
                          end;
                        end
                        else
                          GenerateError
                      ...
                      html := html +
                        '          <TD STYLE="text-align: center; border-width: 0px 2px ' +
                        IntToStr(1 + Integer(((slTemp.Count - 1) - j) = 0)) +
                        'px 2px; border-color: black; font-size: 8pt; padding: 2px;">' +
                        Copy(slTemp[j], 9, 2) + '.' + Copy(slTemp[j], 6, 2) + '.' +
                        Copy(slTemp[j], 0, 4) + '</TD>' + sLineBreak;
                      html := html +
                        '          <TD STYLE="text-align: center; border-width: 0px 2px ' +
                        IntToStr(1 + Integer(((slTemp.Count - 1) - j) = 0)) +
                        'px 0px; border-color: black; font-size: 8pt; padding: 2px;">' + sIns +
                        '</TD>' + sLineBreak;
                      html := html +
                        '          <TD STYLE="text-align: center; border-width: 0px 2px ' +
                        IntToStr(1 + Integer(((slTemp.Count - 1) - j) = 0)) +
                        'px 0px; border-color: black; font-size: 8pt; padding: 2px;">' + sMod +
                        '</TD>' + sLineBreak;
                      html := html +
                        '          <TD STYLE="text-align: center; border-width: 0px 2px ' +
                        IntToStr(1 + Integer(((slTemp.Count - 1) - j) = 0)) +
                        'px 0px; border-color: black; font-size: 8pt; padding: 2px;">' + sDel +
                        '</TD>' + sLineBreak;
                      html := html +
                        '          <TD STYLE="text-align: center; border-width: 0px 2px ' +
                        IntToStr(1 + Integer(((slTemp.Count - 1) - j) = 0)) +
                        'px 0px; border-color: black; font-size: 8pt; padding: 2px;">' + sRec +
                        '</TD>' + sLineBreak;
                      html := html + '        </TR>' + sLineBreak;
                      ProgressBar1.StepIt;
                    end;
                    ...
                    html := html + '        <TR>' + sLineBreak;
                    html := html +
                      '          <TD STYLE="text-align: center; border-width: 0px 2px 2px 2px; border-color: black; font-size: 8pt; padding: 2px;">Итого:</TD>'
                      + sLineBreak;
                    html := html +
                      '          <TD STYLE="text-align: center; border-width: 0px 2px 2px 0px; border-color: black; font-size: 8pt; padding: 2px;">'
                      + IntToStr(iSumIns) + '</TD>' + sLineBreak;
                    html := html +
                      '          <TD STYLE="text-align: center; border-width: 0px 2px 2px 0px; border-color: black; font-size: 8pt; padding: 2px;">'
                      + IntToStr(iSumMod) + '</TD>' + sLineBreak;
                    html := html +
                      '          <TD STYLE="text-align: center; border-width: 0px 2px 2px 0px; border-color: black; font-size: 8pt; padding: 2px;">'
                      + IntToStr(iSumDel) + '</TD>' + sLineBreak;
                    html := html +
                      '          <TD STYLE="text-align: center; border-width: 0px 2px 2px 0px; border-color: black; font-size: 8pt; padding: 2px;">'
                      + IntToStr(iSumRec) + '</TD>' + sLineBreak;
                    html := html + '        </TR>' + sLineBreak;
                    ProgressBar1.Position := ProgressBar1.Min;
                  end
                  else
                  begin
                    html := html + '        <TR>' + sLineBreak;
                    html := html +
                      '          <TD COLSPAN="5" STYLE="text-align: left; border-width: 0px; border-color: white; font-size: 8pt; padding: 2px 2px 2px 0px;">Данные по работе указанного пользователя отсутствуют.</TD>'
                      + sLineBreak;
                    html := html + '        </TR>' + sLineBreak;
                  end;
                end;
            ...
            html := html + '        <TR>' + sLineBreak;
            html := html +
              '          <TD COLSPAN="5" HEIGHT="20" STYLE="text-align: left; color: #EEEEEE; font-size: 5pt; border-color: white; border-width: 1px 0px 0px 0px; padding-top: 1px;"></TD>'
              + sLineBreak;
            html := html + '        </TR>' + sLineBreak;
            html := html + '        <TR>' + sLineBreak;
            html := html +
              '          <TD COLSPAN="5" STYLE="text-align: center; font-size: 5pt; border-bottom-color: white; border-top-color: #CCCCCC; border-left-color: white; border-right-color: white; border-width: 1px 0px 0px 0px; padding-top: 1px;">Отчёт подготовлен '
              + DateToStr(EncodeDate(Year, Month, Day)) + ' в ' +
              TimeToStr(EncodeTime(Hour, Min, Sec, MSec)) +
              ' при помощи АРМ услуги &laquo;Отдых и развлечения&raquo;, &copy;&nbsp;2008&nbsp;by&nbsp;Vlad&nbsp;Ivanov</TD>'
              + sLineBreak;
          ...
          with TStringList.Create do
            try
              ...
              begin
                ...
                DeleteFile(q);
                q := ExtractFilePath(q) + Copy(ExtractFileName(q), 0,
                  Length(ExtractFileName(q)) - 3) + 'htm';
                LogThis('Имя временного файла - ' + #39 + q + #39 + '.', LogGroupGUID, lmtInfo);
                if not FileExists(q) then
                begin
                  LogThis('Попытка сохраниея отчёта во временный файл ' + #39 + q + #39 + '.',
                    LogGroupGUID, lmtInfo);
                  SaveToFile(q);
                  ...
                  GenerateError('Возникла ошибка при сохранении отчёта во временный файл ' + #39 + q
                    + #39 + ' - указанный файл уже существует!', s, bError);
              end
              else
  ...
  try
    ...
    begin
      ...
      begin
        ...
        LogThis('Путь временной папки успешно получен - <' + TempPathNameBuffer + '>.',
          LogGroupGUID, lmtInfo);
      end
      else
        GenerateError('Возникла ошибка при попытке получения пути временной папки!',
...
begin
  ...
  begin
    if (PageControl1.ActivePage = tsOrg) and pnlConnectedMode.Visible and
      (stPhonesMark.Visible and (stPhonesMark.Font.Color = clRed) and
      (stPhonesMark.Caption = 'ЕСТЬ ТЕЛЕФОНЫ')) then
      Show_OrgPhonesForm;
  end;
...
begin
  ...
  begin
    if (PageControl1.ActivePage = tsMsr) and pnlConnectedMode.Visible and
      ((stPhonesMark.Visible and (stPhonesMark.Font.Color = clRed) and
      (stPhonesMark.Caption = 'ЕСТЬ ТЕЛЕФОНЫ')) or CurrentUser.bDBEdition) then
      Show_MsrPhonesForm;
  end;
  if Key = VK_F12 then
    if (ssShift in Shift) and (ssCtrl in Shift) then
...
begin
  ...
  Screen.Cursor := crDefault;
  MessageBox(Handle, PChar(aErrorMessage), PChar('OA4 - Ошибка!'),
...
begin
  LogThis('Производится попытка отображения модального окна ' + aWindowName + '.',
    aLogGroupGUID, lmtInfo);
  Screen.Cursor := crDefault;
end;
...
begin
  ...
  LogThis('Панель статуса ' + GetConditionalMessage(StatusBar1.Visible, 'включена', 'отключена') +
    '.', LogGroupGUID, lmtInfo);
...
begin
  ProcedureHeader('Процедура включения/отключения отображения панели кнопок', LogGroupGUID,
    True, True);

  ActionToolBar1.Visible := miToolbar.Checked;
  Configuration.bNoToolBar := not ActionToolBar1.Visible;
  LogThis('Панель кнопок ' + GetConditionalMessage(ActionToolBar1.Visible, 'включена', 'отключена')
    + '.', LogGroupGUID, lmtInfo);
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        // сохранение значения ID выделенной организации
        if iOrgID >= 0 then
          LogThis('Производится добавление нового мероприятия для организации с ID=' +
            IntToStr(iOrgID) + '.', LogGroupGUID, lmtInfo)
        else
          GenerateError
            ('Возникла ошибка при получении ID организации-владельца создаваемого мероприятия!',
            s, bError);
        ProgressBar1.Max := 5;
      end
      else
    ...
    begin
      ...
      begin
        ...
        // сохранение значения ID выделенной организации
        if iOrgID >= 0 then
          LogThis('Производится редактирование мероприятия принадлежащего организации с ID=' +
            IntToStr(iOrgID) + '.', LogGroupGUID, lmtInfo)
        else
          GenerateError
            ('Возникла ошибка при получении ID организации-владельца редактируемого мероприятия!',
            s, bError);
        ProgressBar1.Max := 5;
      end
      else
      ...
      begin
        iMsrID := StrToIntDef(lvMsr.Selected.SubItems[2], -1);
        // сохранение значения ID выделенного мероприятия
        if iMsrID >= 0 then
          LogThis('Производится внесение изменений в данные мероприятия с ID=' + IntToStr(iMsrID) +
            '.', LogGroupGUID, lmtInfo)
        else
    ...
    begin
      ...
      begin
        q := 'INSERT INTO ' + Configuration.sMySQLDatabase +
          '._msr (msr_id, msr_erased, msr_start_datetime, msr_stop_datetime, msr_unknown_duration, _msr_duration, msr_organization_id, '
          + 'msr_is_premier, msr_is_tour, msr_for_children, msr_for_teenagers, msr_for_adults_only, '
          + 'msr_type, msr_name, msr_author, msr_producer, msr_performer, msr_organizer, msr_ticket_price, msr_other_info, msr_other_info_rtf, '
          + 'msr_created_at, msr_created_by_user_id, msr_modified_at, msr_modified_by_user_id) VALUES ('
          + 'NULL, "0", ' + // msr_id, msr_erased
          GetConditionalMessage(chkbxMsrStart.Checked, '"' + FormatDateTime('yyyy-mm-dd hh:nn:ss',
          dtpMsrStartDate.DateTime) + '", ', 'NULL, ') + // msr_start_datetime
          GetConditionalMessage(chkbxMsrStop.Checked, '"' + FormatDateTime('yyyy-mm-dd hh:nn:ss',
          dtpMsrStopDate.DateTime) + '", ', 'NULL, ') + // msr_stop_datetime
        ...
        begin
          ...
          q := q + '"' + string(System.AnsiStrings.StrPas(pac)) + '", ';
          if pac <> nil then
            FreeMemory(pac);
      ...
      begin
        q := 'UPDATE ' + Configuration.sMySQLDatabase + '._msr SET ' +
          GetConditionalMessage(chkbxMsrStart.Checked, 'msr_start_datetime="' +
          FormatDateTime('yyyy-mm-dd hh:nn:ss', dtpMsrStartDate.DateTime) + '", ',
          'msr_start_datetime=NULL, ') + GetConditionalMessage(chkbxMsrStop.Checked,
          'msr_stop_datetime="' + FormatDateTime('yyyy-mm-dd hh:nn:ss', dtpMsrStopDate.DateTime) +
          '", ', 'msr_stop_datetime=NULL, ') + 'msr_unknown_duration="' +
          IntToStr(Integer(chkbxMsrUnknownDuration.Checked)) + '", ' + // msr_unknown_duration
          'msr_is_premier="' + IntToStr(Integer(chkbxMsrIsPremier.Checked)) + '", ' +
          'msr_is_tour="' + IntToStr(Integer(chkbxMsrIsTour.Checked)) + '", ' + 'msr_for_children="'
          + IntToStr(Integer(chkbxMsrForChildren.Checked)) + '", ' + 'msr_for_teenagers="' +
          IntToStr(Integer(chkbxMsrForTeenagers.Checked)) + '", ' + 'msr_for_adults_only="' +
          IntToStr(Integer(chkbxMsrForAdultsOnly.Checked)) + '", ' + 'msr_type=' +
          NormalizeStringForQuery(cmbbxMsrType.Text, True, True) + ', ' + 'msr_name=' +
          NormalizeStringForQuery(edbxMsrName.Text, True, True) + ', ' + 'msr_author=' +
          NormalizeStringForQuery(cmbbxMsrAuthor.Text, True, True) + ', ' + 'msr_producer=' +
          NormalizeStringForQuery(cmbbxMsrProducer.Text, True, True) + ', ' + 'msr_performer=' +
          NormalizeStringForQuery(cmbbxMsrPerformer.Text, True, True) + ', ' + 'msr_organizer=' +
          NormalizeStringForQuery(cmbbxMsrOrganizer.Text, True, True) + ', ' + 'msr_ticket_price=' +
          NormalizeStringForQuery(meMsrTicketPrice.Text, True, True) + ', ';
        ...
        begin
          pac := GetMemory(Length(PAnsiChar(sPlane)) * 2 + 1);
          mysql_escape_string(pac, PAnsiChar(sPlane), Length(PAnsiChar(sPlane)));
          q := q + 'msr_other_info="' + string(System.AnsiStrings.StrPas(pac)) + '", ';
          if pac <> nil then
            FreeMemory(pac);
          // преобразование текста RTF в строку с ESC-последовательностями
          RTFStream := TMemoryStream.Create;
          try
            ...
            pac := GetMemory(RTFStream.Size * 2 + 1);
            mysql_escape_string(pac, RTFStream.Memory, RTFStream.Size);
            q := q + 'msr_other_info_rtf="' + string(System.AnsiStrings.StrPas(pac)) + '", ';
          finally
            if pac <> nil then
              FreeMemory(pac);
            RTFStream.Clear;
        ...
        q := q + 'msr_modified_at=NOW(), msr_modified_by_user_id=' + CurrentUser.sID +
          ' WHERE msr_id=' + IntToStr(iMsrID) + ';';
      end;
    end;
    ProgressBar1.StepIt;
    Application.ProcessMessages;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          LogThis('Добавление данных о новом мероприятии произведено успешно.',
            LogGroupGUID, lmtInfo);
        end
        else
        ...
        if bAddingMsrNow then
          q := 'INSERT INTO ' + Configuration.sMySQLDatabase +
            '._evn VALUES (NULL, NOW(),"INS","msr",' + IntToStr(iMsrID) + ',' + CurrentUser.sID +
            ',' + #39 + q + #39 + ');'
        else
          q := 'INSERT INTO ' + Configuration.sMySQLDatabase +
            '._evn VALUES (NULL, NOW(),"MOD","msr",' + IntToStr(iMsrID) + ',' + CurrentUser.sID +
            ',' + #39 + q + #39 + ');';
        LogThis(q, LogGroupGUID, lmtSQL);
        i := mysql_real_query(MySQLConnectionHandler, PAnsiChar(AnsiString(q)), Length(q));
        if i = 0 then
    ...
    begin
      if iMsrID < 0 then // если произошла ошибка при получении индекса записи, сгенерировать ошибку
        GenerateError('Получен некорректный идентификатор номера записи в таблице мероприятий!',
          s, bError);
    end;
    ProgressBar1.StepIt;
    Application.ProcessMessages;
    ...
    btbtnSearchClick(nil);
    // когда допишу формирование запроса на основе усправляющих элементов, тогда нужно заменить код полной копией
      ...
      begin
        ...
        try
          ...
          begin
            ...
            begin
              DT := StrToDateTimeDef(Trim(lvMsrDateTimeList.Items[i].Caption + ' ' +
                lvMsrDateTimeList.Items[i].SubItems[0]), EncodeDate(1900, 1, 1));
              if DT <> EncodeDate(1900, 1, 1) then
                sl.Append(FormatDateTime('dd.mm.yyyy hh:nn:ss', DT));
            end;
            LogThis('Количество добавленых пользователем "сеансов" равно [' + IntToStr(sl.Count) +
              '].', LogGroupGUID, lmtInfo);
            LogThis('Количество обработаных элементов списка "сеансов" равно [' + IntToStr(sl.Count)
              + '].', LogGroupGUID, lmtInfo);
          ...
          begin
            ...
            begin
              with MsrBuffer do
              begin
                MsrOrganizationID := StrToIntDef(lvOrg.Selected.SubItems[2], -1);
                // сохранение значения ID выделенной организации
                if MsrOrganizationID >= 0 then
                  LogThis('Производится массовое дублирование мероприятий для организации с ID=' +
                    IntToStr(MsrOrganizationID) + '.', LogGroupGUID, lmtInfo)
                else
                  GenerateError
                    ('Возникла ошибка при получении ID организации-владельца дублируемого мероприятия!',
                    s, bError);
              end;
            end
            else
          ...
          begin
            ...
            begin
              ...
              // сохранение значения ID выделенного мероприятия
              if iMsrID >= 0 then
                LogThis('Производится массовое дублирование мероприятий на основе данных мероприятия с ID='
                  + IntToStr(iMsrID) + '.', LogGroupGUID, lmtInfo)
              else
          ...
          begin
            ...
            begin
              ...
              MsrDurationHours := StrToIntDef(spedMsrDurationHours.Text,
                spedMsrDurationHours.Value);
              MsrDurationMinutes := StrToIntDef(spedMsrDurationMinutes.Text,
                spedMsrDurationMinutes.Value);
              ...
              if bStoredDataExists then
                ...
                GenerateError
                  ('Произошла ошибка при попытке получения данных о мероприятии-образце!',
                  s, bError);
            end;
          end;
                ...
                begin
                  ...
                  begin
                    ...
                    if MsrStartDateTime = EncodeDate(1900, 1, 1) then
                      GenerateError
                        ('Произошла ошибка при попытке конвертации начальной даты мероприятия-образца (исходная строка - <'
                        + sl[j] + '>)!', s, bError);
                  end;
                  ...
                  begin
                    ...
                    a1 := TimeStampToMSecs(t1) + (MsrDurationMinutes * 60000) +
                      (MsrDurationHours * 3600000);
                    t2 := MSecsToTimeStamp(a1);
                  ...
                  begin
                    q := 'INSERT INTO ' + Configuration.sMySQLDatabase +
                      ...
                      + 'msr_created_at, msr_created_by_user_id, msr_modified_at, msr_modified_by_user_id) VALUES ('
                      + 'NULL, "0", ' + // msr_id, msr_erased
                      GetConditionalMessage(MsrHasStartDateTime,
                      '"' + FormatDateTime('yyyy-mm-dd hh:nn:ss', MsrStartDateTime) + '", ',
                      'NULL, ') + GetConditionalMessage(MsrHasStopDateTime,
                      '"' + FormatDateTime('yyyy-mm-dd hh:nn:ss', MsrStopDateTime) + '", ',
                      'NULL, ') + '"' + IntToStr(Integer(MsrUnknownDuration)) + '", ' +
                    // msr_unknown_duration
                      'NULL, ' + IntToStr(MsrOrganizationID) + ', ' +
                    // _msr_duration, msr_organization_id
                      ...
                      NormalizeStringForQuery(MsrTicketPrice, True, True) + ', ' +
                    // msr_ticket_price
                      GetConditionalMessage(Trim(string(MsrOtherInfoPlane)) = '', 'NULL, NULL, ',
                      NormalizeStringForQuery(string(MsrOtherInfoPlane), True, True) + ', ' +
                      NormalizeStringForQuery(string(MsrOtherInfo), True, True) + ', ') + 'NOW(), '
                      + CurrentUser.sID + ', ' + 'NOW(), ' + CurrentUser.sID + ');';
                    LogThis(q, LogGroupGUID, lmtSQL);
                  end;
                  ...
                  begin
                    ...
                    begin
                      ...
                      begin
                        ...
                        LogThis('Внесение записи в таблицу событий.', LogGroupGUID, lmtInfo);
                        q := 'INSERT INTO ' + Configuration.sMySQLDatabase +
                          '._evn VALUES (NULL, NOW(),"INS","msr",' + IntToStr(iLastID) + ',' +
                          CurrentUser.sID + ',' + #39 + q + #39 + ');';
                        LogThis(q, LogGroupGUID, lmtSQL);
                        i := mysql_real_query(MySQLConnectionHandler, PAnsiChar(AnsiString(q)),
                          Length(q));
                        ...
                        begin
                          ...
                          LogThis('Количество обработанных строк равно ' + IntToStr(i) + '.',
                            LogGroupGUID, lmtInfo);
                          if (i = 1) then
                            LogThis('Запись в таблицу событий внесена успешно.',
                              LogGroupGUID, lmtInfo)
                          else
                            GenerateError('Количество обработанных строк (' + IntToStr(i) +
                              ') не соответствует требуемому (1)!', s, bError);
...
var
  PhonesForm: TPhonesForm;
  sExternalLog_Type, sExternalLog_Name, sExternalLog_Address, sExternalLog_NumberOfPhones,
    sExternalLog_PhonesList, sExternalLog_All: string;
  bError: boolean;
  LvGroup: TLVGROUP;
  LvItemA: TLVITEMA;
  i: Integer;
      ...
      begin
        ...
        begin
          ...
          try
            ...
            SendMessage(lvOrg.Handle, LVM_GETGROUPINFO, Longint(LvItemA.iGroupId),
              Longint(@LvGroup));
        ...
        begin
          ...
          begin
            ...
            sExternalLog_PhonesList := sExternalLog_PhonesList + PhonesForm.lvPhones.Items
              [i].Caption;
          end;
        end;
        ...
        if not bError then
        begin
          if (sExternalLog_Type + sExternalLog_Name + sExternalLog_Address +
            sExternalLog_NumberOfPhones + sExternalLog_PhonesList) <> '' then
          begin
            sExternalLog_All := 'Был произведён запрос списка телефонов по следующей организации:';
            if sExternalLog_Type > '' then
              sExternalLog_All := sExternalLog_All + sLineBreak + ' - тип организации: ' +
                sExternalLog_Type;
            if sExternalLog_Name > '' then
              sExternalLog_All := sExternalLog_All + sLineBreak + ' - название организации: ' +
                sExternalLog_Name;
            if sExternalLog_Address > '' then
              sExternalLog_All := sExternalLog_All + sLineBreak + ' - адрес организации: ' +
                sExternalLog_Address;
            if sExternalLog_NumberOfPhones > '' then
              sExternalLog_All := sExternalLog_All + sLineBreak +
                ' - количество телефонов организации: ' + sExternalLog_NumberOfPhones;
            if sExternalLog_PhonesList > '' then
              sExternalLog_All := sExternalLog_All + sLineBreak +
                ' - список телефонов организации: ' + sExternalLog_PhonesList;
          end;
        end;
      end
      else
...
begin
  ...
  m := AnsiString(Application.ExeName + #09 + u + #09 + // текущий залогированный пользователь
    ...
    sGroupGIUD + #09 + StringReplace(Format('%10u', [SessionLogEventCount]), ' ', '0',
    [rfReplaceAll]) + #09 + // порядковый номер сообщения
    a); // текст сообщения
  ...
  begin
    Screen.Cursor := crHourGlass;
    if ((Configuration.sCustomLogClientFile = '') and
      FileExists(ExtractFilePath(ExpandFileName(Application.ExeName)) + 'LogKeeper.exe')) then
      ShellExecute(Application.Handle, 'open',
        PChar(ExtractFilePath(ExpandFileName(Application.ExeName)) + 'LogKeeper.exe'), nil, nil,
        SW_NORMAL); // попытаться его запустить вновь
    if ((Configuration.sCustomLogClientFile <> '') and
      FileExists(Configuration.sCustomLogClientFile)) then
      ShellExecute(Application.Handle, 'open', PChar(Configuration.sCustomLogClientFile), nil, nil,
        SW_NORMAL); // попытаться его запустить вновь
  ...
  begin
    ...
    MessageBox(Handle,
      PChar('Не удалось отправить сообщение программе лог-клиента, так как не было найдено окно программы лог-клиента!'
      + sLineBreak + 'Обратитесь к администратору!'), PChar('OA4 - Ошибка!'), MB_OK + MB_ICONERROR);
end;
...
begin
  ...
  begin
    ...
    LogThis('Произведена отправка во внешнюю программу следующей строки данных: [' + aData + ']',
      LogGroupGUID, lmtInfo);
  end
  else
    GenerateError('Не найдена програма-получатель данных!', s, bError);
...
var
  ...
  s: string;
begin
  ProcedureHeader('Процедура выделения искомых строк в тексте компонента RichEdit', LogGroupGUID,
    False, False);
...
// сделать скрытие скрытых номеров телефонов, как в арм 109
