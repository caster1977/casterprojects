...
begin
  if MessageBox(Handle, PChar('Сгенерировано мероприятий: ' +
    ...
    PChar('OA4 - Подтверждение добавления'), MB_OKCANCEL + MB_ICONQUESTION + MB_DEFBUTTON2) = IDOK
  then
    ModalResult := mrOk;
end;
...
begin
  if FormatDateTime('dd.mm.yyyy', MonthCalendar1.Date) = FormatDateTime('dd.mm.yyyy',
    MonthCalendar1.EndDate) then
    lblPeriod.Caption := DateToStr(MonthCalendar1.Date)
  else
    lblPeriod.Caption := 'c ' + DateToStr(MonthCalendar1.Date) + ' по ' +
  ...
  try
    ...
    begin
      ...
      begin
        ...
        if StrToTimeDef(aSL[i] + ':00', aTime) = aTime then
...
begin
  ...
  if (Item1 <> nil) and (Item2 <> nil) then
    if (Item1.SubItems.Count = 1) and (Item1.SubItems.Count = 1) then
      ...
      begin
        if (StrToDateDef(Item1.Caption, aDate) = aDate) or
          (StrToDateDef(Item2.Caption, aDate) = aDate) or
          (StrToTimeDef(Item1.SubItems[0], aTime) = aTime) or
          (StrToTimeDef(Item2.SubItems[0], aTime) = aTime) then
          Compare := 0
        else
        begin
          if (StrToDateTimeDef(Item1.Caption + ' ' + Item1.SubItems[0], aDate + aTime) = aDate +
            aTime) or (StrToDateTimeDef(Item2.Caption + ' ' + Item2.SubItems[0], aDate + aTime)
            = aDate + aTime) then
            Compare := 0
          else
          begin
