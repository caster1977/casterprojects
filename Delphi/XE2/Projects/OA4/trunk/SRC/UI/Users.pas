...
begin
  ...
  q := 'SELECT usr_login, usr_is_admin, usr_erased FROM ' + MainForm.Configuration.sMySQLDatabase
    + '._usr;';
        ...
        begin
          ...
          begin
            ...
            begin
              ...
              if Node <> nil then
              begin
                Child := TreeView1.Items.AddChild(Node, string(ResultRow[0]));
                Child.ImageIndex := StrToInt(string(ResultRow[2]));
                Child.SelectedIndex := StrToInt(string(ResultRow[2]));
                Child.StateIndex := StrToInt(string(ResultRow[2]));
              end;
            end;
...
procedure TUsersForm.FormMouseWheelDown(Sender: TObject; Shift: TShiftState; MousePos: TPoint;
  var Handled: Boolean);
begin
  SendMessage(TreeView1.Handle, WM_VSCROLL, SB_LINEDOWN, 0);
end;

procedure TUsersForm.FormMouseWheelUp(Sender: TObject; Shift: TShiftState; MousePos: TPoint;
  var Handled: Boolean);
begin
  ...
  with TSetPasswordForm.Create(Self) do
    try
      ...
      begin
        ...
        MainForm.LogThis('Смена пароля для учётной записи ID="' + edbxID.Text + '"...',
          GroupGUID, lmtInfo);
        q := 'UPDATE ' + MainForm.Configuration.sMySQLDatabase + '._usr SET usr_password_md5=md5("'
          + edbxPassword.Text + '"), usr_modified_at=Now(), usr_modified_by_user_id=' +
          MainForm.CurrentUser.sID + ' WHERE usr_id="' + edbxID.Text + '";';
        MainForm.LogThis(q, GroupGUID, lmtSQL);
        i := mysql_real_query(MainForm.MySQLConnectionHandler, PAnsiChar(AnsiString(q)), Length(q));
        if i = 0 then
        begin
          ...
          if i = 1 then
          begin
            MainForm.LogThis('Количество строк выборки соответствует требуемому.',
              GroupGUID, lmtInfo);
            MainForm.LogThis('Смена пароля для учётной записи ID="' + edbxID.Text +
              '" прошла успешно.', GroupGUID, lmtInfo);
            MessageBox(Handle, PChar('Пароль для выбранной учётной записи успешно сменён.'),
              PChar('OA3 - Информация'), MB_OK + MB_ICONINFORMATION + MB_DEFBUTTON1);
          ...
          begin
            s := 'Количество обработанных строк (' + IntToStr(i) +
        ...
        begin
          MainForm.LogThis(s, GroupGUID, lmtError);
          MessageBox(Handle, PChar(s), PChar('OA3 - Ошибка!'),
...
begin
  ...
  if MessageBox(Handle,
    PChar('Вы действительно хотите пометить данную учётную запись на удаление?'),
    PChar('OA3 - Внимание!'), MB_YESNO + MB_ICONWARNING + MB_DEFBUTTON2) = IDYES then
  begin
    Screen.Cursor := crHourGlass;
    MainForm.LogThis('Пометка учётной записи ID="' + edbxID.Text + '" на удаление...',
      GroupGUID, lmtInfo);
    MainForm.ProgressBar1.Max := 2;
    q := 'UPDATE ' + MainForm.Configuration.sMySQLDatabase +
      '._usr SET usr_erased="1", usr_modified_at=Now(), usr_modified_by_user_id=' +
      MainForm.CurrentUser.sID + ' WHERE usr_id="' + edbxID.Text + '";';
    MainForm.LogThis(q, GroupGUID, lmtSQL);
    i := mysql_real_query(MainForm.MySQLConnectionHandler, PAnsiChar(AnsiString(q)), Length(q));
    if i = 0 then
    begin
      ...
      begin
        ...
        Node.StateIndex := 1;
        MainForm.LogThis('Учётная запись ID="' + edbxID.Text + '" успешно помечена на удаление.',
          GroupGUID, lmtInfo);
        UpdateButtons;
        TreeView1.Select(Node.Parent);
        TreeView1.Select(Node);
    ...
    begin
      ...
      q := 'INSERT INTO ' + MainForm.Configuration.sMySQLDatabase +
        '._evn VALUES (NULL, NOW(),"DEL","usr",' + edbxID.Text + ',' + MainForm.CurrentUser.sID +
        ',' + #39 + q + #39 + ');';
      MainForm.LogThis(q, GroupGUID, lmtSQL);
      i := mysql_real_query(MainForm.MySQLConnectionHandler, PAnsiChar(AnsiString(q)), Length(q));
      if i = 0 then
      begin
        MainForm.LogThis('Запрос выполнен успешно.', GroupGUID, lmtInfo);
        i := mysql_affected_rows(MainForm.MySQLConnectionHandler);
        MainForm.LogThis('Количество обработанных строк равно ' + IntToStr(i) + '.',
          GroupGUID, lmtInfo);
        if (i = 1) then
...
begin
  ...
  bError := False;
  if MessageBox(Handle,
    PChar('Вы действительно хотите снять пометку об удалении с текущей учётной записи?'),
    PChar('OA3 - Внимание!'), MB_YESNO + MB_ICONQUESTION + MB_DEFBUTTON2) = IDYES then
  begin
    Screen.Cursor := crHourGlass;
    MainForm.LogThis('Снятие пометки об удалении с учётной записи ID="' + edbxID.Text + '"...',
      GroupGUID, lmtInfo);
    MainForm.ProgressBar1.Max := 2;
    q := 'UPDATE ' + MainForm.Configuration.sMySQLDatabase +
      '._usr SET usr_erased="0", usr_modified_at=Now(), usr_modified_by_user_id=' +
      MainForm.CurrentUser.sID + ' WHERE usr_id="' + edbxID.Text + '";';
    MainForm.LogThis(q, GroupGUID, lmtSQL);
    i := mysql_real_query(MainForm.MySQLConnectionHandler, PAnsiChar(AnsiString(q)), Length(q));
    if i = 0 then
    begin
      ...
      begin
        ...
        Node.StateIndex := 0;
        MainForm.LogThis('Снятие пометки об удалении с учётной pfgbcb ID="' + edbxID.Text +
          '" прошло успешно.', GroupGUID, lmtInfo);
        UpdateButtons;
        TreeView1.Select(Node.Parent);
        TreeView1.Select(Node);
      end
    ...
    begin
      MainForm.LogThis('Внесение записи в таблицу событий.', GroupGUID, lmtInfo);
      q := 'INSERT INTO ' + MainForm.Configuration.sMySQLDatabase +
        '._evn VALUES (NULL, NOW(),"REC","usr",' + edbxID.Text + ',' + MainForm.CurrentUser.sID +
        ',' + #39 + q + #39 + ');';
      MainForm.LogThis(q, GroupGUID, lmtSQL);
      i := mysql_real_query(MainForm.MySQLConnectionHandler, PAnsiChar(AnsiString(q)), Length(q));
      if i = 0 then
  ...
  begin
    ...
    begin
      ...
      q := 'SELECT a.usr_id, a.usr_login, a.usr_fullname, ' +
        ...
        'f.usr_fullname, date_format(a.usr_modified_at,"%d.%m.%Y %T") ' + 'FROM ' +
        MainForm.Configuration.sMySQLDatabase + '._usr a ' + 'LEFT JOIN ' +
        MainForm.Configuration.sMySQLDatabase + '._usr b ON b.usr_id=a.usr_created_by_user_id ' +
        'LEFT JOIN ' + MainForm.Configuration.sMySQLDatabase +
        '._usr f ON f.usr_id=a.usr_modified_by_user_id ' + 'WHERE a.usr_login="' + Node.Text + '";';
      MainForm.LogThis(q, GroupGUID, lmtSQL);
      i := mysql_real_query(MainForm.MySQLConnectionHandler, PAnsiChar(AnsiString(q)), Length(q));
      if i = 0 then
      begin
        ...
        begin
          ...
          begin
            ...
            begin
              ...
              MainForm.LogThis
                ('Получение данных и прав доступа указанного пользователя произведено успешно.',
                GroupGUID, lmtInfo);
              edbxID.Text := string(ResultRow[0]);
              edbxLogin.Text := string(ResultRow[1]);
              edbxName.Text := string(ResultRow[2]);
              chkbxDBEdition.Checked := string(ResultRow[3]) = '1';
              chkbxDBBackup.Checked := string(ResultRow[4]) = '1';
              chkbxDBCleaning.Checked := string(ResultRow[5]) = '1';
              chkbxDBDefragmentation.Checked := string(ResultRow[6]) = '1';
              chkbxDBMaintenance.Checked := string(ResultRow[7]) = '1';
              chkbxDBUsersEdition.Checked := string(ResultRow[8]) = '1';
              lblCreationDetails.Caption := string(ResultRow[9]) + ', ' + string(ResultRow[10]);
              lblModificationDetails.Caption := string(ResultRow[11]) + ', ' +
                string(ResultRow[12]);
            end
            else
            begin
    ...
    lblHint.Caption := 'Нажмите кнопку "Добавить" для добавления нового пользователя в группу "' +
      Node.Text + '".';
  bUserModified := False;
  UpdateButtons;
end;
...
begin
  ...
  begin
    ...
    MainForm.ProgressBar1.Max := 2;
    MainForm.LogThis('Добавление новой учётной записи с группу "' + Node.Parent.Text + '"...',
      GroupGUID, lmtInfo);
    q := 'INSERT INTO ' + MainForm.Configuration.sMySQLDatabase +
      '._usr (usr_id, usr_erased, usr_login, usr_password_md5, ' +
      ...
      'usr_modified_at, usr_modified_by_user_id) VALUES ' + '(NULL, "' +
      IntToStr(integer(Node.ImageIndex = 1)) + '", "' + edbxLogin.Text + '", md5(""), "' +
      edbxName.Text + '", "' + IntToStr(integer(Node.Parent.Text = 'Администраторы')) + '", "' +
      ...
      IntToStr(integer(chkbxDBUsersEdition.Checked and chkbxDBUsersEdition.Enabled)) + '", Now(), '
      + MainForm.CurrentUser.sID + ', Now(), ' + MainForm.CurrentUser.sID + ')';
    MainForm.LogThis(q, GroupGUID, lmtSQL);
    i := mysql_real_query(MainForm.MySQLConnectionHandler, PAnsiChar(AnsiString(q)), Length(q));
    if i = 0 then
    begin
      MainForm.LogThis('Запрос выполнен успешно.', GroupGUID, lmtInfo);
      iLastID := mysql_insert_id(MainForm.MySQLConnectionHandler);
      ...
      begin
        ...
        TreeView1.Select(Node.Parent);
        TreeView1.Select(Node);
        MainForm.LogThis('Учётная запись ID="' + IntToStr(iLastID) + '" добавлена успешно.',
          GroupGUID, lmtInfo);
      end
      else
      begin
    ...
    begin
      ...
      MainForm.LogThis('Внесение записи в таблицу событий.', GroupGUID, lmtInfo);
      q := 'INSERT INTO ' + MainForm.Configuration.sMySQLDatabase +
        '._evn VALUES (NULL, NOW(),"INS","usr",' + IntToStr(iLastID) + ',' +
        MainForm.CurrentUser.sID + ',' + #39 + q + #39 + ');';
      MainForm.LogThis(q, GroupGUID, lmtSQL);
      i := mysql_real_query(MainForm.MySQLConnectionHandler, PAnsiChar(AnsiString(q)), Length(q));
      if i = 0 then
      begin
    ...
    begin
      MessageBox(Handle, PChar('Для данной учётной записи задан пустой пароль.' + sLineBreak +
        'Не забудьте сменить его!'), PChar('OA4 - Внимание!'),
  ...
  begin
    Screen.Cursor := crHourGlass;
    MainForm.LogThis('Внесение изменений в учётную запись "' + Node.Text + '"...',
      GroupGUID, lmtInfo);
    q := 'UPDATE ' + MainForm.Configuration.sMySQLDatabase + '._usr SET ' + 'usr_login="' +
      edbxLogin.Text + '", ' + 'usr_erased="' + IntToStr(integer(Node.ImageIndex = 1)) + '", ' +
      'usr_fullname="' + edbxName.Text + '", ' + 'usr_is_admin="' +
      IntToStr(integer(Node.Parent.Text = 'Администраторы')) + '", ' + 'usr_can_edit_db="' +
      IntToStr(integer(chkbxDBEdition.Checked)) + '", ' + 'usr_can_backup_db="' +
      IntToStr(integer(chkbxDBBackup.Checked and chkbxDBBackup.Enabled)) + '", ' +
      ...
      'usr_can_maintain_db="' +
      IntToStr(integer(chkbxDBMaintenance.Checked and chkbxDBMaintenance.Enabled)) + '", ' +
      'usr_can_edit_db_users="' +
      IntToStr(integer(chkbxDBUsersEdition.Checked and chkbxDBUsersEdition.Enabled)) + '", ' +
      'usr_modified_at=Now(), ' + 'usr_modified_by_user_id="' + MainForm.CurrentUser.sID + '" ' +
    ...
    begin
      ...
      begin
        ...
        if TreeView1.Selected.Text <> edbxLogin.Text then
          TreeView1.Selected.Text := edbxLogin.Text;
        MainForm.LogThis('Изменения в учётную запись ID="' + edbxID.Text + '" внесены успешно.',
          GroupGUID, lmtInfo);
      end
      else
      begin
    ...
    begin
      ...
      MainForm.LogThis('Внесение записи в таблицу событий.', GroupGUID, lmtInfo);
      q := 'INSERT INTO ' + MainForm.Configuration.sMySQLDatabase +
        '._evn VALUES (NULL, NOW(),"MOD","usr",' + edbxID.Text + ',' + MainForm.CurrentUser.sID +
        ',' + #39 + q + #39 + ');';
      MainForm.LogThis(q, GroupGUID, lmtSQL);
      i := mysql_real_query(MainForm.MySQLConnectionHandler, PAnsiChar(AnsiString(q)), Length(q));
      if i = 0 then
...
begin
  if bUserModified then
    if MessageBox(Handle, PChar('Вы внесли изменения в учётную запись "' + TreeView1.Selected.Text +
      '".' + sLineBreak + 'Сохранить изменения?'), PChar('OA3 - Внимание!'),
      MB_OKCANCEL + MB_ICONWARNING + MB_DEFBUTTON1) = mrOk then
...
begin
  b := False;
  bUserNode := (TreeView1.Selected.Text <> 'Администраторы') and
    (TreeView1.Selected.Text <> 'Пользователи');
  for i := 0 to TreeView1.Items.Count - 1 do
    if (TreeView1.Items.Item[i].Text = edbxLogin.Text) and
      (TreeView1.Items.Item[i] <> TreeView1.Selected) then
      b := True;
  btnSaveUser.Enabled := bUserModified and (edbxLogin.Text <> 'Администраторы') and
    (edbxLogin.Text <> 'Пользователи') and (edbxLogin.Text <> '') and
    (edbxLogin.Text <> '<НОВЫЙ ПОЛЬЗОВАТЕЛЬ>') and (not b);
  btnAddUser.Enabled := (not bUserNode) and (TreeView1.Selected.Text <> '<НОВЫЙ ПОЛЬЗОВАТЕЛЬ>');
  btnEraseUser.Visible := (not Boolean(TreeView1.Selected.ImageIndex)) and bUserNode and
    (TreeView1.Selected.Text <> '<НОВЫЙ ПОЛЬЗОВАТЕЛЬ>');
  btnEraseUser.Enabled := (not Boolean(TreeView1.Selected.ImageIndex)) and bUserNode and
    (TreeView1.Selected.Text <> '<НОВЫЙ ПОЛЬЗОВАТЕЛЬ>');
  btnRestoreUser.Visible := Boolean(TreeView1.Selected.ImageIndex) and bUserNode and
    (TreeView1.Selected.Text <> '<НОВЫЙ ПОЛЬЗОВАТЕЛЬ>');
  btnRestoreUser.Enabled := Boolean(TreeView1.Selected.ImageIndex) and bUserNode and
    (TreeView1.Selected.Text <> '<НОВЫЙ ПОЛЬЗОВАТЕЛЬ>');
