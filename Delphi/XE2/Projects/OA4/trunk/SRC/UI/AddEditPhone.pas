...
// функция проверки, состоит ли строка-источник из цифровых символов
var
  i: integer;
  b: boolean;
begin
...
var
  sPhone: string;
  // переменная для хранения значения "очищенной" от символов-разделителей строки с номером телефона
begin
  sPhone := // получаем очищенную от символов-разделителей строку с номером телефона
    StringReplace
    (StringReplace(StringReplace(StringReplace(StringReplace(StringReplace(StringReplace
    (edbxPhoneNumber.Text, ' ', '', [rfReplaceAll]), // очищаем от пробелов
  ...
  begin
    if (Pos('2', sPhone) = 1) and (Length(sPhone) = Length('2') + 6) then
    // производим отбор городских телефонов на 2
      cmbbxPhoneType.ItemIndex := cmbbxPhoneType.Items.IndexOf('стационарный')
    else
      if (Pos('3', sPhone) = 1) and (Length(sPhone) = Length('3') + 6) then
      // производим отбор городских телефонов на 3
        cmbbxPhoneType.ItemIndex := cmbbxPhoneType.Items.IndexOf('стационарный')
      else
        if (Pos('5', sPhone) = 1) and (Length(sPhone) = Length('5') + 6) then
        // производим отбор пригородных телефонов на 5
          cmbbxPhoneType.ItemIndex := cmbbxPhoneType.Items.IndexOf('стационарный')
        else
          if (Pos('375255', sPhone) = 1) and (Length(sPhone) = Length('375255') + 6) then
          // производим отбор телефонов БеСТ
                ...
                if (Pos('00255', sPhone) = 1) and (Length(sPhone) = Length('00255') + 6) then
                  ...
                  // производим отбор телефонов БеСТ
                    ...
                    if (Pos('80256', sPhone) = 1) and (Length(sPhone) = Length('80256') + 6) then
                      cmbbxPhoneType.ItemIndex :=
                        cmbbxPhoneType.Items.IndexOf('мобильный (life:)/БеСТ)')
                    else
                      if (Pos('0256', sPhone) = 1) and (Length(sPhone) = Length('0256') + 6) then
                        cmbbxPhoneType.ItemIndex :=
                          cmbbxPhoneType.Items.IndexOf('мобильный (life:)/БеСТ)')
                      else
                        if (Pos('00256', sPhone) = 1) and (Length(sPhone) = Length('00256') + 6)
                        then
                          cmbbxPhoneType.ItemIndex :=
                            cmbbxPhoneType.Items.IndexOf('мобильный (life:)/БеСТ)')
                        else
                          if (Pos('375257', sPhone) = 1) and (Length(sPhone) = Length('375257') + 6)
                          then // производим отбор телефонов БеСТ
                            cmbbxPhoneType.ItemIndex :=
                              cmbbxPhoneType.Items.IndexOf('мобильный (life:)/БеСТ)')
                          else
                            if (Pos('80257', sPhone) = 1) and (Length(sPhone) = Length('80257') + 6)
                            then
                              cmbbxPhoneType.ItemIndex :=
                                cmbbxPhoneType.Items.IndexOf('мобильный (life:)/БеСТ)')
                            else
                              if (Pos('0257', sPhone) = 1) and (Length(sPhone) = Length('0257') + 6)
                              then
                                cmbbxPhoneType.ItemIndex :=
                                  cmbbxPhoneType.Items.IndexOf('мобильный (life:)/БеСТ)')
                              else
                                if (Pos('00257', sPhone) = 1) and
                                  (Length(sPhone) = Length('00257') + 6) then
                                  cmbbxPhoneType.ItemIndex :=
                                    cmbbxPhoneType.Items.IndexOf('мобильный (life:)/БеСТ)')
                                else
                                  if (Pos('375259', sPhone) = 1) and
                                    (Length(sPhone) = Length('375259') + 6) then
                                  // производим отбор телефонов БеСТ
                                    cmbbxPhoneType.ItemIndex :=
                                      cmbbxPhoneType.Items.IndexOf('мобильный (life:)/БеСТ)')
                                  else
                                    if (Pos('80259', sPhone) = 1) and
                                      (Length(sPhone) = Length('80259') + 6) then
                                      cmbbxPhoneType.ItemIndex :=
                                        cmbbxPhoneType.Items.IndexOf('мобильный (life:)/БеСТ)')
                                    else
                                      if (Pos('0259', sPhone) = 1) and
                                        (Length(sPhone) = Length('0259') + 6) then
                                        cmbbxPhoneType.ItemIndex :=
                                        cmbbxPhoneType.Items.IndexOf('мобильный (life:)/БеСТ)')
                                      else
                                        if (Pos('00259', sPhone) = 1) and
                                        (Length(sPhone) = Length('00259') + 6) then
                                        cmbbxPhoneType.ItemIndex :=
                                        cmbbxPhoneType.Items.IndexOf('мобильный (life:)/БеСТ)')
                                        else
                                        if (Pos('375294', sPhone) = 1) and
                                        (Length(sPhone) = Length('375294') + 6) then
                                        // производим отбор телефонов БелСел
                                        cmbbxPhoneType.ItemIndex :=
                                        cmbbxPhoneType.Items.IndexOf('мобильный (Diallog/БелСел)')
                                        else
                                        if (Pos('80294', sPhone) = 1) and
                                        (Length(sPhone) = Length('80294') + 6) then
                                        cmbbxPhoneType.ItemIndex :=
                                        cmbbxPhoneType.Items.IndexOf('мобильный (Diallog/БелСел)')
                                        else
                                        if (Pos('0294', sPhone) = 1) and
                                        (Length(sPhone) = Length('0294') + 6) then
                                        cmbbxPhoneType.ItemIndex :=
                                        cmbbxPhoneType.Items.IndexOf('мобильный (Diallog/БелСел)')
                                        else
                                        if (Pos('00294', sPhone) = 1) and
                                        (Length(sPhone) = Length('00294') + 6) then
                                        cmbbxPhoneType.ItemIndex :=
                                        cmbbxPhoneType.Items.IndexOf('мобильный (Diallog/БелСел)')
                                        else
                                        if (Pos('375297', sPhone) = 1) and
                                        (Length(sPhone) = Length('375297') + 6) then
                                        // производим отбор телефонов МТС
                                        cmbbxPhoneType.ItemIndex :=
                                        cmbbxPhoneType.Items.IndexOf('мобильный (МТС)')
                                        else
                                        if (Pos('80297', sPhone) = 1) and
                                        (Length(sPhone) = Length('80297') + 6) then
                                        cmbbxPhoneType.ItemIndex :=
                                        cmbbxPhoneType.Items.IndexOf('мобильный (МТС)')
                                        else
                                        if (Pos('0297', sPhone) = 1) and
                                        (Length(sPhone) = Length('0297') + 6) then
                                        cmbbxPhoneType.ItemIndex :=
                                        cmbbxPhoneType.Items.IndexOf('мобильный (МТС)')
                                        else
                                        if (Pos('00297', sPhone) = 1) and
                                        (Length(sPhone) = Length('00297') + 6) then
                                        cmbbxPhoneType.ItemIndex :=
                                        cmbbxPhoneType.Items.IndexOf('мобильный (МТС)')
                                        else
                                        if (Pos('375295', sPhone) = 1) and
                                        (Length(sPhone) = Length('375295') + 6) then
                                        cmbbxPhoneType.ItemIndex :=
                                        cmbbxPhoneType.Items.IndexOf('мобильный (МТС)')
                                        else
                                        if (Pos('80295', sPhone) = 1) and
                                        (Length(sPhone) = Length('80295') + 6) then
                                        cmbbxPhoneType.ItemIndex :=
                                        cmbbxPhoneType.Items.IndexOf('мобильный (МТС)')
                                        else
                                        if (Pos('0295', sPhone) = 1) and
                                        (Length(sPhone) = Length('0295') + 6) then
                                        cmbbxPhoneType.ItemIndex :=
                                        cmbbxPhoneType.Items.IndexOf('мобильный (МТС)')
                                        else
                                        if (Pos('00295', sPhone) = 1) and
                                        (Length(sPhone) = Length('00295') + 6) then
                                        cmbbxPhoneType.ItemIndex :=
                                        cmbbxPhoneType.Items.IndexOf('мобильный (МТС)')
                                        else
                                        if (Pos('375292', sPhone) = 1) and
                                        (Length(sPhone) = Length('375292') + 6) then
                                        cmbbxPhoneType.ItemIndex :=
                                        cmbbxPhoneType.Items.IndexOf('мобильный (МТС)')
                                        else
                                        if (Pos('80292', sPhone) = 1) and
                                        (Length(sPhone) = Length('80292') + 6) then
                                        cmbbxPhoneType.ItemIndex :=
                                        cmbbxPhoneType.Items.IndexOf('мобильный (МТС)')
                                        else
                                        if (Pos('0292', sPhone) = 1) and
                                        (Length(sPhone) = Length('0292') + 6) then
                                        cmbbxPhoneType.ItemIndex :=
                                        cmbbxPhoneType.Items.IndexOf('мобильный (МТС)')
                                        else
                                        if (Pos('00292', sPhone) = 1) and
                                        (Length(sPhone) = Length('00292') + 6) then
                                        cmbbxPhoneType.ItemIndex :=
                                        cmbbxPhoneType.Items.IndexOf('мобильный (МТС)')
                                        else
                                        if (Pos('375298', sPhone) = 1) and
                                        (Length(sPhone) = Length('375298') + 6) then
                                        cmbbxPhoneType.ItemIndex :=
                                        cmbbxPhoneType.Items.IndexOf('мобильный (МТС)')
                                        else
                                        if (Pos('80298', sPhone) = 1) and
                                        (Length(sPhone) = Length('80298') + 6) then
                                        cmbbxPhoneType.ItemIndex :=
                                        cmbbxPhoneType.Items.IndexOf('мобильный (МТС)')
                                        else
                                        if (Pos('0298', sPhone) = 1) and
                                        (Length(sPhone) = Length('0298') + 6) then
                                        cmbbxPhoneType.ItemIndex :=
                                        cmbbxPhoneType.Items.IndexOf('мобильный (МТС)')
                                        else
                                        if (Pos('00298', sPhone) = 1) and
                                        (Length(sPhone) = Length('00298') + 6) then
                                        cmbbxPhoneType.ItemIndex :=
                                        cmbbxPhoneType.Items.IndexOf('мобильный (МТС)')
                                        else
                                        if (Pos('37533', sPhone) = 1) and
                                        (Length(sPhone) = Length('37533') + 7) then
                                        cmbbxPhoneType.ItemIndex :=
                                        cmbbxPhoneType.Items.IndexOf('мобильный (МТС)')
                                        else
                                        if (Pos('8033', sPhone) = 1) and
                                        (Length(sPhone) = Length('8033') + 7) then
                                        cmbbxPhoneType.ItemIndex :=
                                        cmbbxPhoneType.Items.IndexOf('мобильный (МТС)')
                                        else
                                        if (Pos('033', sPhone) = 1) and
                                        (Length(sPhone) = Length('033') + 7) then
                                        cmbbxPhoneType.ItemIndex :=
                                        cmbbxPhoneType.Items.IndexOf('мобильный (МТС)')
                                        else
                                        if (Pos('0033', sPhone) = 1) and
                                        (Length(sPhone) = Length('0033') + 7) then
                                        cmbbxPhoneType.ItemIndex :=
                                        cmbbxPhoneType.Items.IndexOf('мобильный (МТС)')
                                        else
                                        if (Pos('375296', sPhone) = 1) and
                                        (Length(sPhone) = Length('375296') + 6) then
                                        // производим отбор телефонов МЦС
                                        cmbbxPhoneType.ItemIndex :=
                                        cmbbxPhoneType.Items.IndexOf('мобильный (Velcom/МЦС)')
                                        else
                                        if (Pos('80296', sPhone) = 1) and
                                        (Length(sPhone) = Length('80296') + 6) then
                                        cmbbxPhoneType.ItemIndex :=
                                        cmbbxPhoneType.Items.IndexOf('мобильный (Velcom/МЦС)')
                                        else
                                        if (Pos('0296', sPhone) = 1) and
                                        (Length(sPhone) = Length('0296') + 6) then
                                        cmbbxPhoneType.ItemIndex :=
                                        cmbbxPhoneType.Items.IndexOf('мобильный (Velcom/МЦС)')
                                        else
                                        if (Pos('00296', sPhone) = 1) and
                                        (Length(sPhone) = Length('00296') + 6) then
                                        cmbbxPhoneType.ItemIndex :=
                                        cmbbxPhoneType.Items.IndexOf('мобильный (Velcom/МЦС)')
                                        else
                                        if (Pos('375293', sPhone) = 1) and
                                        (Length(sPhone) = Length('375293') + 6) then
                                        cmbbxPhoneType.ItemIndex :=
                                        cmbbxPhoneType.Items.IndexOf('мобильный (Velcom/МЦС)')
                                        else
                                        if (Pos('80293', sPhone) = 1) and
                                        (Length(sPhone) = Length('80293') + 6) then
                                        cmbbxPhoneType.ItemIndex :=
                                        cmbbxPhoneType.Items.IndexOf('мобильный (Velcom/МЦС)')
                                        else
                                        if (Pos('0293', sPhone) = 1) and
                                        (Length(sPhone) = Length('0293') + 6) then
                                        cmbbxPhoneType.ItemIndex :=
                                        cmbbxPhoneType.Items.IndexOf('мобильный (Velcom/МЦС)')
                                        else
                                        if (Pos('00293', sPhone) = 1) and
                                        (Length(sPhone) = Length('00293') + 6) then
                                        cmbbxPhoneType.ItemIndex :=
                                        cmbbxPhoneType.Items.IndexOf('мобильный (Velcom/МЦС)')
                                        else
                                        if (Pos('375291', sPhone) = 1) and
                                        (Length(sPhone) = Length('375291') + 6) then
                                        cmbbxPhoneType.ItemIndex :=
                                        cmbbxPhoneType.Items.IndexOf('мобильный (Velcom/МЦС)')
                                        else
                                        if (Pos('80291', sPhone) = 1) and
                                        (Length(sPhone) = Length('80291') + 6) then
                                        cmbbxPhoneType.ItemIndex :=
                                        cmbbxPhoneType.Items.IndexOf('мобильный (Velcom/МЦС)')
                                        else
                                        if (Pos('0291', sPhone) = 1) and
                                        (Length(sPhone) = Length('0291') + 6) then
                                        cmbbxPhoneType.ItemIndex :=
                                        cmbbxPhoneType.Items.IndexOf('мобильный (Velcom/МЦС)')
                                        else
                                        if (Pos('00291', sPhone) = 1) and
                                        (Length(sPhone) = Length('00291') + 6) then
                                        cmbbxPhoneType.ItemIndex :=
                                        cmbbxPhoneType.Items.IndexOf('мобильный (Velcom/МЦС)')
                                        else
                                        if (Pos('375299', sPhone) = 1) and
                                        (Length(sPhone) = Length('375299') + 6) then
                                        cmbbxPhoneType.ItemIndex :=
                                        cmbbxPhoneType.Items.IndexOf('мобильный (Velcom/МЦС)')
                                        else
                                        if (Pos('80299', sPhone) = 1) and
                                        (Length(sPhone) = Length('80299') + 6) then
                                        cmbbxPhoneType.ItemIndex :=
                                        cmbbxPhoneType.Items.IndexOf('мобильный (Velcom/МЦС)')
                                        else
                                        if (Pos('0299', sPhone) = 1) and
                                        (Length(sPhone) = Length('0299') + 6) then
                                        cmbbxPhoneType.ItemIndex :=
                                        cmbbxPhoneType.Items.IndexOf('мобильный (Velcom/МЦС)')
                                        else
                                        if (Pos('00299', sPhone) = 1) and
                                        (Length(sPhone) = Length('00299') + 6) then
                                        cmbbxPhoneType.ItemIndex :=
                                        cmbbxPhoneType.Items.IndexOf('мобильный (Velcom/МЦС)')
                                        else
                                        if (Pos('37544', sPhone) = 1) and
                                        (Length(sPhone) = Length('37544') + 7) then
                                        cmbbxPhoneType.ItemIndex :=
                                        cmbbxPhoneType.Items.IndexOf('мобильный (Velcom/МЦС)')
                                        else
                                        if (Pos('8044', sPhone) = 1) and
                                        (Length(sPhone) = Length('8044') + 7) then
                                        cmbbxPhoneType.ItemIndex :=
                                        cmbbxPhoneType.Items.IndexOf('мобильный (Velcom/МЦС)')
                                        else
                                        if (Pos('044', sPhone) = 1) and
                                        (Length(sPhone) = Length('044') + 7) then
                                        cmbbxPhoneType.ItemIndex :=
                                        cmbbxPhoneType.Items.IndexOf('мобильный (Velcom/МЦС)')
                                        else
                                        if (Pos('0044', sPhone) = 1) and
                                        (Length(sPhone) = Length('0044') + 7) then
                                        cmbbxPhoneType.ItemIndex :=
                                        cmbbxPhoneType.Items.IndexOf('мобильный (Velcom/МЦС)')
                                        else
                                        cmbbxPhoneType.ItemIndex :=
                                        cmbbxPhoneType.Items.IndexOf('');
  end
  else
...
begin
  edbxPhoneNumber.Text :=
    StringReplace
    (StringReplace(StringReplace(StringReplace(StringReplace(StringReplace(StringReplace
    (StringReplace(Trim(edbxPhoneNumber.Text), '  ', ' ', [rfReplaceAll]),
...
// процедура-обработчик поети фокуса полем ввода "Комментарии"
begin
  ...
  // то выполняем валидацию значения строки поля ввода "Комментария"
end;
