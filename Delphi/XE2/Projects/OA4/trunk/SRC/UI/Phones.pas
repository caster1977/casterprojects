...
begin
  ...
  begin
    lvPhones.Column[2].Width := lvPhones.Width - lvPhones.Column[0].Width - lvPhones.Column[1].Width
      - integer(b) * 70 - (lvPhones.BevelWidth * 2) - 2 - GetSystemMetrics(SM_CXVSCROLL);
  end
  else
  begin
    lvPhones.Column[2].Width := lvPhones.Width - lvPhones.Column[0].Width - lvPhones.Column[1].Width
      - integer(b) * 70 - (lvPhones.BevelWidth * 2) - 2;
  end;
...
begin
  ...
  MainForm.ProcedureHeader('Процедура реакции на двойной шелчок в списке телефонов', LogGroupGUID,
    False, True);

  if lvPhones.Selected <> nil then
  begin
    MainForm.LogThis
      ('Двойной щелчок сделан по элементу списка телефонов. Открываем окно редактирования для выделенного элемента.',
      LogGroupGUID, lmtInfo);
    if btnEdit.Visible and btnEdit.Enabled then
      btnEdit.Click;
  end
  else
    MainForm.LogThis('Двойной щелчок сделан мимо элементов списка телефонов.',
      LogGroupGUID, lmtInfo);
...
begin
  ...
  MainForm.ProcedureHeader('Процедура реакции на отжатие клавиши клавиатуры в списке телефонов',
    LogGroupGUID, False, True);

  if Key = VK_SPACE then
  begin
    MainForm.LogThis
      ('Была нажата клавиша ПРОБЕЛ. Инициируем отправку выделенного номера телефона на голосовую карту.',
      LogGroupGUID, lmtInfo);
    btnVioceCard.Click;
  end;
...
begin
  ...
  MainForm.ProcedureHeader('Процедура реакции на изменение выделения элементов списка телефонов',
    LogGroupGUID, False, True);
...
begin
  ...
  MainForm.ProcedureHeader('Процедура создания и инициализации окна списка телефонов', LogGroupGUID,
    False, True);
...
begin
  ...
  MainForm.ProcedureHeader('Процедура отображения окна списка телефонов', LogGroupGUID,
    False, True);

  pnlModification.Visible := lvPhones.Items.Count > 0;
  if lvPhones.Items.Count > 0 then
  begin
    MainForm.LogThis('Список телефонов не пустой, выделяем первый элемент в списке телефонов.',
      LogGroupGUID, lmtInfo);
...
procedure TPhonesForm.lvPhonesCustomDrawItem(Sender: TCustomListView; Item: TListItem;
  State: TCustomDrawState; var DefaultDraw: Boolean);
begin
  if Item <> nil then
  begin
...
begin
  ...
  begin
    ...
    if phn_id > -1 then
      q := 'SELECT phn_otdel_id FROM ' + MainForm.Configuration.sMySQLDatabase +
        '._phn WHERE phn_id=' + IntToStr(phn_id) + ';'
      // запрос на получение id записи, которую нужно удалить из таблицы minsk.otdel
    else
    begin
    ...
    begin
      ...
      begin
        ...
        begin
          if i = 1 then
          begin
            MainForm.LogThis('Количество строк полученной выборки соответствует требуемому (1).',
              LogGroupGUID, lmtInfo);
          ...
          begin
            s := 'Получено некорректное количество строк результирующей выборки (' +
              IntToStr(i) + ')!';
            bError := True;
          end;
        end
  ...
  if not bError then
  begin
    q := 'DELETE FROM ' + MainForm.Configuration.sMySQLDatabase + '._phn WHERE phn_id=' +
      IntToStr(phn_id) + ';';
    MainForm.LogThis(q, LogGroupGUID, lmtSQL);
    i := mysql_real_query(MainForm.MySQLConnectionHandler, PAnsiChar(AnsiString(q)), Length(q));
    if i = 0 then
    begin
      ...
      if i = 1 then
        MainForm.LogThis('Количество удалённых строк соответствует требуемому (1).',
          LogGroupGUID, lmtInfo)
      else
      begin
  ...
  begin
    ...
    q := 'INSERT INTO ' + MainForm.Configuration.sMySQLDatabase +
      '._evn VALUES (NULL, NOW(),"DEL","phn",' + IntToStr(phn_id) + ',' + MainForm.CurrentUser.sID +
      ',' + #39 + q + #39 + ');';
    MainForm.LogThis(q, LogGroupGUID, lmtSQL);
    i := mysql_real_query(MainForm.MySQLConnectionHandler, PAnsiChar(AnsiString(q)), Length(q));
    if i = 0 then
...
var
  ...
  i, iLastID, i_firm_code, i_city_id, i_street_id, i_house_num, i_subhouse_num, i_flat_num,
    i_phn_owner_id, i_otdel_id: integer;
  bError: Boolean;
  ResultSet: PMYSQL_RES;
  ResultRow: PMYSQL_ROW;
  LogGroupGUID: string;
      ...
      begin
          ...
          begin
            ...
            begin
              q := 'SELECT firm_code, city_id, street_id, house_num, house_sym, subhouse_num, subhouse_sym, flat_num, flat_sym '
                + 'FROM ' + MainForm.Configuration.sMySQLDatabase +
                '._org LEFT JOIN minsk.firma ON firm_code=org_firm_code WHERE org_id=' +
                IntToStr(i_phn_owner_id) + ';';
            end
            else
            begin
        ...
        begin
          ...
          begin
            ...
            begin
              q := 'SELECT firm_code, city_id, street_id, house_num, house_sym, subhouse_num, subhouse_sym, flat_num, flat_sym '
                + 'FROM ' + MainForm.Configuration.sMySQLDatabase + '._msr LEFT JOIN ' +
                MainForm.Configuration.sMySQLDatabase +
                '._org ON org_id=msr_organization_id LEFT JOIN minsk.firma ON firm_code=org_firm_code WHERE msr_id='
                + IntToStr(i_phn_owner_id) + ';';
            end
            else
            begin
          ...
          begin
            ...
            begin
              ...
              begin
                ...
                begin
                  MainForm.LogThis
                    ('Количество строк полученной выборки соответствует требуемому (1).',
                    LogGroupGUID, lmtInfo);
                  ResultRow := mysql_fetch_row(ResultSet);
                  if ResultRow <> nil then
                  begin
        ...
        begin
          ...
          begin
            ...
            if i = 1 then
            begin
              MainForm.LogThis('Добавление новой записи в таблицу minsk.otdel произведено успешно',
                LogGroupGUID, lmtInfo);
              i_otdel_id := iLastID;
            end
            else
        ...
        begin
          q := 'INSERT INTO ' + MainForm.Configuration.sMySQLDatabase + '._phn ' +
            ...
            '"' + IntToStr(integer(MainForm.PageControl1.ActivePage = MainForm.tsMsr)) + '", ' +
          // phn_owner_is_measure
            'NOW(), ' + // phn_modified_at
          ...
          begin
            ...
            i := mysql_affected_rows(MainForm.MySQLConnectionHandler);
            if i = 1 then
              MainForm.LogThis('Добавление новой записи в таблицу _phn произведено успешно',
                LogGroupGUID, lmtInfo)
            else
            begin
              s := 'Количество обработанных строк (' + IntToStr(i) +
        ...
        begin
          MainForm.LogThis('Внесение записи в таблицу событий.', LogGroupGUID, lmtInfo);
          q := 'INSERT INTO ' + MainForm.Configuration.sMySQLDatabase +
            '._evn VALUES (NULL, NOW(),"INS","phn",' + IntToStr(iLastID) + ',' +
    ...
    try
      ...
      begin
        ...
        if i_phn_id > -1 then
          q := 'SELECT phone, phn_type, date_format(phn_compared,"%d.%m.%Y"), phn_comments, phn_otdel_id, '
            + 'date_format(phn_modified_at,"%d.%m.%Y %T"), usr_fullname, phn_priority FROM ' +
            MainForm.Configuration.sMySQLDatabase + '._phn ' +
            'LEFT JOIN minsk.otdel ON otdel_id=phn_otdel_id ' + 'LEFT JOIN ' +
            MainForm.Configuration.sMySQLDatabase +
            '._usr ON usr_id=phn_modified_by_user_id WHERE phn_id=' + IntToStr(i_phn_id) + ';'
          // запрос на получение данных для заполнения полей формы
        else
        begin
      ...
      begin
        ...
        if i_phn_otdel_id > -1 then
          q := 'UPDATE minsk.otdel SET ' + 'phone="' + Trim(edbxPhoneNumber.Text) + '", ' +
            'date=NOW() ' + 'WHERE otdel_id=' + IntToStr(i_phn_otdel_id) + ';'
        else
        begin
        ...
        begin
          ...
          begin
            ...
            i := mysql_affected_rows(MainForm.MySQLConnectionHandler);
            if i = 1 then
              MainForm.LogThis('Изменение записи в таблице minsk.otdel произведено успешно',
                LogGroupGUID, lmtInfo)
            else
            begin
              s := 'Количество обработанных строк (' + IntToStr(i) +
        ...
        if not bError then
        begin
          q := 'UPDATE ' + MainForm.Configuration.sMySQLDatabase + '._phn SET ' + '_phn_phone="' +
            Trim(edbxPhoneNumber.Text) + '", ' + 'phn_priority="' + edbxPhonePriority.Text + '", ' +
            'phn_type="' + cmbbxPhoneType.Text + '", ' + 'phn_comments=' +
            NormalizeStringForQuery(mePhoneComments.Text, True, True) + ', ' + 'phn_compared="' +
            FormatDateTime('yyyy-mm-dd', dtpPhoneCompared.Date) + '", ' + 'phn_modified_at=NOW(), '
            + 'phn_modified_by_user_id=' + MainForm.CurrentUser.sID + ' ' + 'WHERE phn_id=' +
            IntToStr(i_phn_id) + ';';
          MainForm.LogThis(q, LogGroupGUID, lmtSQL);
          i := mysql_real_query(MainForm.MySQLConnectionHandler, PAnsiChar(AnsiString(q)),
            Length(q));
          ...
          begin
            ...
            i := mysql_affected_rows(MainForm.MySQLConnectionHandler);
            if i = 1 then
              MainForm.LogThis('Изменение записи в таблице _phn произведено успешно',
                LogGroupGUID, lmtInfo)
            else
            begin
              s := 'Количество обработанных строк (' + IntToStr(i) +
        ...
        begin
          MainForm.LogThis('Внесение записи в таблицу событий.', LogGroupGUID, lmtInfo);
          q := 'INSERT INTO ' + MainForm.Configuration.sMySQLDatabase +
            '._evn VALUES (NULL, NOW(),"MOD","phn",' + IntToStr(i_phn_id) + ',' +
            MainForm.CurrentUser.sID + ',' + #39 + q + #39 + ');';
          MainForm.LogThis(q, LogGroupGUID, lmtSQL);
          i := mysql_real_query(MainForm.MySQLConnectionHandler, PAnsiChar(AnsiString(q)),
            Length(q));
...
begin
  ...
  MainForm.ProcedureHeader
    ('Процедура реакции на нажатие кнопки отправки номера телефона на голосовую карту',
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          MainForm.LogThis
            ('Для отправки номера на голосовую карту была произведена передача во внешнюю программу следующей строки данных: ['
...
begin
  ...
  MainForm.ProcedureHeader
    ('Процедура реакции на нажатие кнопки соединения абонента А с абонентом Б', LogGroupGUID,
    True, True);
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          SendMessage(hTargetWnd, WM_COPYDATA, Longint(Handle), Longint(@aCopyData));
          MainForm.LogThis
            ('Для соединения абонента А с абонентом Б была произведена передача во внешнюю программу следующей строки данных: ['
            + sParameter + ']', LogGroupGUID, lmtInfo);
        end
        else
        begin
...
begin
  ...
  MainForm.ProcedureHeader('Процедура обновления состояния кнопок окна списка телефонов',
    LogGroupGUID, False, False);

  pnlModification.Visible := (lvPhones.Items.Count > 0) and (lvPhones.Selected <> nil);
  pnlID.Visible := MainForm.Configuration.bShowID and (lvPhones.Items.Count > 0) and
    (lvPhones.Selected <> nil);
  if lvPhones.Selected <> nil then
  begin
...
begin
  ...
  if (MainForm.PageControl1.ActivePage = MainForm.tsOrg) or
    (MainForm.PageControl1.ActivePage = MainForm.tsMsr) then
  begin
      ...
      begin
        ...
        begin
          ...
          q := 'SELECT otdel.phn_owner, ' + // тип телефона
            ...
            'IFNULL(_usr.usr_fullname,"неопределенный пользователь"), ' +
          // личный номер оператора выполнившего последнюю модификацию
            '"-", ' + // дата сверки
            'otdel.otdel_id, ' + // идентификатор записи
            'otdel.phone, ' + // старый номер телефона
            'otdel.sort_level ' + // приоритет
            'FROM minsk.otdel ' + 'LEFT JOIN minsk.firma ON firma.firm_code=otdel.firm_code ' +
            'LEFT JOIN ' + MainForm.Configuration.sMySQLDatabase +
            '._org ON minsk.otdel.firm_code=_org.org_firm_code ' + 'LEFT JOIN ' +
            MainForm.Configuration.sMySQLDatabase + '._phn ON _phn.phn_otdel_id=otdel.otdel_id ' +
            'LEFT JOIN ' + MainForm.Configuration.sMySQLDatabase +
            '._usr ON _usr.usr_id=otdel.oper ' +
            'WHERE ((!(_phn.phn_owner_is_measure="1")) OR ISNULL(_phn.phn_owner_is_measure)) AND _org.org_id='
            + IntToStr(owner_id) + ' ' + 'ORDER BY otdel.sort_level, otdel.phn_owner, otdel.phone;';
        end
        else
        begin
          s := 'Возникла ошибка в процессе получения ID организации-владельца!';
      ...
      begin
        ...
        begin
          q := 'SELECT phn_type, phone, phn_comments, date_format(phn_modified_at,"%d.%m.%Y %T"), '
            ...
            + 'FROM ' + MainForm.Configuration.sMySQLDatabase + '._phn ' +
            'LEFT JOIN minsk.otdel ON otdel_id=phn_otdel_id ' + 'LEFT JOIN ' +
            MainForm.Configuration.sMySQLDatabase + '._usr ON usr_id=phn_modified_by_user_id ' +
            'WHERE phn_owner_is_measure="1" AND phn_owner_id=' + IntToStr(owner_id) +
            ' ORDER BY phn_priority, phn_type, phone;'
        end
        else
        begin
        ...
        begin
          ...
          begin
            ...
            begin
              if not bError then
              begin
                MainForm.LogThis('Попытка получения очередной строки выборки (#' + IntToStr(j) +
                  ').', LogGroupGUID, lmtInfo);
                ResultRow := mysql_fetch_row(ResultSet);
                if ResultRow <> nil then
                begin
