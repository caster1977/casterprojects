...
begin
  ...
  with TCreateViewPostForm.Create(Owner) do
    try
      ...
      q := 'SELECT usr_fullname FROM ' + MainForm.Configuration.sMySQLDatabase +
        '._usr WHERE usr_erased="0" GROUP BY usr_fullname ORDER BY usr_fullname;';
      ...
      begin
        ...
        try
          ...
          begin
            ...
            begin
              MainForm.LogThis('Получение списка идентификаторов администраторов...',
                GroupGUID, lmtInfo);
              q := 'SELECT usr_id FROM ' + MainForm.Configuration.sMySQLDatabase +
                '._usr WHERE usr_erased="0" and usr_is_admin="1" GROUP BY usr_id ORDER BY usr_id;';
            end;
            if cmbbxTo.Text = '[ группа операторов ]' then
            begin
              MainForm.LogThis('Получение списка идентификаторов операторов...', GroupGUID,
                lmtInfo);
              q := 'SELECT usr_id FROM ' + MainForm.Configuration.sMySQLDatabase +
                '._usr WHERE usr_erased="0" and usr_can_edit_db="1" GROUP BY usr_id ORDER BY usr_id;';
            end;
            if cmbbxTo.Text = '[ группа пользователей ]' then
            begin
              MainForm.LogThis('Получение списка идентификаторов пользователей...',
                GroupGUID, lmtInfo);
              q := 'SELECT usr_id FROM ' + MainForm.Configuration.sMySQLDatabase +
                '._usr WHERE usr_erased="0" and usr_is_admin="0" GROUP BY usr_id ORDER BY usr_id;';
            end;
          end
          else
          begin
            MainForm.LogThis('Получение идентификатора указанного пользователя...',
              GroupGUID, lmtInfo);
            q := 'SELECT usr_id FROM ' + MainForm.Configuration.sMySQLDatabase +
              '._usr WHERE usr_fullname="' + cmbbxTo.Text + '" ORDER BY usr_id;';
          end;
          MainForm.LogThis(q, GroupGUID, lmtSQL);
              ...
              begin
                ...
                begin
                  ...
                  q := 'INSERT INTO ' + MainForm.Configuration.sMySQLDatabase +
                    ...
                    string(pacText) + '", Now(), ' + MainForm.CurrentUser.sID + ');';
                  MainForm.LogThis(q, GroupGUID, lmtSQL);
                  i := mysql_real_query(MainForm.MySQLConnectionHandler, PAnsiChar(AnsiString(q)),
                    Length(q));
                  if i = 0 then
                  begin
                    ...
                    begin
                      MainForm.LogThis('Количество строк выборки соответствует требуемому.',
                        GroupGUID, lmtInfo);
                      MainForm.LogThis('Сообщение успешно отправлено получателю.',
                        GroupGUID, lmtInfo);
                    end
                    else
                    begin
                  ...
                  begin
                    ...
                    begin
                      ...
                      MainForm.LogThis('Количество обработанных строк равно ' + IntToStr(i) + '.',
                        GroupGUID, lmtInfo);
                      if (i = 1) then
                        MainForm.LogThis('Запись в таблицу событий внесена успешно',
                          GroupGUID, lmtInfo)
                      else
                      begin
                        s := 'Количество обработанных строк (' + IntToStr(i) +
...
begin
  ...
  b := MessageBox(Handle, PChar('Вы действительно хотите удалить выделенное сообщение?'),
    PChar('OA4 - Подтверждение удаления'), MB_OKCANCEL + MB_ICONQUESTION + MB_DEFBUTTON2) = IDOK;
  if b then
  begin
    Screen.Cursor := crHourGlass;
    ...
    q := 'UPDATE ' + MainForm.Configuration.sMySQLDatabase +
      '._msg SET msg_erased="1" WHERE msg_id=' + s_msg_id + ';';
    MainForm.LogThis(q, GroupGUID, lmtSQL);
    i := mysql_real_query(MainForm.MySQLConnectionHandler, PAnsiChar(AnsiString(q)), Length(q));
    if i = 0 then
    begin
    ...
    begin
      MainForm.LogThis('Внесение записи в таблицу событий.', GroupGUID, lmtInfo);
      q := 'INSERT INTO ' + MainForm.Configuration.sMySQLDatabase +
        '._evn VALUES (NULL, NOW(),"DEL","msg",' + s_msg_id + ',' + MainForm.CurrentUser.sID + ',' +
        #39 + q + #39 + ');';
      MainForm.LogThis(q, GroupGUID, lmtSQL);
      i := mysql_real_query(MainForm.MySQLConnectionHandler, PAnsiChar(AnsiString(q)), Length(q));
      if i = 0 then
...
begin
  ...
  with TCreateViewPostForm.Create(Owner) do
    try
      ...
      q := 'SELECT msg_id, msg_processed, msg_caption, msg_text, msg_created_at, usr_fullname FROM '
        + MainForm.Configuration.sMySQLDatabase + '._msg LEFT JOIN ' +
      ...
      begin
        ...
        begin
          MainForm.LogThis
            ('Обновление списка сообщений после успешного удаления открытого сообщения.',
            GroupGUID, lmtInfo);
          lvMessages.Selected.Delete;
          btnView.Enabled := (lvMessages.Items.Count > 0) and (lvMessages.ItemIndex >= 0);
          btnErase.Enabled := (lvMessages.Items.Count > 0) and (lvMessages.ItemIndex >= 0);
        ...
        begin
          MainForm.LogThis
            ('Обновление списка сообщений после успешной пометки открытого сообщения как прочитанного.',
            GroupGUID, lmtInfo);
...
procedure TViewPostListForm.lvMessagesSelectItem(Sender: TObject; Item: TListItem;
  Selected: Boolean);
begin
  btnView.Enabled := (lvMessages.Items.Count > 0) and (lvMessages.ItemIndex >= 0);
  btnErase.Enabled := (lvMessages.Items.Count > 0) and (lvMessages.ItemIndex >= 0);
  if lvMessages.ItemIndex >= 0 then
    ...
    lblID.Caption := '';
  pnlID.Visible := MainForm.Configuration.bShowID and (lvMessages.Items.Count > 0) and
