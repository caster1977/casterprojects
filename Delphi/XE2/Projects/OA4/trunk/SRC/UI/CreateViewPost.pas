...
begin
  ...
  b := MessageBox(Handle, PChar('Вы действительно хотите удалить текущее сообщение?'),
...
begin
  ...
  q := 'UPDATE ' + MainForm.Configuration.sMySQLDatabase +
    '._msg SET msg_processed="1" WHERE msg_id=' + s + ';';
  MainForm.LogThis(q, GroupGUID, lmtSQL);
  i := mysql_real_query(MainForm.MySQLConnectionHandler, PAnsiChar(AnsiString(q)), Length(q));
  if i = 0 then
  begin
    ...
    begin
      ...
      MainForm.LogThis('Количество обработанных строк соответствует требуемому (1).',
        GroupGUID, lmtInfo);
      MainForm.LogThis('Сообщение успешно удалено.', GroupGUID, lmtInfo);
      b1 := True;
      MainForm.ProgressBar1.StepIt;
...
begin
  btnSend.Enabled := ((edbxCaption.Text <> '') and (cmbbxTo.ItemIndex > -1) and
    (meMessage.Text <> ''));
  ...
  begin
    btnSend.Hint :=
      'Вы должны заполнить поля "Тема сообщения", "Адресат" и "Текст сообщения" перед отправкой сообщения';
