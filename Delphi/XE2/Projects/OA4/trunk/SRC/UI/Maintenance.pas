...
begin
  ...
  b := MessageBox(Handle, PChar('ОСТОРОЖНО! При нажатии кнопки "Да" текущие данные, ' +
    'содержащиеся в БД будут безвозвратно утеряны! Вы уверены в том что хотите удалить ' +
    'все имеющиеся на текущий момент данные и импортировать содержащиеся в "старой" базе?'),
    PChar('OA4 - Подтверждение импорта базы данных'), MB_YESNO + MB_ICONEXCLAMATION +
    MB_DEFBUTTON2) = IDYES;
  if not b then
  begin
  ...
  begin
    ...
    begin
      s := 'Возникла ошибка при попытке удаления таблицы организаций ' +
        MainForm.Configuration.sMySQLDatabase + '._org!';
      bError := True;
    end;
    if bError then
    begin
  ...
  begin
    sbMaintenance.SimpleText :=
      'Попытка очистки таблицы фирм от ранее импортированных записей об организациях...';
    pbImportDetail.Position := pbImportDetail.Min;
    pbImportDetail.Max := 1;
    Application.ProcessMessages;
  ...
  begin
    sbMaintenance.SimpleText :=
      'Попытка очистки таблицы отделов от ранее внесённых записей о телефонах...';
    pbImportDetail.Position := pbImportDetail.Min;
    pbImportDetail.Max := 1;
    Application.ProcessMessages;
  ...
  begin
    sbMaintenance.SimpleText :=
      'Попытка очистки таблицы групп от ранее импортированныз типов организаций...';
    pbImportDetail.Position := pbImportDetail.Min;
    pbImportDetail.Max := 1;
    Application.ProcessMessages;
    ...
    begin
      ...
      if i >= 0 then
      begin
        MainForm.LogThis('Удаление ранее импортированных групп произведено успешно.',
          GroupGUID, lmtInfo)
      end
      else
      begin
  ...
  begin
    ...
    pbImportDetail.Max := 1;
    Application.ProcessMessages;
    q := 'CREATE TABLE ' + MainForm.Configuration.sMySQLDatabase + '._org (' +
      'org_id smallint(5) unsigned NOT NULL auto_increment, ' +
      'org_erased enum("0","1") NOT NULL default "0", ' + '_org_type text, ' + '_org_name text, ' +
      '_org_address text, ' + 'org_firm_code int(10) unsigned default NULL, ' +
      'org_how_to_reach text, ' + 'org_work_time text, ' + 'org_other_info mediumtext, ' +
      ...
      'PRIMARY KEY  (org_id), ' + 'KEY org_erased (org_erased), ' +
      ...
      'KEY org_firm_code (org_firm_code), ' + 'FULLTEXT KEY org_work_time (org_work_time) ' +
  ...
  begin
    ...
    pbImportDetail.Max := 1;
    Application.ProcessMessages;
    q := 'CREATE TABLE ' + MainForm.Configuration.sMySQLDatabase + '._msr (' +
      'msr_id smallint(5) unsigned NOT NULL auto_increment, ' +
      'msr_erased enum("0","1") NOT NULL default "0", ' +
      'msr_start_datetime datetime default NULL, ' + 'msr_stop_datetime datetime default NULL, ' +
      'msr_unknown_duration enum("0","1") NOT NULL default "1", ' +
      ...
      'msr_for_adults_only enum("0","1") NOT NULL default "0", ' + 'msr_type text, ' +
      'msr_name text, ' + 'msr_author text, ' + 'msr_producer text, ' + 'msr_performer text, ' +
      'msr_organizer text, ' + 'msr_ticket_price text, ' + 'msr_other_info mediumtext, ' +
      ...
      'PRIMARY KEY  (msr_id), ' + 'KEY msr_erased (msr_erased), ' +
      'KEY msr_organization_id (msr_organization_id), ' + 'KEY msr_created_at (msr_created_at), ' +
      ...
      'KEY msr_is_premier (msr_is_premier), ' + 'KEY msr_is_tour (msr_is_tour), ' +
      'KEY msr_for_children (msr_for_children), ' + 'KEY msr_for_teenagers (msr_for_teenagers), ' +
      ...
      'KEY msr_unknown_duration (msr_unknown_duration), ' + 'FULLTEXT KEY msr_type (msr_type), ' +
      'FULLTEXT KEY msr_name (msr_name), ' + 'FULLTEXT KEY msr_author (msr_author), ' +
      'FULLTEXT KEY msr_producer (msr_producer), ' + 'FULLTEXT KEY msr_performer (msr_performer), '
      + 'FULLTEXT KEY msr_organizer (msr_organizer) ' +
  ...
  begin
    ...
    pbImportDetail.Max := 1;
    Application.ProcessMessages;
    q := 'CREATE TABLE ' + MainForm.Configuration.sMySQLDatabase + '._phn (' +
      'phn_id int(10) unsigned NOT NULL auto_increment, ' +
      'phn_priority tinyint(3) unsigned NOT NULL default "0", ' + '_phn_phone tinytext NOT NULL, ' +
      'phn_otdel_id int(10) unsigned default NULL, ' +
      'phn_type enum("", "стационарный", "мобильный", "мобильный (Velcom/МЦС)", ' +
      '"мобильный (МТС)", "мобильный (Diallog/БелСел)", "мобильный (БеСТ)") NOT NULL default "", ' +
      'phn_comments text, ' + 'phn_compared date NOT NULL default "1900-01-01", ' +
      ...
      'phn_modified_by_user_id tinyint(3) unsigned default NULL, ' + 'PRIMARY KEY  (phn_id), ' +
      'KEY phn_priority (phn_priority), ' + 'KEY phn_owner_is_measure (phn_owner_is_measure), ' +
      'KEY phn_owner_id (phn_owner_id), ' + 'KEY phn_otdel_id (phn_otdel_id), ' +
      'KEY phn_type (phn_type), ' + 'KEY phn_compared (phn_compared), ' +
      'KEY phn_modified_at (phn_modified_at), ' +
      'KEY phn_modified_by_user_id (phn_modified_by_user_id), ' +
      'FULLTEXT KEY _phn_phone (_phn_phone) ' +
  ...
  begin
    ...
    pbImportDetail.Max := 1;
    Application.ProcessMessages;
    q := 'CREATE TABLE ' + MainForm.Configuration.sMySQLDatabase + '._evn (' +
      'evn_id int(10) unsigned NOT NULL auto_increment, ' +
      'evn_datetime datetime NOT NULL default "0000-00-00 00:00:00", ' +
      'evn_type enum("INS","MOD","DEL","REC") default NULL, ' +
      'evn_object enum("usr","phn","org","msr","msg") default NULL, ' +
      'evn_object_id int(10) unsigned default NULL, ' +
      'evn_by_user_id tinyint(3) unsigned NOT NULL default "0", ' + 'evn_details text NOT NULL, ' +
      'PRIMARY KEY  (evn_id), ' + 'KEY evn_datetime (evn_datetime), ' + 'KEY evn_type (evn_type), '
      + 'KEY evn_by_user_id (evn_by_user_id), ' + 'KEY evn_object (evn_object), ' +
      'KEY evn_object_id (evn_object_id)' +
      ') TYPE=MyISAM COMMENT="Таблица событий для версии 4.0";';
    MainForm.LogThis(q, GroupGUID, lmtSQL);
    i := mysql_real_query(MainForm.MySQLConnectionHandler, PAnsiChar(AnsiString(q)), Length(q));
    if i = 0 then
    begin
  ...
  begin
    sbMaintenance.SimpleText :=
      'Попытка подготовки таблицы организаций старого формата к экспорту данных...';
    pbImportDetail.Position := pbImportDetail.Min;
    pbImportDetail.Max := 1;
    Application.ProcessMessages;
    ...
    begin
      MainForm.LogThis
        ('Подготовка таблицы организаций старого формата к экспорту данных произведена успешно.',
        GroupGUID, lmtInfo);
    end
    else
    begin
  ...
  begin
    sbMaintenance.SimpleText :=
      'Попытка подготовки таблицы мероприятий старого формата к экспорту данных...';
    pbImportDetail.Position := pbImportDetail.Min;
    pbImportDetail.Max := 1;
    Application.ProcessMessages;
    ...
    begin
      MainForm.LogThis
        ('Подготовка таблицы мероприятий старого формата к экспорту данных произведена успешно.',
        GroupGUID, lmtInfo);
    end
    else
    begin
  ...
  begin
    ...
    pbImportDetail.Max := 1;
    Application.ProcessMessages;
    q := 'INSERT INTO ' + MainForm.Configuration.sMySQLDatabase + '._org ' + 'SELECT a.id, ' +
    // org_id
      '"0", ' + // org_erased
      ...
      '"", ' + // org_work_time
      'CONCAT("<B> ",a.organizationphones," </B><BR><BR> ",a.organizationinfo), ' +
    // org_other_info
    ...
    begin
      MainForm.LogThis('Запрос выполнен успешно.', GroupGUID, lmtInfo);
      i := mysql_affected_rows(MainForm.MySQLConnectionHandler);
      MainForm.LogThis('Количество импортированных строк равно ' + IntToStr(i) + '.',
        GroupGUID, lmtInfo);
      if i > 0 then
      begin
        MainForm.LogThis('Импортирование данных об организациях произведено успешно.',
          GroupGUID, lmtInfo)
      end
      else
      begin
  ...
  begin
    sbMaintenance.SimpleText :=
      'Попытка удаления "пустой" организации из таблицы нового формата...';
    pbImportDetail.Position := pbImportDetail.Min;
    pbImportDetail.Max := 1;
    Application.ProcessMessages;
  ...
  begin
    ...
    pbImportDetail.Max := 1;
    Application.ProcessMessages;
    q := 'INSERT INTO ' + MainForm.Configuration.sMySQLDatabase +
      '._org VALUES(25000, "0", "", "", "", NULL, "", "", "", NULL, "00000000000000", "6", "00000000000000", "6");';
    MainForm.LogThis(q, GroupGUID, lmtSQL);
    i := mysql_real_query(MainForm.MySQLConnectionHandler, PAnsiChar(AnsiString(q)), Length(q));
    if i = 0 then
    begin
      MainForm.LogThis('Запрос выполнен успешно.', GroupGUID, lmtInfo);
      i := mysql_affected_rows(MainForm.MySQLConnectionHandler);
      MainForm.LogThis('Количество вставленых строк равно ' + IntToStr(i) + '.', GroupGUID,
        lmtInfo);
      if i = 1 then
      begin
  ...
  // создание списка для помещения туда строк SQL-запросов получаемых в результате выполнения запроса
  try
    if not bError then
    begin
      ...
      pbImportDetail.Position := pbImportDetail.Min;
      Application.ProcessMessages;
      q := 'SELECT CONCAT("INSERT INTO minsk.firma(name,main,city_id,street_id,comment,ext_comment) VALUES ('
        + #39 + ' ",TRIM(_org_name)," | ",TRIM(_org_type),' + '"' + #39 + ',' + #39 + '!R' + #39 +
        ',2507,1077,' + #39 + '",org_id,"' + #39 + ',' + #39 + '",_org_address,"' + #39 +
      ...
      begin
        ...
        begin
          ...
          begin
            MainForm.LogThis('Запрос выполнен успешно.', GroupGUID, lmtInfo);
            i := mysql_affected_rows(MainForm.MySQLConnectionHandler);
            MainForm.LogThis('Количество вставленых строк равно ' + IntToStr(i) + '.',
              GroupGUID, lmtInfo);
            if i = 1 then
            begin
              MainForm.LogThis('Вставка записи в таблицу фирм произведена успешно.',
                GroupGUID, lmtInfo)
            end
            else
            begin
              s := 'Количество вставленных строк (' + IntToStr(i) +
  ...
  try
    ...
    begin
      sbMaintenance.SimpleText :=
        'Попытка создания текста запросов на обновление данных таблицы организаций...';
      pbImportDetail.Position := pbImportDetail.Min;
      Application.ProcessMessages;
      q := 'SELECT CONCAT("UPDATE rne4._org SET org_firm_code=",m.firm_code," ' +
        'WHERE org_id=",m.comment,";") FROM minsk.firma m LEFT JOIN ' +
        MainForm.Configuration.sMySQLDatabase + '._org o ON m.comment=o.org_id ' +
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            if i = 1 then
            begin
              MainForm.LogThis('Обновление записи таблицы организаций произведена успешно.',
                GroupGUID, lmtInfo)
            end
            else
            begin
  ...
  begin
    sbMaintenance.SimpleText :=
      'Попытка импортирования словаря типов организации в таблицу групп...';
    pbImportDetail.Position := pbImportDetail.Min;
    pbImportDetail.Max := 1;
    Application.ProcessMessages;
    q := 'INSERT INTO minsk.grp SELECT "0", UCASE(CONCAT(" ",TRIM(_org_type))) AS name,' +
      '"333","00000000000000" FROM ' + MainForm.Configuration.sMySQLDatabase + '._org ' +
      'LEFT JOIN minsk.grp ON UCASE(name)=UCASE(CONCAT(" ",TRIM(_org_type))) ' +
    ...
    begin
      ...
      if i >= 0 then
      begin
        MainForm.LogThis('Вставка словаря типов организаций в таблицу групп произведена успешно.',
          GroupGUID, lmtInfo)
      end
      else
      begin
  ...
  begin
    ...
    pbImportDetail.Max := 1;
    Application.ProcessMessages;
    q := 'INSERT INTO ' + MainForm.Configuration.sMySQLDatabase + '._msr ' + 'SELECT ' + 'NULL, ' +
      '"0", ' + 'a.measurestartdatetime, ' + 'a.measurestopdatetime, ' + '"1", ' +
      'a.measureduration, ' + 'a.organizationnameid, ' + 'IF(a.measurepremier=1,"1","0"), ' +
      '"0", ' + 'IF(a.childrenmeasure=1,"1","0"), ' + 'IF(a.teenagersmeasure,"1","0"), ' + '"0", ' +
      'REPLACE(REPLACE(REPLACE(b.measuretype,"<B>",""),"</B>",""),"  "," "), ' +
      'REPLACE(REPLACE(REPLACE(c.measurename,"<B>",""),"</B>",""),"  "," "), ' + 'd.measureauthor, '
      + 'e.measureproducer, ' + 'a.measureperformer, ' + 'a.measureorganizer, ' +
      'a.measureticketprice, ' + 'a.measureinfo, ' + 'NULL, ' + '"00000000000000", ' +
      'a.ridcreatorid, ' + 'a.LMTS, ' + 'a.ridcreatorid ' + 'FROM rne2.measures a ' +
      'LEFT JOIN rne2.measuretypes b ON b.id=a.measuretypeid ' +
      'LEFT JOIN rne2.measurenames c ON c.id=a.measurenameid ' +
      'LEFT JOIN rne2.measureauthors d ON d.id=a.measureauthorid ' +
      'LEFT JOIN rne2.measureproducers e ON e.id=a.measureproducerid ' + 'ORDER BY a.id; ';
    MainForm.LogThis(q, GroupGUID, lmtSQL);
    i := mysql_real_query(MainForm.MySQLConnectionHandler, PAnsiChar(AnsiString(q)), Length(q));
    if i = 0 then
    begin
      ...
      if i > 0 then
      begin
        MainForm.LogThis('Импортирование данных о мероприятиях произведено успешно.',
          GroupGUID, lmtInfo)
      end
      else
      begin
  ...
  begin
    ...
    pbImportDetail.Max := 1;
    Application.ProcessMessages;
    q := 'UPDATE ' + MainForm.Configuration.sMySQLDatabase +
      '._msr SET msr_organization_id=25000 WHERE msr_organization_id=0;';
    MainForm.LogThis(q, GroupGUID, lmtSQL);
    i := mysql_real_query(MainForm.MySQLConnectionHandler, PAnsiChar(AnsiString(q)), Length(q));
    if i = 0 then
    begin
  ...
  begin
    ...
    pbImportDetail.Max := 1;
    Application.ProcessMessages;
    q := 'DELETE FROM rne4._msr WHERE (msr_start_datetime="" OR ISNULL(msr_start_datetime)) AND ' +
      ...
      '(msr_type="" OR ISNULL(msr_type)) AND ' + '(msr_name="" OR ISNULL(msr_name)) AND ' +
    ...
    begin
      ...
      if i >= 0 then
      begin
        MainForm.LogThis('Очистка таблицы мероприятий от пустых мероприятий произведена успешно.',
          GroupGUID, lmtInfo)
      end
      else
      begin
  ...
  begin
    ...
    Screen.Cursor := crDefault;
    Application.ProcessMessages;
    MessageBox(Handle, PChar('Опериция успешно завершена!'), PChar('OA4 - Информация'),
