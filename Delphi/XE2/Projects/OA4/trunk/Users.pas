...
uses Windows, Messages, SysUtils, Variants, Classes, Graphics, Controls, Forms, Dialogs, StdCtrls, ComCtrls, ExtCtrls, CommCtrl, Main, mysql, ImgList;
...
procedure TUsersForm.FormMouseWheelDown(Sender: TObject; Shift: TShiftState; MousePos: TPoint; var Handled: Boolean);
begin
  SendMessage(TreeView1.Handle, WM_VSCROLL, SB_LINEDOWN, 0);
end;

procedure TUsersForm.FormMouseWheelUp(Sender: TObject; Shift: TShiftState; MousePos: TPoint; var Handled: Boolean);
begin
...
begin
  ...
  if bUserNode then
    begin
      if Node.Text<>'<НОВЫЙ ПОЛЬЗОВАТЕЛЬ>' then
        begin
          ...
          q:='SELECT a.usr_id, a.usr_login, a.usr_fullname, '+'a.usr_can_edit_db, a.usr_can_backup_db, a.usr_can_clear_db, a.usr_can_defrag_db, '+'a.usr_can_maintain_db, a.usr_can_edit_db_users, b.usr_fullname, '+
            'date_format(a.usr_created_at,"%d.%m.%Y %T"), '+'f.usr_fullname, date_format(a.usr_modified_at,"%d.%m.%Y %T") '+'FROM '+MainForm.Configuration.sMySQLDatabase+'._usr a '+'LEFT JOIN '+MainForm.Configuration.sMySQLDatabase+
            '._usr b ON b.usr_id=a.usr_created_by_user_id '+'LEFT JOIN '+MainForm.Configuration.sMySQLDatabase+'._usr f ON f.usr_id=a.usr_modified_by_user_id '+'WHERE a.usr_login="'+Node.Text+'";';
          MainForm.LogThis(q, GroupGUID, lmtSQL);
          i:=mysql_real_query(MainForm.MySQLConnectionHandler, PAnsiChar(AnsiString(q)), Length(q));
          if i=0 then
            begin
                    ...
                    begin
                      ...
                      if ResultRow<>nil then
                        begin
                          ...
                          lblModificationDetails.Caption:=string(ResultRow[11])+', '+string(ResultRow[12]);
                        end
                      else
                        begin
    ...
    lblHint.Caption:='Нажмите кнопку "Добавить" для добавления нового пользователя в группу "'+Node.Text+'".';
  bUserModified:=False;
  UpdateButtons;
end;
...
begin
  ...
  if btnCancelAddingUser.Visible then
    begin
      ...
      q:='INSERT INTO '+MainForm.Configuration.sMySQLDatabase+'._usr (usr_id, usr_erased, usr_login, usr_password_md5, '+'usr_fullname, usr_is_admin, usr_can_edit_db, usr_can_backup_db, '+'usr_can_clear_db, usr_can_defrag_db, usr_can_maintain_db, '+
        'usr_can_edit_db_users, usr_created_at, usr_created_by_user_id, '+'usr_modified_at, usr_modified_by_user_id) VALUES '+'(NULL, "'+IntToStr(integer(Node.ImageIndex=1))+'", "'+edbxLogin.Text+'", md5(""), "'+edbxName.Text+'", "'+IntToStr
        (integer(Node.Parent.Text='Администраторы'))+'", "'+IntToStr(integer(chkbxDBEdition.Checked))+'", "'+IntToStr(integer(chkbxDBBackup.Checked and chkbxDBBackup.Enabled))+'", "'+IntToStr
        (integer(chkbxDBCleaning.Checked and chkbxDBCleaning.Enabled))+'", "'+IntToStr(integer(chkbxDBDefragmentation.Checked and chkbxDBDefragmentation.Enabled))+'", "'+IntToStr(integer(chkbxDBMaintenance.Checked and chkbxDBMaintenance.Enabled))
        +'", "'+IntToStr(integer(chkbxDBUsersEdition.Checked and chkbxDBUsersEdition.Enabled))+'", Now(), '+MainForm.CurrentUser.sID+', Now(), '+MainForm.CurrentUser.sID+')';
      MainForm.LogThis(q, GroupGUID, lmtSQL);
      i:=mysql_real_query(MainForm.MySQLConnectionHandler, PAnsiChar(AnsiString(q)), Length(q));
      if i=0 then
        begin
    ...
    begin
      ...
      q:='UPDATE '+MainForm.Configuration.sMySQLDatabase+'._usr SET '+'usr_login="'+edbxLogin.Text+'", '+'usr_erased="'+IntToStr(integer(Node.ImageIndex=1))+'", '+'usr_fullname="'+edbxName.Text+'", '+'usr_is_admin="'+IntToStr
        (integer(Node.Parent.Text='Администраторы'))+'", '+'usr_can_edit_db="'+IntToStr(integer(chkbxDBEdition.Checked))+'", '+'usr_can_backup_db="'+IntToStr(integer(chkbxDBBackup.Checked and chkbxDBBackup.Enabled))
        +'", '+'usr_can_clear_db="'+IntToStr(integer(chkbxDBCleaning.Checked and chkbxDBCleaning.Enabled))+'", '+'usr_can_defrag_db="'+IntToStr(integer(chkbxDBDefragmentation.Checked and chkbxDBDefragmentation.Enabled))
        +'", '+'usr_can_maintain_db="'+IntToStr(integer(chkbxDBMaintenance.Checked and chkbxDBMaintenance.Enabled))+'", '+'usr_can_edit_db_users="'+IntToStr(integer(chkbxDBUsersEdition.Checked and chkbxDBUsersEdition.Enabled))
        +'", '+'usr_modified_at=Now(), '+'usr_modified_by_user_id="'+MainForm.CurrentUser.sID+'" '+'WHERE usr_id="'+edbxID.Text+'";';
      MainForm.LogThis(q, GroupGUID, lmtSQL);
      i:=mysql_real_query(MainForm.MySQLConnectionHandler, PAnsiChar(AnsiString(q)), Length(q));
      if i=0 then
        begin
...
begin
  ...
  for i:=0 to TreeView1.Items.Count-1 do
    if (TreeView1.Items.Item[i].Text=edbxLogin.Text)and(TreeView1.Items.Item[i]<>TreeView1.Selected) then
      b:=True;
  btnSaveUser.Enabled:=bUserModified and(edbxLogin.Text<>'Администраторы')and(edbxLogin.Text<>'Пользователи')and(edbxLogin.Text<>'')and(edbxLogin.Text<>'<НОВЫЙ ПОЛЬЗОВАТЕЛЬ>')and(not b);
