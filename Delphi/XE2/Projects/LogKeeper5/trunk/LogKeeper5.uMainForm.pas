...
begin
  Log.SendDebug('Установлен режим "' + IfThen(iBusyCounter > 0, 'Занято', 'Готово') + '".');
  with MainForm do
  begin
    ...
    StatusBar1.Panels[STATUSBAR_HINT_PANEL_NUMBER].Text :=
      IfThen(iBusyCounter > 0, 'Пожалуйста, подождите...', 'Готово');
  end;
  Application.ProcessMessages;
end;
...
begin
  ...
  begin
    ...
    Handled := True;
    MessageBox(Handle, PWideChar('Получено сообщение msgLogKeeperClientQuery в форму ' + name +
      '!'), PWideChar('LogKeeper - Информация!'), MB_OK + MB_ICONINFORMATION + MB_DEFBUTTON1);
  end;
end;
...
begin
  ProcedureHeader('Процедура-обработчик события отображения окна',
    '{9F96D348-2423-4E7C-94F0-833BA86269F3}');
