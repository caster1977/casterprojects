...
type
  ...
  TMainForm = class(TForm)
    ...
    procedure lvLogCustomDrawSubItem(Sender: TCustomListView; Item: TListItem; SubItem: Integer; State: TCustomDrawState;
      var DefaultDraw: Boolean);
    ...
    procedure lvActionListMouseWheel(Sender: TObject; Shift: TShiftState; WheelDelta: Integer; MousePos: TPoint;
      var Handled: Boolean);
  ...
  private
    ...
    function Do_SIC_01(const aStartDate, aStopDate: TDate; const aOpenWithBrowser: Boolean; const aPhonesList: TStringList;
      const aShowUnserved: Boolean): Boolean;
    function Do_SIC_03(const aStartDate, aStopDate: TDate; const aOpenWithBrowser: Boolean;
      const aNetIndex: Integer): Boolean;
    function Do_SIC_04(const aStartDate, aStopDate: TDate; const aOpenWithBrowser: Boolean;
      const aGroupByPeriod: TGroupByPeriod; const aCalculateWeekFromMonday, aAllUsers, aCreateDetailedReport,
      aShowSQLQueries, aShowIdleUsers: Boolean; const aSelectedUserList: TStringList): Boolean;
    function Do_SIC_05(const aStartDate, aStopDate: TDate; const aOpenWithBrowser: Boolean;
      const aGroupByPeriod: TGroupByPeriod; const aServiceIndex: Integer; const aCalculateWeekFromMonday: Boolean): Boolean;
    function Do_SIC_06(const aStartDate, aStopDate: TDate; const aOpenWithBrowser: Boolean;
      const aServiceIndex: Integer): Boolean;
    function Do_SIC_07(const aStartDate, aStopDate: TDate; const aOpenWithBrowser: Boolean;
      const aGroupByPeriod: TGroupByPeriod; const aCalculateWeekFromMonday: Boolean): Boolean;
    function Do_SIC_08(const aStartDate, aStopDate: TDate; const aOpenWithBrowser: Boolean): Boolean;
    function Do_SIC_09(const aStartDate, aStopDate: TDate; const aOpenWithBrowser: Boolean; const aNetIndex: Integer;
      const aSummaryReport, aOldReport: Boolean; const aSelectedServicesList: TStringList): Boolean;
    function Do_SIC_10(const aStartDate, aStopDate: TDate; const aOpenWithBrowser: Boolean;
      const aGroupByPeriod: TGroupByPeriod; const aCalculateWeekFromMonday, aAllUsers, aCreateDetailedReport: Boolean;
      const aCCID: Integer; const aSelectedUserList: TStringList): Boolean;
  ...
  public
    ...
    function Do_CreateXRDA(const aStartDate, aStopDate: TDate; const aSilentYes: Boolean; const aXRD: tXRD;
      var aTableName: string; const bRecreateXRD: Boolean; const bCreateXRDA, bRecreateXRDA: Boolean): Boolean;
    procedure ShowErrorBox(const aHandle: HWND; const aErrorMessage: string; const aLogGroupGUID: string);
    function Do_SaveReportAsHTMLAndOpen(const aHTML, aFileName: string; const aOpenWithBrowser, aSilentYes: Boolean)
      : Boolean;
    ...
    procedure MySQL_UnlockTables(const aLogGroupGUID: string;
      const aTMySQLServerConnectionDetails: TMySQLServerConnectionDetails; var bError: Boolean; var sErrorMessage: string);
    procedure MySQL_CloseConnection(const aLogGroupGUID: string;
      var aTMySQLServerConnectionDetails: TMySQLServerConnectionDetails; var bError: Boolean; var sErrorMessage: string);
    procedure MySQL_LockTables(const aLogGroupGUID: string;
      const aTMySQLServerConnectionDetails: TMySQLServerConnectionDetails; var bError: Boolean; var sErrorMessage: string;
      const aTablesList: string);
    procedure MySQL_OpenConnection(const aLogGroupGUID: string;
      var aTMySQLServerConnectionDetails: TMySQLServerConnectionDetails; var bError: Boolean; var sErrorMessage: string);
    procedure MySQL_CheckTableExistance(const aLogGroupGUID: string;
      const aTMySQLServerConnectionDetails: TMySQLServerConnectionDetails; const aMySQLDatabase: string; var bError: Boolean;
      var sErrorMessage: string; const aTableName: string);
    procedure MySQL_CreateTable(const aLogGroupGUID: string;
      const aTMySQLServerConnectionDetails: TMySQLServerConnectionDetails; var bError: Boolean; var sErrorMessage: string;
      const aQuery: string);
    procedure MySQL_DropTable(const aLogGroupGUID: string;
      const aTMySQLServerConnectionDetails: TMySQLServerConnectionDetails; var bError: Boolean; var sErrorMessage: string;
      const aQuery: string);
    function MySQL_InsertRecords(const aLogGroupGUID: string;
      const aTMySQLServerConnectionDetails: TMySQLServerConnectionDetails; var bError: Boolean; var sErrorMessage: string;
      const aQuery: string): Integer;
    function MySQL_UpdateRecords(const aLogGroupGUID: string;
      const aTMySQLServerConnectionDetails: TMySQLServerConnectionDetails; var bError: Boolean; var sErrorMessage: string;
      const aQuery: string): Integer;
  end;
...
begin
  ...
  begin
    if (((Configuration.bKeepErrorLog) and (aMessageType = lmtError)) or
      ((Configuration.bKeepWarningLog) and (aMessageType = lmtWarning)) or
      ((Configuration.bKeepInfoLog) and (aMessageType = lmtInfo)) or ((Configuration.bKeepSQLLog) and (aMessageType = lmtSQL)
      ) or ((Configuration.bKeepDebugLog) and (aMessageType = lmtDebug))) then
    begin
      case aMessageType of
        lmtError:
          i := ICON_ERROR;
  ...
  with lvLog do
    if (Items.Count > 0) and (Selected <> nil) then
      ...
      begin
        ...
        Routines_CopyStringToClipboard(s + #09 + Caption + #09 + SubItems[0] + #09 + SubItems[1] + #09 + SubItems[2] + #09 +
          SubItems[3]);
      end;
...
begin
  ProcedureHeader('Процедура копирования текста SQL-запроса выделенного элемента списка сообщений протокола в буфер обмена',
    LogGroupGUID);
...
begin
  ...
  LogThis('Флажок "' + cbCreateXRDA.Caption + '" ' + Routines_GetConditionalMessage(cbCreateXRDA.Checked, 'включён',
    'отключён') + '.', LogGroupGUID, lmtDebug);
...
begin
  ...
  LogThis('Флажок "' + cbCustomNameOfXRDA.Caption + '" ' + Routines_GetConditionalMessage(cbCustomNameOfXRDA.Checked,
    'включён', 'отключён') + '.', LogGroupGUID, lmtDebug);
...
begin
  ...
  if not(cbRecreateXRDA.Checked or cbCreateXRDA.Checked) then
    cbCustomNameOfXRDA.Checked := False;
  LogThis('Флажок "' + cbRecreateXRDA.Caption + '" ' + Routines_GetConditionalMessage(cbRecreateXRDA.Checked, 'включён',
    'отключён') + '.', LogGroupGUID, lmtDebug);
  ...
  for i := 0 to lvActionList.Items.Count - 1 do
    if lvActionList.Items[i] <> nil then
      ...
      begin
        ...
        TProgressBar(lvActionList.Items[i].Data).Visible :=
          (BOOL(ListView_IsItemVisible(lvActionList.Handle, lvActionList.Items[i].index)) = True) and
          (i >= ListView_GetTopIndex(lvActionList.Handle));
      end;
end;
...
procedure TMainForm.lvActionListMouseWheel(Sender: TObject; Shift: TShiftState; WheelDelta: Integer; MousePos: TPoint;
  var Handled: Boolean);
begin
  Update_ProgressBarsInActionsListView;
end;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          MessageBox(Handle, PWideChar('Строк с GUID ' + s + ' ниже указанной в списке протокола более не найдено!'),
            PWideChar('LogKeeper - Внимание!'), MB_OK + MB_ICONWARNING + MB_DEFBUTTON1);
          Application.ProcessMessages;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          MessageBox(Handle, PWideChar('Строк с GUID ' + s + ' выше указанной в списке протокола более не найдено!'),
            PWideChar('LogKeeper - Внимание!'), MB_OK + MB_ICONWARNING + MB_DEFBUTTON1);
          Application.ProcessMessages;
...
begin
  ...
  if Configuration.bShowQuitConfirmation then
    CanClose := MessageBox(Handle, PWideChar('Вы действительно хотите завершить работу программы?'),
      PWideChar('OVERSEER - Подтверждение выхода'), MB_OKCANCEL + MB_ICONQUESTION + MB_DEFBUTTON2) = IDOK
  else
    CanClose := True;
  Application.ProcessMessages;
...
begin
  ...
  with TIniFile.Create(ChangeFileExt(ExpandFileName(Application.ExeName), '.ini')) do
    try
      ...
      Configuration.sLastLogin := Routines_GetConditionalMessage(Configuration.bStoreLastLogin,
        ReadString('Интерфейс', 'sLastLogin', ''), '');
      Configuration.bStoreLastPassword := ReadBool('Интерфейс', 'bStoreLastPassword', False);
      Configuration.sLastPassword := Routines_GetConditionalMessage(Configuration.bStoreLastPassword,
        ReadString('Интерфейс', 'sLastPassword', ''), '');
      ...
      Configuration.OverseerServer.sMySQLHost := ReadString('Сервер и база данных', 'OverseerServer.sMySQLHost',
        'OVERSEERSERVER'); // overseerserver
      Configuration.OverseerServer.iMySQLPort := ReadInteger('Сервер и база данных', 'OverseerServer.iMySQLPort',
        MYSQL_PORT);
      ...
      Configuration.OverseerServer.sMySQLDatabase := ReadString('Сервер и база данных', 'OverseerServer.sMySQLDatabase',
        'overseer');
      ...
      Configuration.DBServer.sMySQLDatabase_IRD_Location :=
        ReadString('Сервер и база данных', 'DBServer.sMySQLDatabase_IRD_Location', 'statistika');
      Configuration.DBServer.sMySQLDatabase_ORD_Location :=
        ReadString('Сервер и база данных', 'DBServer.sMySQLDatabase_ORD_Location', 'statistika');
      Configuration.DBServer.sMySQLDatabase_PRIJAVE_Location :=
        ReadString('Сервер и база данных', 'DBServer.sMySQLDatabase_PRIJAVE_Location', 'test');
      Configuration.DBServer.sMySQLDatabase_PRIJAVE_Q_Location :=
        ReadString('Сервер и база данных', 'DBServer.sMySQLDatabase_PRIJAVE_Q_Location', 'test');
      Configuration.DBServer.sMySQLDatabase_T_DAY_Location :=
        ReadString('Сервер и база данных', 'DBServer.sMySQLDatabase_T_DAY_Location', 'MyStat');
      ...
      Configuration.StatServer.sMySQLDatabase_IRD_Location :=
        ReadString('Сервер и база данных', 'StatServer.sMySQLDatabase_IRD_Location', 'ird');
      Configuration.StatServer.sMySQLDatabase_ORD_Location :=
        ReadString('Сервер и база данных', 'StatServer.sMySQLDatabase_ORD_Location', 'ord');
      Configuration.StatServer.sMySQLDatabase_IRDA_Location :=
        ReadString('Сервер и база данных', 'StatServer.sMySQLDatabase_IRDA_Location', 'irda');
      Configuration.StatServer.sMySQLDatabase_ORDA_Location :=
        ReadString('Сервер и база данных', 'StatServer.sMySQLDatabase_ORDA_Location', 'orda');
      Configuration.StatServer.sMySQLDatabase_PRIJAVE_Location :=
        ReadString('Сервер и база данных', 'StatServer.sMySQLDatabase_PRIJAVE_Location', 'prijaves');
      Configuration.StatServer.sMySQLDatabase_PRIJAVE_Q_Location :=
        ReadString('Сервер и база данных', 'StatServer.sMySQLDatabase_PRIJAVE_Q_Location', 'prijaves');
  ...
  with TIniFile.Create(ChangeFileExt(ExpandFileName(Application.ExeName), '.ini')) do
    try
      try
        ...
        WriteString('Сервер и база данных', 'DBServer.sMySQLDatabase_IRD_Location',
          Configuration.DBServer.sMySQLDatabase_IRD_Location);
        WriteString('Сервер и база данных', 'DBServer.sMySQLDatabase_ORD_Location',
          Configuration.DBServer.sMySQLDatabase_ORD_Location);
        WriteString('Сервер и база данных', 'DBServer.sMySQLDatabase_PRIJAVE_Location',
          Configuration.DBServer.sMySQLDatabase_PRIJAVE_Location);
        WriteString('Сервер и база данных', 'DBServer.sMySQLDatabase_PRIJAVE_Q_Location',
          Configuration.DBServer.sMySQLDatabase_PRIJAVE_Q_Location);
        WriteString('Сервер и база данных', 'DBServer.sMySQLDatabase_T_DAY_Location',
          Configuration.DBServer.sMySQLDatabase_T_DAY_Location);
        ...
        WriteString('Сервер и база данных', 'StatServer.sMySQLDatabase_IRD_Location',
          Configuration.StatServer.sMySQLDatabase_IRD_Location);
        WriteString('Сервер и база данных', 'StatServer.sMySQLDatabase_ORD_Location',
          Configuration.StatServer.sMySQLDatabase_ORD_Location);
        WriteString('Сервер и база данных', 'StatServer.sMySQLDatabase_IRDA_Location',
          Configuration.StatServer.sMySQLDatabase_IRDA_Location);
        WriteString('Сервер и база данных', 'StatServer.sMySQLDatabase_ORDA_Location',
          Configuration.StatServer.sMySQLDatabase_ORDA_Location);
        WriteString('Сервер и база данных', 'StatServer.sMySQLDatabase_PRIJAVE_Location',
          Configuration.StatServer.sMySQLDatabase_PRIJAVE_Location);
        WriteString('Сервер и база данных', 'StatServer.sMySQLDatabase_PRIJAVE_Q_Location',
          Configuration.StatServer.sMySQLDatabase_PRIJAVE_Q_Location);
        ...
        on EIniFileException do
          ShowErrorBox(Handle,
            'Возникла ошибка при потытке сохранения конфигурации в INI-файле! Обратитесь к администратору!', LogGroupGUID);
      end;
    finally
      Free;
    end;
...
begin
  ...
  rbLastWeek.Caption := '&3) прошедшая неделя (' + FormatDateTime('dd.mm.yyyy', dtLastWeekStart) + ' - ' +
    FormatDateTime('dd.mm.yyyy', dtLastWeekStop) + ')';
  rbLastMonth.Caption := '&4) прошедший месяц (' + cbChoisenMonth_Month.Items[wLastMonth_Month - 1] + ' ' +
    IntToStr(wLastMonth_Year) + ' года)';
  rbLastQuarter.Caption := '&6) прошедший квартал (' + cbChoisenQuarter_Quarter.Items[wLastQuarter_Quarter - 1] + ' квартал '
    + IntToStr(wLastQuarter_Year) + ' года)';
...
begin
  ...
  if DirectoryExists(sDirectory) then
    ...
    Routines_GenerateError('Папка сохранённых отчётов не существует! Проверьте правильность указанного пути [' + sDirectory +
...
begin
  ...
  with ConfigurationForm do
    try
      ...
      vleInterface.Cells[1, 7] := Routines_GetConditionalMessage
        (vleInterface.ItemProps[6].PickList.IndexOf(Configuration.sDefaultAction) > -1, Configuration.sDefaultAction, '');
      ...
      edbxFlushLogOnStringsQuantity.Enabled := (not Configuration.bUseExternalLog) and
        chkbxFlushLogOnStringsQuantity.Checked;
      edbxFlushLogOnStringsQuantity.Text := Routines_GetConditionalMessage(edbxFlushLogOnStringsQuantity.Enabled,
      ...
      rbSaveIntoTheApplicationFolder.Caption := rbSaveIntoTheApplicationFolder.Caption + ' (' +
        Configuration.sApplicationFolder + ')';
      ...
      edbxSelectedFolder.Text := Routines_GetConditionalMessage(rbSaveIntoTheSelectedFolder.Checked,
      ...
      begin
        ...
        Timer1.Enabled := Configuration.bUseExternalLog;
        Configuration.sDefaultAction := Routines_GetConditionalMessage(cbActionsList.Items.IndexOf(vleInterface.Cells[1, 7])
          > -1, vleInterface.Cells[1, 7], '');
        ...
        if rbSaveIntoTheTempFolder.Checked then
          ...
          if rbSaveIntoTheApplicationFolder.Checked then
            ...
            begin
              ...
              Routines_GenerateError('Возникла ошибка при определении выбранной папки для сохранения отчётов!',
                sErrorMessage, bError);
...
begin
  ...
  begin
    if FileExists(ExtractFilePath(ExpandFileName(Application.ExeName)) + 'LogKeeper.exe') then
      ShellExecute(Application.Handle, 'open', PWideChar(ExtractFilePath(ExpandFileName(Application.ExeName)) +
        'LogKeeper.exe'), nil, nil, SW_NORMAL);
    // попытаться его запустить вновь
    if FileExists('D:\My Documents\RAD Studio\Projects\LogKeeper\LogKeeper.exe') then
      ShellExecute(Application.Handle, 'open', PWideChar('D:\My Documents\RAD Studio\Projects\LogKeeper\LogKeeper.exe'), nil,
        nil, SW_NORMAL);
    // попытаться его запустить вновь
  end;
  Update_Actions;
...
begin
  ...
  LogThis(PWideChar('Действие "' + Action_Logon.Caption + '" ' + Routines_GetConditionalMessage(Action_Logon.Enabled,
    'включено', 'отключено') + ', ' + 'действие "' + Action_Logout.Caption + '" ' +
    Routines_GetConditionalMessage(Action_Logout.Enabled, 'включено', 'отключено') + '.'), LogGroupGUID, lmtDebug);
...
begin
  ...
  begin
    ...
    if not(rbLastDate.Checked or rbChoisenDate.Checked or rbLastWeek.Checked or rbLastMonth.Checked or
      rbChoisenMonth.Checked or rbLastQuarter.Checked or rbChoisenQuarter.Checked or rbLastYear.Checked or
      rbChoisenYear.Checked or rbChoisenPeriod.Checked) then
    ...
    begin
      ...
      begin
        if dtpChoisenPeriod_Start.Date > dtpChoisenPeriod_Stop.Date then
          Routines_GenerateError('Выбранная начальная дата (' + FormatDateTime('dd.mm.yyyy', dtpChoisenPeriod_Start.Date) +
            ') больше, чем конечная (' + FormatDateTime('dd.mm.yyyy', dtpChoisenPeriod_Stop.Date) + ')!',
            sErrorMessage, bError)
        else
        begin
  ...
  begin
    ...
    if (not(rbGroupByYears.Checked or rbGroupByQuarters.Checked or rbGroupByMonths.Checked or rbGroupByWeeks.Checked or
      rbGroupByDays.Checked or rbGroupByHours.Checked)) and
      (rbGroupByYears.Enabled or rbGroupByQuarters.Enabled or rbGroupByMonths.Enabled or rbGroupByWeeks.Enabled or
      rbGroupByDays.Enabled or rbGroupByHours.Enabled) then
  ...
  begin
          ...
          begin
            ...
            begin
              if lvPhonesList_Payoff.Items[iRoutineCounter].SubItems.Count <> 9 then
                Routines_GenerateError('Не удалось преобразовать данные записи из списка (строка №' +
                  IntToStr(iRoutineCounter + 1) + ')!', sErrorMessage, bError)
              else
              begin
                ...
                begin
                  ...
                  izg := Routines_GetConditionalMessage(lvPhonesList_Payoff.Items[iRoutineCounter].SubItems[5] = 'Да',
                    '1', '0');
    ...
    begin
      ...
      try
        ...
        if slPhonesList.Count <= 0 then
          Routines_GenerateError('Не задано ни одного абонентского номера телефона для формирования отчёта!',
            sErrorMessage, bError)
        else
          if not Do_SIC_01(EncodeDate(wStartYear, wStartMonth, wStartDay), EncodeDate(wStopYear, wStopMonth, wStopDay), True,
            slPhonesList, cbShowUnserved_SIC_01.Checked and cbShowUnserved_SIC_01.Enabled) then
            Routines_GenerateError('Произошла ошибка во вложенной функции формирования отчёта!', sErrorMessage, bError);
      finally
        slPhonesList.Free;
      end;
    ...
    if cbActionsList.Text = 'Формирование отчёта по форме № СИЦ-03-...' then
      if not Do_SIC_03(EncodeDate(wStartYear, wStartMonth, wStartDay), EncodeDate(wStopYear, wStopMonth, wStopDay), True,
        cbPhoneNet_SIC_03.ItemIndex) then
        Routines_GenerateError('Произошла ошибка во вложенной функции формирования отчёта!', sErrorMessage, bError);
    ...
    begin
      ...
      try
        ...
        if rbSelectedUsers_SIC_04.Checked and (slUserList.Count <= 0) then
          ...
          if not Do_SIC_04(EncodeDate(wStartYear, wStartMonth, wStartDay), EncodeDate(wStopYear, wStopMonth, wStopDay), True,
            gbp, not(cbWeekStartsFromDayOfBeginning_SIC_04.Checked and cbWeekStartsFromDayOfBeginning_SIC_04.Enabled),
            rbAllUsers_SIC_04.Checked, cbCreateDetailedReport_SIC_04.Checked and cbCreateDetailedReport_SIC_04.Enabled,
            cbShowSQLQueries_SIC_04.Checked and cbShowSQLQueries_SIC_04.Enabled, cbShowIdleUsers_SIC_04.Checked and
            cbShowIdleUsers_SIC_04.Enabled, slUserList) then
            Routines_GenerateError('Произошла ошибка во вложенной функции формирования отчёта!', sErrorMessage, bError);
      finally
        slUserList.Free;
      end;
    ...
    begin
      if (cbService_SIC_05.ItemIndex < 0) and (cbService_SIC_05.ItemIndex > (aServices_Count - 1)) then
        ...
        if not Do_SIC_05(EncodeDate(wStartYear, wStartMonth, wStartDay), EncodeDate(wStopYear, wStopMonth, wStopDay), True,
          gbp, cbService_SIC_05.ItemIndex, not(cbWeekStartsFromDayOfBeginning_SIC_05.Checked and
          cbWeekStartsFromDayOfBeginning_SIC_05.Enabled)) then
          Routines_GenerateError('Произошла ошибка во вложенной функции формирования отчёта!', sErrorMessage, bError);
    end;
    ...
    begin
      if (cbService_SIC_06.ItemIndex < 0) and (cbService_SIC_06.ItemIndex > (aServices_Count - 1)) then
        ...
        if not Do_SIC_06(EncodeDate(wStartYear, wStartMonth, wStartDay), EncodeDate(wStopYear, wStopMonth, wStopDay), True,
          cbService_SIC_06.ItemIndex) then
          Routines_GenerateError('Произошла ошибка во вложенной функции формирования отчёта!', sErrorMessage, bError);
    end;

    if cbActionsList.Text = 'Формирование отчёта по форме № СИЦ-07-...' then
      if not Do_SIC_07(EncodeDate(wStartYear, wStartMonth, wStartDay), EncodeDate(wStopYear, wStopMonth, wStopDay), True,
        gbp, not(cbWeekStartsFromDayOfBeginning_SIC_07.Checked and cbWeekStartsFromDayOfBeginning_SIC_07.Enabled)) then
        Routines_GenerateError('Произошла ошибка во вложенной функции формирования отчёта!', sErrorMessage, bError);

    if cbActionsList.Text = 'Формирование отчёта по форме № СИЦ-08-...' then
      if not Do_SIC_08(EncodeDate(wStartYear, wStartMonth, wStartDay), EncodeDate(wStopYear, wStopMonth, wStopDay), True)
      then
        Routines_GenerateError('Произошла ошибка во вложенной функции формирования отчёта!', sErrorMessage, bError);
    ...
    begin
      ...
      try
        ...
        if slSelectedServicesList.Count <= 0 then
          ...
          if not Do_SIC_09(EncodeDate(wStartYear, wStartMonth, wStartDay), EncodeDate(wStopYear, wStopMonth, wStopDay), True,
            cbPhoneNet_SIC_09.ItemIndex, cbSummaryReport_SIC_09.Enabled and cbSummaryReport_SIC_09.Checked,
      ...
      begin
        ...
        try
          ...
          if rbSelectedUsers_SIC_10.Checked and (slUserList.Count <= 0) then
            ...
            if not Do_SIC_10(EncodeDate(wStartYear, wStartMonth, wStartDay), EncodeDate(wStopYear, wStopMonth, wStopDay),
              True, gbp, not(cbWeekStartsFromDayOfBeginning_SIC_10.Checked and
              cbWeekStartsFromDayOfBeginning_SIC_10.Enabled), rbAllUsers_SIC_10.Checked,
              cbCreateDetailedReport_SIC_10.Checked and cbCreateDetailedReport_SIC_10.Enabled, iCCID, slUserList) then
              Routines_GenerateError('Произошла ошибка во вложенной функции формирования отчёта!', sErrorMessage, bError);
        finally
          slUserList.Free;
        end;
    ...
    if cbActionsList.Text = 'Формирование таблиц данных входящих/исходящих вызовов (<i/o>rd_<yyyymmdd>, <i/o>rd_<yyyy>[mm][dd][_[yyyy][mm][dd]])'
    then
    begin
      aXRD := XRDInput;
      ...
      if rbORD.Checked then
        aXRD := XRDOutput;
      sTableName := Routines_GetConditionalMessage((cbCustomNameOfXRDA.Checked) and (ebCustomNameOfXRDA.Text <> ''),
        ebCustomNameOfXRDA.Text, '');
      if not Do_CreateXRDA(EncodeDate(wStartYear, wStartMonth, wStartDay), EncodeDate(wStopYear, wStopMonth, wStopDay),
        False, aXRD, sTableName, cbRecreateXRD.Checked, cbCreateXRDA.Checked, cbRecreateXRDA.Checked) then
...
begin
  ...
  if (cbActionsList.Text =
    'Формирование таблиц данных входящих/исходящих вызовов (<i/o>rd_<yyyymmdd>, <i/o>rd_<yyyy>[mm][dd][_[yyyy][mm][dd]])')
  then
  begin
    rbIRD.Checked := True;
    rbORD.Checked := False;
...
begin
  ...
  if (cbActionsList.Text = 'Формирование отчёта по форме № СИЦ-01-...') or
    (cbActionsList.Text = 'Формирование отчёта по форме № СИЦ-03-...') or
    (cbActionsList.Text = 'Формирование отчёта по форме № СИЦ-06-...') or
    (cbActionsList.Text = 'Формирование отчёта по форме № СИЦ-08-...') or
    (cbActionsList.Text = 'Формирование отчёта по форме № СИЦ-09-...') or
    (cbActionsList.Text =
    'Формирование таблиц данных входящих/исходящих вызовов (<i/o>rd_<yyyymmdd>, <i/o>rd_<yyyy>[mm][dd][_[yyyy][mm][dd]])') or
    (cbActionsList.Text = 'Снятие оплаты за звонок на голосовые службы СИЦ') or
  ...
  begin
    ...
    if rbLastYear.Checked or rbChoisenYear.Checked or (rbChoisenPeriod.Checked and bStartOfTheYear and bEndOfTheYear and
      (wStartYear <= wStopYear)) then
    begin
      ...
      if rbLastQuarter.Checked or rbChoisenQuarter.Checked or
        (rbChoisenPeriod.Checked and bStartOfTheQuarter and bEndOfTheQuarter and
        (((wStartYear = wStopYear) and (wStopMonth >= wStartMonth + 2)) or (wStartYear < wStopYear))) then
      begin
        rbGroupByYears.Enabled := False;
        rbGroupByQuarters.Enabled := True;
        rbGroupByMonths.Enabled := True;
        ...
        if rbLastMonth.Checked or rbChoisenMonth.Checked or
          (rbChoisenPeriod.Checked and bStartOfTheMonth and bEndOfTheMonth and
          (((wStartYear = wStopYear) and (wStopMonth >= wStartMonth)) or (wStartYear < wStopYear))) then
        begin
          rbGroupByYears.Enabled := False;
          rbGroupByQuarters.Enabled := False;
          rbGroupByMonths.Enabled := True;
...
function TMainForm.Do_SaveReportAsHTMLAndOpen(const aHTML, aFileName: string;
  const aOpenWithBrowser, aSilentYes: Boolean): Boolean;
const
...
begin
  ProcedureHeader('Функция сохранения данных отчёта в файл и открытия данного файла при помощи ассоциированного приложения',
    LogGroupGUID);
  bError := False;
      ...
      try
        ...
        if not bError then
          if (not Configuration.bDontDemandOverwriteConfirmation) and FileExists(sFileName) and (not aSilentYes) then
            if MessageBox(Handle, PWideChar('Вы действительно хотите заменить существующий файл?'),
              PWideChar('OVERSEER - Подтверждение замены'), MB_OKCANCEL + MB_ICONQUESTION + MB_DEFBUTTON2) <> IDOK then
              Routines_GenerateError('Возникла ошибка при сохранении отчёта в файл ' + #39 + sFileName + #39 +
                ' - операция замены файла не была подтверждена пользователем!', sErrorMessage, bError);
        if not bError then
        begin
...
begin
  ProcedureHeader('Процедура реакции на переключение состояния флажка "' + cbCreateDetailedReport_SIC_04.Caption + '"',
    LogGroupGUID);

  cbShowSQLQueries_SIC_04.Enabled := cbCreateDetailedReport_SIC_04.Checked;
  if not cbShowSQLQueries_SIC_04.Enabled then
    cbShowSQLQueries_SIC_04.Checked := False;
  LogThis('Флажок "' + cbCreateDetailedReport_SIC_04.Caption + '" ' + Routines_GetConditionalMessage
    (cbCreateDetailedReport_SIC_04.Checked, 'включён', 'отключён') + '.', LogGroupGUID, lmtDebug);
...
function TMainForm.Do_CreateXRDA(const aStartDate, aStopDate: TDate; const aSilentYes: Boolean; const aXRD: tXRD;
  var aTableName: string; const bRecreateXRD: Boolean; const bCreateXRDA, bRecreateXRDA: Boolean): Boolean;
const
...
begin
  ...
  begin
    LogThis('>> Выполняется операция генерации имени таблицы данных xrda_<yyyy>[mm][dd][_[yyyy][mm][dd]]...', LogGroupGUID,
      lmtDebug);
    if aTableName <> '' then
      ...
      if bStartOfTheYear and bEndOfTheYear and (wStartYear = wStopYear) then
        ...
        if bStartOfTheMonth and bEndOfTheMonth and (wStartYear = wStopYear) and (wStartMonth = wStopMonth) then
          ...
          if (wStartYear = wStopYear) and (wStartMonth = wStopMonth) and (wStartDay = wStopDay) then
            ...
            sXRDATableName := cXRDPrefix + 'rda_' + FormatDateTime('yyyymmdd', aStartDate) + '_' +
              FormatDateTime('yyyymmdd', aStopDate);
    end;
    aTableName := sXRDATableName;
    LogThis('Сгенерированное имя таблицы данных xrda_<yyyy>[mm][dd][_[yyyy][mm][dd]] равно "' + sXRDATableName + '".',
      LogGroupGUID, lmtDebug);
    LogThis('<< Выполнение операции генерации имени таблицы данных xrda_<yyyy>[mm][dd][_[yyyy][mm][dd]] завершено.',
      LogGroupGUID, lmtDebug);

    // Проверка наличия готовой таблицы данных xrda_<yyyy>[mm][dd][_[yyyy][mm][dd]], если нужно (пере)формировать XRDA
    LogThis('>> Выполняется операция проверки наличия таблицы данных "' + sXRDATableName + '" в базе данных...',
      LogGroupGUID, lmtDebug);
      ...
      begin
        ...
        begin
          ...
          if ResultSet = nil then
            Routines_GenerateError('Не удалось получить результирующую выборку по последнему SQL-запросу!',
              sErrorMessage, bError)
          else
          begin
            ...
            if iRoutineCounter < 0 then
              Routines_GenerateError('Возникла ошибка при получении количества срок результирующей выборки!',
                sErrorMessage, bError)
            else
            begin
              if (iRoutineCounter < 0) or (iRoutineCounter > 1) then
                Routines_GenerateError('Количество строк выборки (' + IntToStr(iRoutineCounter) +
                  ') не соответствует требуемому (0 или 1)!', sErrorMessage, bError)
              else
              begin
                ...
                begin
                  ...
                  if ResultRow = nil then
                    Routines_GenerateError
                      ('Возникла ошибка при загрузке данных строки выборки (порядковый номер строки равен 1)!',
                      sErrorMessage, bError)
                  else
                  begin
                    if LowerCase(string(ResultRow[0])) <> LowerCase(sXRDATableName) then
                      Routines_GenerateError('Полученное имя таблицы данных не соответствует требуемому!',
                        sErrorMessage, bError)
                    else
                    begin
    ...
    LogThis('<< Выполнение операции проверки наличия таблицы данных "' + sXRDATableName + '" в базе данных завершено.',
      LogGroupGUID, lmtDebug);
    StepProgressBar;
  end;
  ...
  begin
    ...
    try
      ...
      begin
        ...
        LogThis('>> Выполняется операция составления списка таблиц данных для проверки их наличия в базе данных...',
          LogGroupGUID, lmtDebug);
        dtDate := aStartDate;
        repeat
        ...
        LogThis('Финальное количество строк списка таблиц данных равно ' + IntToStr(slXRDFullList.Count) + '.', LogGroupGUID,
          lmtDebug);
        LogThis('<< Выполнение операции составления списка таблиц данных для проверки их наличия в базе данных завершено.',
          LogGroupGUID, lmtDebug);
        ...
        try
          ...
          begin
            for iFullListCounter := 0 to slXRDFullList.Count - 1 do
              ...
              begin
                LogThis('>> Выполняется операция проверки наличия таблицы данных "' + slXRDFullList[iFullListCounter] +
                  '" в базе данных...', LogGroupGUID, lmtDebug);
                if not Configuration.StatServer.bConnected then
                  Routines_GenerateError('Для выполнения операции необходимо подключение к серверу MySQL!',
                    sErrorMessage, bError)
                else
                begin
                  if mysql_ping(Configuration.StatServer.hConnection) <> 0 then
                    Routines_GenerateError('Возникла ошибка при попытке проверки подключения к серверу MySQL!',
                      sErrorMessage, bError)
                  else
                  begin
                    q := 'SHOW TABLES FROM ' + sSTATSERVER_XRD_Location + ' LIKE "' +
                      LowerCase(slXRDFullList[iFullListCounter]) + '";';
                    LogThis(q, LogGroupGUID, lmtSQL);
                    if mysql_real_query(Configuration.StatServer.hConnection, PAnsiChar(AnsiString(q)), Length(q)) <> 0 then
                      Routines_GenerateError('Возникла ошибка при выполнении последнего SQL-запроса!', sErrorMessage, bError)
                    else
                        ...
                        begin
                          ...
                          begin
                            ...
                            begin
                              ...
                              LogThis('Искомая таблица данных (' + sSTATSERVER_XRD_Location + '.' +
                                slXRDFullList[iFullListCounter] + ') не была найдена!', LogGroupGUID, lmtWarning);
                            end
                            else
                            begin
                              ResultRow := mysql_fetch_row(ResultSet);
                ...
                LogThis('<< Выполнение операции проверки наличия таблицы данных "' + slXRDFullList[iFullListCounter] +
                  '" в базе данных завершено.', LogGroupGUID, lmtDebug);
              end;
          end;
          if not bError then
            LogThis('Количество недостающих таблиц данных равно ' + IntToStr(slXRDMissedList.Count) + '.', LogGroupGUID,
              lmtWarning);
            ...
            begin
              ...
              for iMissedListCounter := 0 to slXRDMissedList.Count - 1 do
                ...
                begin
                  ...
                  LogThis('>> Выполняется операция проверки наличия таблицы данных "' + slXRDMissedList[iMissedListCounter] +
                    '" в базе данных...', LogGroupGUID, lmtDebug);
                  ...
                  begin
                    ...
                    begin
                      q := 'SHOW TABLES FROM ' + sDBSERVER_XRD_Location + ' LIKE "' +
                        LowerCase(slXRDMissedList[iMissedListCounter]) + '";';
                      LogThis(q, LogGroupGUID, lmtSQL);
                      if mysql_real_query(Configuration.DBServer.hConnection, PAnsiChar(AnsiString(q)), Length(q)) <> 0 then
                        Routines_GenerateError('Возникла ошибка при выполнении последнего SQL-запроса!',
                          sErrorMessage, bError)
                      else
                      begin
                        LogThis('Запрос выполнен успешно.', LogGroupGUID, lmtDebug);
                        ...
                        begin
                          LogThis('Результирующая выборка получена успешно.', LogGroupGUID, lmtDebug);
                          iRoutineCounter := mysql_num_rows(ResultSet);
                          LogThis('Количество строк выборки равно ' + IntToStr(iRoutineCounter) + '.', LogGroupGUID,
                            lmtDebug);
                          if iRoutineCounter < 0 then
                            Routines_GenerateError('Возникла ошибка при получении количества срок результирующей выборки!',
                              sErrorMessage, bError)
                          ...
                          begin
                            ...
                            begin
                              LogThis('Количество строк выборки (' + IntToStr(iRoutineCounter) +
                                ') не соответствует требуемому (1)!', LogGroupGUID, lmtDebug);
                              Routines_GenerateError('Искомая таблица данных  (' + sDBSERVER_XRD_Location + '.' +
                                slXRDMissedList[iMissedListCounter] + ') не была найдена!', sErrorMessage, bError);
                            end
                            else
                            begin
                  ...
                  LogThis('<< Выполнение операции проверки наличия таблицы данных "' + slXRDMissedList[iMissedListCounter] +
                    '" в базе данных завершено.', LogGroupGUID, lmtDebug);
                  StepProgressBar;
                  ...
                  if not bError then
                  begin
                    LogThis('>> Выполняется операция получения запроса на создание очередной недостающей таблицы данных...',
                      LogGroupGUID, lmtDebug);
                    if not Configuration.DBServer.bConnected then
                      Routines_GenerateError('Для выполнения операции необходимо подключение к серверу MySQL!',
                        sErrorMessage, bError)
                    ...
                    begin
                      ...
                      begin
                        q := 'SHOW CREATE TABLE ' + sDBSERVER_XRD_Location + '.' +
                          LowerCase(slXRDMissedList[iMissedListCounter]) + ';';
                        LogThis(q, LogGroupGUID, lmtSQL);
                        if mysql_real_query(Configuration.DBServer.hConnection, PAnsiChar(AnsiString(q)), Length(q)) <> 0
                        then
                          Routines_GenerateError('Возникла ошибка при выполнении последнего SQL-запроса!',
                            sErrorMessage, bError)
                        else
                        begin
                          ...
                          begin
                            ...
                            begin
                              if iRoutineCounter > 1 then
                                Routines_GenerateError('Количество строк выборки (' + IntToStr(iRoutineCounter) +
                                  ') не соответствует требуемому (1)!', sErrorMessage, bError)
                              else
                              begin
                                ResultRow := mysql_fetch_row(ResultSet);
                                if ResultRow = nil then
                                  ...
                                  if ResultRow[1] = '' then
                                    Routines_GenerateError
                                      ('Возникла ошибка при получении строки запроса на создание очередной недостающей таблицы данных!',
                                      sErrorMessage, bError)
                                  else
                                  begin
                                    sCreateQuery :=
                                      StringReplace(StringReplace(StringReplace(string(ResultRow[1]), #$A, '',
                                      [rfReplaceAll]), '`', '', [rfReplaceAll]), 'CREATE TABLE ' + cXRDPrefix + 'rd_',
                                      'CREATE TABLE ' + sSTATSERVER_XRD_Location + '.' + cXRDPrefix + 'rd_', [rfReplaceAll]);
                                    LogThis('Запрос на создание недостающей таблицы данных получен успешно.', LogGroupGUID,
                                      lmtDebug);
                                  end;
                              end;
                            end;
                    ...
                    LogThis('<< Выполнение операции получения запроса на создание очередной недостающей таблицы данных завершено.',
                      LogGroupGUID, lmtDebug);
                    StepProgressBar;
                    ...
                    begin
                      ...
                      MySQL_DropTable(LogGroupGUID, Configuration.StatServer, bError, sErrorMessage, //
                        'DROP TABLE IF EXISTS ' + sSTATSERVER_XRD_Location + '.' +
                        LowerCase(slXRDMissedList[iMissedListCounter]) + ';'); //
                      StepProgressBar;
                      ...
                      begin
                        ...
                        begin
                          ...
                          MySQL_CheckTableExistance(LogGroupGUID, Configuration.StatServer, sSTATSERVER_XRD_Location, bError,
                            sErrorMessage, LowerCase(slXRDMissedList[iMissedListCounter]));
                          StepProgressBar;

                          if not bError then
                          begin
                            // Блокирование таблицы на DBSERVER на чтение
                            MySQL_LockTables(LogGroupGUID, Configuration.DBServer, bError, sErrorMessage,
                              sDBSERVER_XRD_Location + '.' + LowerCase(slXRDMissedList[iMissedListCounter]) + ' READ');
                            StepProgressBar;

                            if not bError then
                            begin
                              // Блокирование таблиц на STATSERVER на запись
                              MySQL_LockTables(LogGroupGUID, Configuration.StatServer, bError, sErrorMessage,
                                sSTATSERVER_XRD_Location + '.' + LowerCase(slXRDMissedList[iMissedListCounter]) + ' WRITE');
                              StepProgressBar;
                            end;
                          end;
                        end;
                  ...
                  begin
                    ...
                    LogThis('>> Выполняется операция получения записей таблицы данных "' +
                      LowerCase(slXRDMissedList[iMissedListCounter]) + '"...', LogGroupGUID, lmtDebug);
                    if not Configuration.DBServer.bConnected then
                      Routines_GenerateError('Для выполнения операции необходимо подключение к серверу MySQL!',
                        sErrorMessage, bError)
                    else
                      ...
                      begin
                        ...
                        begin
                          ...
                          begin
                            ...
                            LogThis('Количество строк выборки равно ' + IntToStr(iRowCountSource) + '.', LogGroupGUID,
                              lmtDebug);
                            ...
                            begin
                              ...
                              for iSourceRowCounter := 0 to iRowCountSource - 1 do
                                ...
                                begin
                                  LogThis('Получение очередной строки выборки (' + IntToStr(iSourceRowCounter) + ').',
                                    LogGroupGUID, lmtDebug);
                                  ResultRow := mysql_fetch_row(ResultSet);
                                  if ResultRow = nil then
                                    Routines_GenerateError
                                      ('Возникла ошибка при загрузке данных строки выборки (порядковый номер строки равен ' +
                                      IntToStr(iSourceRowCounter) + ')!', sErrorMessage, bError)
                                  else
                                  begin
                                    LogThis('Операция получения очередной строки выборки (' + IntToStr(iSourceRowCounter) +
                                      ') прошла успешно.', LogGroupGUID, lmtDebug);

                                    // Вставка очередной строки данных в таблицу на STATSERVER
                                    iRoutineCounter := MySQL_InsertRecords(LogGroupGUID, Configuration.StatServer, bError,
                                      sErrorMessage, //
                                      'INSERT INTO ' + sSTATSERVER_XRD_Location + '.' +
                                      LowerCase(slXRDMissedList[iMissedListCounter]) + ' ' //
                                      + 'VALUES (' //
                                    ...
                                    if iRoutineCounter <> 1 then
                                      Routines_GenerateError('Количество обработанных строк (' + IntToStr(iRoutineCounter) +
                                        ') не соответствует требуемому (1)!', sErrorMessage, bError);

                                    // Проверка очередной вставленной в таблицу на STATSERVER строки данных
                                    if not bError then
                                    begin
                                      LogThis('>> Выполняется операция проверки данных, внесённых в таблицу ' +
                                        LowerCase(slXRDMissedList[iMissedListCounter]) + '...', LogGroupGUID, lmtDebug);
                                      if not Configuration.StatServer.bConnected then
                                        Routines_GenerateError
                                        ('Для выполнения операции необходимо подключение к серверу MySQL!',
                                        sErrorMessage, bError)
                                      else
                                      begin
                                        if mysql_ping(Configuration.StatServer.hConnection) <> 0 then
                                        ...
                                        begin
                                        q := 'SELECT * FROM ' + sSTATSERVER_XRD_Location + '.' +
                                        LowerCase(slXRDMissedList[iMissedListCounter]) + ' ' //
                                        + 'WHERE' //
                                        ...
                                        //
                                        + ' AND vrijeme=' + Routines_NormalizeStringForQuery(string(ResultRow[3]),
                                        True, False) //
                                        ...
                                        //
                                        + ' AND izgnum=' + Routines_NormalizeStringForQuery(string(ResultRow[9]),
                                        True, False) //
                                        + ' AND druga_info=' + Routines_NormalizeStringForQuery(string(ResultRow[10]),
                                        True, False) //
                                        + ' AND v_oper=' + Routines_NormalizeStringForQuery(string(ResultRow[11]),
                                        True, False) //
                                        + ' AND v_mreza=' + Routines_NormalizeStringForQuery(string(ResultRow[12]),
                                        True, False) //
                                        ...
                                        //
                                        + ' AND d_qid=' + Routines_NormalizeStringForQuery(string(ResultRow[15]),
                                        True, False) //
                                        + ' AND d_ccid=' + Routines_NormalizeStringForQuery(string(ResultRow[16]),
                                        True, False) //
                                        + ' AND qdur=' + Routines_NormalizeStringForQuery(string(ResultRow[17]), True,
                                        False) + ';'; //
                                        LogThis(q, LogGroupGUID, lmtSQL);
                                        if mysql_real_query(Configuration.StatServer.hConnection, PAnsiChar(AnsiString(q)),
                                        Length(q)) <> 0 then
                                        Routines_GenerateError('Возникла ошибка при выполнении последнего SQL-запроса!',
                                        sErrorMessage, bError)
                                        else
                                        ...
                                        LogThis('Количество строк выборки равно ' + IntToStr(iRoutineCounter) + '.',
                                        LogGroupGUID, lmtDebug);
                                        if iRoutineCounter < 0 then
                                        Routines_GenerateError
                                        ('Возникла ошибка при получении количества срок результирующей выборки!',
                                        sErrorMessage, bError)
                                        else
                                        begin
                                        if iRoutineCounter <= 0 then
                                        Routines_GenerateError('Количество строк выборки (' + IntToStr(iRoutineCounter) +
                                        ') не соответствует требуемому (>0)!', sErrorMessage, bError)
                                        else
                                        begin
                                        LogThis('Количество строк выборки соответствует требуемому.', LogGroupGUID,
                                        lmtDebug);
                                        LogThis('Проверка данных, внесённых в таблицу ' +
                                        LowerCase(slXRDMissedList[iMissedListCounter]) + ' выполнена успешно.', LogGroupGUID,
                                        lmtDebug);
                                        end;
                                        end;
                                        mysql_free_result(ResultSet2);
                                        LogThis('Освобождение ресурсов, занятых результатом выборки, выполнено.',
                                        LogGroupGUID, lmtDebug);
                                        end;
                                        end;
                                        end;
                                      end;
                                      LogThis('<< Выполнение операции проверки данных, внесённых в таблицу ' +
                                        LowerCase(slXRDMissedList[iMissedListCounter]) + ' завершено.', LogGroupGUID,
                                        lmtDebug);
                                    end;
                                    StepProgressBar;
                                  end;
                            ...
                            mysql_free_result(ResultSet);
                            LogThis('Освобождение ресурсов, занятых результатом выборки, выполнено.', LogGroupGUID,
                              lmtDebug);
                          end;
                        end;
                      end;
                    end;
                    LogThis('<< Выполнение операции получения записей таблицы данных "' +
                      LowerCase(slXRDMissedList[iMissedListCounter]) + '" завершено.', LogGroupGUID, lmtDebug);
                    StepProgressBar;
                  end;
                  ...
                  if not bError then
                  begin
                    LogThis('>> Выполняется операция получения количества записей таблицы данных "' +
                      LowerCase(slXRDMissedList[iMissedListCounter]) + '"...', LogGroupGUID, lmtDebug);
                    if not Configuration.StatServer.bConnected then
                      Routines_GenerateError('Для выполнения операции необходимо подключение к серверу MySQL!',
                        sErrorMessage, bError)
                    ...
                    begin
                      ...
                      begin
                        q := 'SELECT COUNT(*) FROM ' + sSTATSERVER_XRD_Location + '.' +
                          LowerCase(slXRDMissedList[iMissedListCounter]) + ';';
                        LogThis(q, LogGroupGUID, lmtSQL);
                        if mysql_real_query(Configuration.StatServer.hConnection, PAnsiChar(AnsiString(q)), Length(q)) <> 0
                        then
                          Routines_GenerateError('Возникла ошибка при выполнении последнего SQL-запроса!',
                            sErrorMessage, bError)
                        else
                              ...
                              begin
                                ...
                                begin
                                  ...
                                  if iRowCountDestination = -1 then
                                    Routines_GenerateError
                                      ('Не удалось произвести конвертацию полученного количества таблицы данных!',
                                      sErrorMessage, bError)
                                  else
                                  begin
                                    LogThis('Получение количества записей таблицы данных получено успешно.', LogGroupGUID,
                                      lmtDebug);
                                    LogThis('Количество строк таблицы данных составляет: ' + IntToStr(iRowCountDestination) +
                                      '.', LogGroupGUID, lmtDebug);
                                  end;
                                end;
                              end;
                    ...
                    LogThis('<< Выполнение операции получения количества записей таблицы данных "' +
                      LowerCase(slXRDMissedList[iMissedListCounter]) + '" завершено.', LogGroupGUID, lmtDebug);
                    StepProgressBar;
                  end;
                  ...
                  begin
                    ...
                    if iRowCountDestination <> iRowCountSource then
                      Routines_GenerateError('Количество записей (' + IntToStr(iRowCountSource) +
                        ') в исходной таблице данных (' + sDBSERVER_XRD_Location + '.' +
                        LowerCase(slXRDMissedList[iMissedListCounter]) + ') отличается от количества записей (' +
                        IntToStr(iRowCountDestination) + ') в результирующей таблице данных (' + sSTATSERVER_XRD_Location +
                        '.' + LowerCase(slXRDMissedList[iMissedListCounter]) + ')!', sErrorMessage, bError)
                    else
                      LogThis('Количество записей таблиц данных совпадает (' + IntToStr(iRowCountSource) + ').',
                        LogGroupGUID, lmtDebug);
                    LogThis('<< Выполнение операции сравнения количества записей таблиц данных завершено.', LogGroupGUID,
                      lmtDebug);
                    StepProgressBar;
        ...
        begin
          ...
          begin
            ...
            begin
              ...
              MySQL_CheckTableExistance(LogGroupGUID, Configuration.StatServer, sSTATSERVER_XRDA_Location, bError,
                sErrorMessage, LowerCase(sXRDATableName));
              StepProgressBar;
  ...
  begin
    ...
    if bRecreateXRD then
      sMessage := sMessage + sLineBreak + 'Таблицы ' + sSTATSERVER_XRD_Location + '.' + cXRDPrefix +
        'rd_<yyyymmdd> были успешно переформированы на сервере ' + Configuration.StatServer.sMySQLHost + '.'
    else
      sMessage := sMessage + sLineBreak + 'Наличие таблиц ' + sSTATSERVER_XRD_Location + '.' + cXRDPrefix +
        'rd_<yyyymmdd> на сервере ' + Configuration.StatServer.sMySQLHost + ' успешно проверено.';
    if bCreateXRDA and bXRDATableExists and (not bRecreateXRDA) then
      sMessage := sMessage + sLineBreak + 'Наличие таблицы ' + sSTATSERVER_XRDA_Location + '.' + LowerCase(sXRDATableName) +
        ' на сервере ' + Configuration.StatServer.sMySQLHost + ' успешно проверено.';
    if bCreateXRDA and (not bXRDATableExists) then
      sMessage := sMessage + sLineBreak + 'Таблица ' + sSTATSERVER_XRDA_Location + '.' + LowerCase(sXRDATableName) +
        ' была успешно сформирована на сервере ' + Configuration.StatServer.sMySQLHost + '.';
    if bRecreateXRDA and bXRDATableExists then
      sMessage := sMessage + sLineBreak + 'Таблица ' + sSTATSERVER_XRDA_Location + '.' + LowerCase(sXRDATableName) +
        ' была успешно переформирована на сервере ' + Configuration.StatServer.sMySQLHost + '.';
...
begin
  ...
  begin
    ...
    MySQL_CheckTableExistance(LogGroupGUID, Configuration.RNE4Server, Configuration.RNE4Server.sMySQLDatabase, bError,
      sErrorMessage, '_usr');
    StepProgressBar; // 2
    ...
    begin
      ...
      MySQL_LockTables(LogGroupGUID, Configuration.RNE4Server, bError, sErrorMessage, Configuration.RNE4Server.sMySQLDatabase
        + '._usr READ');
      StepProgressBar; // 3
      ...
      begin
                  ...
                  for iRowCounter := 0 to iRowQuantity - 1 do
                    ...
                    begin
                      ...
                      if ResultRow = nil then
                        Routines_GenerateError
                          ('Возникла ошибка при загрузке данных строки выборки (порядковый номер строки равен ' +
                          IntToStr(iRowCounter) + ')!', sErrorMessage, bError)
                      else
                      begin
                        LogThis('Операция получения очередной строки выборки (' + IntToStr(iRowCounter) +
                          ') прошла успешно.', LogGroupGUID, lmtDebug);
        ...
        LogThis('<< Выполнение операции получения данных пользователей и заполнение списка завершено.', LogGroupGUID,
          lmtDebug);
        StepProgressBar; // 4
    ...
    begin
      ...
      try
        ...
        try
          ...
          LogThis('Финальное количество строк списка таблиц данных равно ' + IntToStr(slIRDFullList.Count) + '.',
            LogGroupGUID, lmtDebug);
            ...
            begin
              ...
              for iFullListCounter := 0 to slIRDFullList.Count - 1 do
                ...
                begin
                  ...
                  MySQL_CheckTableExistance(LogGroupGUID, Configuration.StatServer,
                    Configuration.StatServer.sMySQLDatabase_IRD_Location, bError, sErrorMessage,
                    LowerCase(slIRDFullList[iFullListCounter]));

                  if not bError then
                  begin
                    // блокирование таблицы
                    MySQL_LockTables(LogGroupGUID, Configuration.StatServer, bError, sErrorMessage,
                      Configuration.StatServer.sMySQLDatabase_IRD_Location + '.' + LowerCase(slIRDFullList[iFullListCounter])
                      + ' READ');
                    ...
                    begin
                      ...
                      begin
                        ...
                        begin
                          q := 'SELECT' //
                            ...
                            + ' v_oper,' //
                            + ' v_mreza ' //
                            + 'FROM ' + Configuration.StatServer.sMySQLDatabase_IRD_Location + '.' +
                            LowerCase(slIRDFullList[iFullListCounter]) + ' ' //
                          ...
                          begin
                            ...
                            begin
                              LogThis('Результирующая выборка получена успешно.', LogGroupGUID, lmtDebug);
                              iRowQuantity := mysql_num_rows(ResultSet);
                              LogThis('Количество строк выборки равно ' + IntToStr(iRowQuantity) + '.', LogGroupGUID,
                                lmtDebug);
                              if iRowQuantity < 0 then
                                Routines_GenerateError
                                  ('Возникла ошибка при получении количества срок результирующей выборки!',
                              ...
                              begin
                                ...
                                for iRowCounter := 0 to iRowQuantity - 1 do
                                  ...
                                  begin
                                    LogThis('Получение очередной строки выборки (' + IntToStr(iRowCounter) + ').',
                                      LogGroupGUID, lmtDebug);
                                    ResultRow := mysql_fetch_row(ResultSet);
                                    if ResultRow = nil then
                                      Routines_GenerateError
                                    ...
                                    begin
                                      ...
                                      ListItem.SubItems.Add(Routines_GetConditionalMessage(string(ResultRow[6]) = '1', 'Да',
                                        Routines_GetConditionalMessage(string(ResultRow[6]) = '0', 'Нет', 'Ошибка!')));
                      ...
                      LogThis('<< Выполнение операции получения данных и заполнение списка завершено.', LogGroupGUID,
                        lmtDebug);
        ...
        if not bError then
          ...
          begin
            ...
            begin
              // проверка наличия таблицы в базе данных
              MySQL_CheckTableExistance(LogGroupGUID, Configuration.DBServer,
                Configuration.DBServer.sMySQLDatabase_IRD_Location, bError, sErrorMessage,
                'ird_' + FormatDateTime('yyyymmdd', aStopDate));
                  ...
                  begin
                    ...
                    begin
                      q := 'SELECT' //
                        ...
                        + ' v_oper,' //
                        + ' v_mreza ' //
                        + 'FROM ' + Configuration.DBServer.sMySQLDatabase_IRD_Location + '.ird_' +
                        FormatDateTime('yyyymmdd', aStopDate) + ' ' //
                        + 'WHERE rm>0 AND srv="0" AND ';
                      if slPhonesList.Count > 1 then
                        q := q + '('; //
                        ...
                        begin
                          ...
                          begin
                            ...
                            for iRowCounter := 0 to iRowQuantity - 1 do
                              ...
                              begin
                                LogThis('Получение очередной строки выборки (' + IntToStr(iRowCounter) + ').', LogGroupGUID,
                                  lmtDebug);
                                ResultRow := mysql_fetch_row(ResultSet);
                                if ResultRow = nil then
                                  Routines_GenerateError
...
begin
  ...
  b := False;
  for i := 0 to lvServices_SIC_09.Items.Count - 1 do
    if ((not lvServices_SIC_09.Items[i].Checked) and (lvServices_SIC_09.Items[i].ImageIndex = ICON_ORALSERVICES)) or
      (lvServices_SIC_09.Items[i].Checked and (lvServices_SIC_09.Items[i].ImageIndex = ICON_AUTOSERVICES)) then
    begin
      b := True;
      Break;
    end;
  ...
  b := False;
  for i := 0 to lvServices_SIC_09.Items.Count - 1 do
    if (lvServices_SIC_09.Items[i].Checked and (lvServices_SIC_09.Items[i].ImageIndex = ICON_ORALSERVICES)) or
      ((not lvServices_SIC_09.Items[i].Checked) and (lvServices_SIC_09.Items[i].ImageIndex = ICON_AUTOSERVICES)) then
    begin
      b := True;
      Break;
    end;
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        // проверка наличия таблицы пользователей в базе данных
        MySQL_CheckTableExistance(LogGroupGUID, Configuration.DBServer, Configuration.DBServer.sMySQLDatabase_T_DAY_Location,
          bError, sErrorMessage, 't_day');
        StepProgressBar; // 2
        ...
        begin
          // блокирование таблицы
          MySQL_LockTables(LogGroupGUID, Configuration.DBServer, bError, sErrorMessage,
            Configuration.DBServer.sMySQLDatabase_T_DAY_Location + '.t_day READ');
          StepProgressBar; // 3
            ...
            begin
              ...
              begin
                q := 'SELECT DISTINCT name ' //
                  ...
                  + 'WHERE' //
                  + sCCID //
                  + ' date BETWEEN "' + FormatDateTime('yyyymmdd', EncodeDate(wStartYear, wStartMonth, wStartDay)) +
                  '" AND "' + FormatDateTime('yyyymmdd', EncodeDate(wStopYear, wStopMonth, wStopDay)) + '" ' //
                  + 'ORDER BY name;'; //
                LogThis(q, LogGroupGUID, lmtSQL);
                if mysql_real_query(Configuration.DBServer.hConnection, PAnsiChar(AnsiString(q)), Length(q)) <> 0 then
                  Routines_GenerateError('Возникла ошибка при выполнении последнего SQL-запроса!', sErrorMessage, bError)
...
begin
  ...
  with LoginForm do
    try
      edbxLogin.Text := Routines_GetConditionalMessage(Configuration.bStoreLastLogin and (Configuration.sLastLogin <> ''),
        Configuration.sLastLogin, '');
      mePassword.Text := Routines_GetConditionalMessage(Configuration.bStoreLastPassword and
        (Configuration.sLastPassword <> '') and (Configuration.sLastLogin <> ''), Configuration.sLastPassword, '');
  ...
  begin
    ...
    begin
      ...
      MySQL_CheckTableExistance(LogGroupGUID, Configuration.OverseerServer, Configuration.OverseerServer.sMySQLDatabase,
        bError, sErrorMessage, '_users');
      StepProgressBar; // 2

      if not bError then
      begin
        // блокирование таблицы
        MySQL_LockTables(LogGroupGUID, Configuration.OverseerServer, bError, sErrorMessage,
          Configuration.OverseerServer.sMySQLDatabase + '._users READ');
        StepProgressBar; // 3
          ...
          begin
              ...
              begin
                ...
                begin
                  ...
                  begin
                    ...
                    begin
                      if i = 0 then
                        Routines_GenerateError('Пользователя с указанными логином и паролем не существует! ' + sLineBreak +
                          'Проверьте текущий язык ввода, состояние клавиши Caps Lock и правильность набора.',
                          sErrorMessage, bError)
                      else
                      begin
                        if i > 1 then
                        begin
                          LogThis('Количество строк выборки (' + IntToStr(i) + ')не соответствует требуемому (1)!',
                            LogGroupGUID, lmtError);
                          Routines_GenerateError
                            ('Найдено более одной учётной записи с указанными логином и паролем! Обратитесь к администратору!',
                            sErrorMessage, bError);
                        end
                        else
                        begin
                          ...
                          begin
                            if ResultRow[1] = '1' then
                              Routines_GenerateError
                                ('В настоящий момент данная учётная запись отключена! Обратитесь к администратору!',
                                sErrorMessage, bError)
                            else
                              with CurrentUser do
                              begin
            ...
            begin
              ...
              begin
                ...
                MySQL_CheckTableExistance(LogGroupGUID, Configuration.OverseerServer,
                  Configuration.OverseerServer.sMySQLDatabase, bError, sErrorMessage, '_actions');
                StepProgressBar; // 6
                ...
                begin
                  ...
                  MySQL_CheckTableExistance(LogGroupGUID, Configuration.OverseerServer,
                    Configuration.OverseerServer.sMySQLDatabase, bError, sErrorMessage, '_permissions');
                  StepProgressBar; // 7
                  ...
                  begin
                    // блокирование таблиц
                    MySQL_LockTables(LogGroupGUID, Configuration.OverseerServer, bError, sErrorMessage,
                      Configuration.OverseerServer.sMySQLDatabase + '._actions READ, _permissions READ');
                    StepProgressBar; // 8
                    ...
                    if not bError then
                      ...
                      begin
                        ...
                        LogThis('<< Выполнение операции получения полного списка действий завершено.', LogGroupGUID,
                          lmtDebug);
                        StepProgressBar; // 9
                        ...
                        begin
                          ...
                          // только в случае полного успеха логирования пользователя выставляем флаг логирования
                ...
                LogThis('<< Выполнение операции получения прав доступа текущего пользователя завершено.', LogGroupGUID,
                  lmtDebug);
              end;
            end;
          end;
...
begin
  LogThis('Установлен режим "' + Routines_GetConditionalMessage(iBusyCounter > 0, 'Занято', 'Готово') + '".', aLogGroupGUID,
    lmtDebug);
...
function TMainForm.Do_SIC_01(const aStartDate, aStopDate: TDate; const aOpenWithBrowser: Boolean;
  const aPhonesList: TStringList; const aShowUnserved: Boolean): Boolean;
const
...
function TMainForm.Do_SIC_03(const aStartDate, aStopDate: TDate; const aOpenWithBrowser: Boolean;
  const aNetIndex: Integer): Boolean;
const
...
function TMainForm.Do_SIC_07(const aStartDate, aStopDate: TDate; const aOpenWithBrowser: Boolean;
  const aGroupByPeriod: TGroupByPeriod; const aCalculateWeekFromMonday: Boolean): Boolean;
const
  LogGroupGUID: string = '{71E749C1-C998-43D1-B806-3BFD243D3BFA}';
var
...
function TMainForm.Do_SIC_04(const aStartDate, aStopDate: TDate; const aOpenWithBrowser: Boolean;
  const aGroupByPeriod: TGroupByPeriod; const aCalculateWeekFromMonday, aAllUsers, aCreateDetailedReport, aShowSQLQueries,
  aShowIdleUsers: Boolean; const aSelectedUserList: TStringList): Boolean;
const
...
function TMainForm.Do_SIC_05(const aStartDate, aStopDate: TDate; const aOpenWithBrowser: Boolean;
  const aGroupByPeriod: TGroupByPeriod; const aServiceIndex: Integer; const aCalculateWeekFromMonday: Boolean): Boolean;
const
...
function TMainForm.Do_SIC_06(const aStartDate, aStopDate: TDate; const aOpenWithBrowser: Boolean;
  const aServiceIndex: Integer): Boolean;
const
...
function TMainForm.Do_SIC_10(const aStartDate, aStopDate: TDate; const aOpenWithBrowser: Boolean;
  const aGroupByPeriod: TGroupByPeriod; const aCalculateWeekFromMonday, aAllUsers, aCreateDetailedReport: Boolean;
  const aCCID: Integer; const aSelectedUserList: TStringList): Boolean;
const
...
begin
  ...
  // создание нового потока в котором будут выполняться все действия по формированию отчёта
  with TThread_Do_SIC_10.Create('{1726AE8E-E2E2-4051-B375-8CC618EC7FBE}', aStartDate, aStopDate, aOpenWithBrowser, aAllUsers,
    aCalculateWeekFromMonday, aCreateDetailedReport, CurrentUser, Configuration, aCCID, aGroupByPeriod, aSelectedUserList) do
    try
      Start; // запускаем выполнение потока
    except
      on Exception do
...
begin
  ProcedureHeader('Процедура реакции на переключение состояния флажка "' + cbSummaryReport_SIC_09.Caption + '"',
    LogGroupGUID);

  cbOldReport_SIC_09.Enabled := not cbSummaryReport_SIC_09.Checked;
  if not cbOldReport_SIC_09.Enabled then
    cbOldReport_SIC_09.Checked := False;
  LogThis('Флажок "' + cbSummaryReport_SIC_09.Caption + '" ' + Routines_GetConditionalMessage(cbSummaryReport_SIC_09.Checked,
    'включён', 'отключён') + '.', LogGroupGUID, lmtDebug);
...
begin
  ...
  if not cbSummaryReport_SIC_09.Enabled then
    cbSummaryReport_SIC_09.Checked := False;
  LogThis('Флажок "' + cbOldReport_SIC_09.Caption + '" ' + Routines_GetConditionalMessage(cbOldReport_SIC_09.Checked,
    'включён', 'отключён') + '.', LogGroupGUID, lmtDebug);

  ProcedureFooter(LogGroupGUID);
end;

function TMainForm.Do_SIC_09(const aStartDate, aStopDate: TDate; const aOpenWithBrowser: Boolean; const aNetIndex: Integer;
  const aSummaryReport, aOldReport: Boolean; const aSelectedServicesList: TStringList): Boolean;
const
...
procedure TMainForm.MySQL_UnlockTables(const aLogGroupGUID: string;
  const aTMySQLServerConnectionDetails: TMySQLServerConnectionDetails; var bError: Boolean; var sErrorMessage: string);
var
  q: string;
begin
  LogThis('>> Выполняется операция разблокирования всех ранее заблокированных таблиц базы данных...', aLogGroupGUID,
    lmtDebug);
...
procedure TMainForm.MySQL_CloseConnection(const aLogGroupGUID: string;
  var aTMySQLServerConnectionDetails: TMySQLServerConnectionDetails; var bError: Boolean; var sErrorMessage: string);
begin
...
procedure TMainForm.MySQL_LockTables(const aLogGroupGUID: string;
  const aTMySQLServerConnectionDetails: TMySQLServerConnectionDetails; var bError: Boolean; var sErrorMessage: string;
  const aTablesList: string);
var
  q: string;
...
function TMainForm.MySQL_UpdateRecords(const aLogGroupGUID: string;
  const aTMySQLServerConnectionDetails: TMySQLServerConnectionDetails; var bError: Boolean; var sErrorMessage: string;
  const aQuery: string): Integer;
begin
  Result := -1;
  ...
  begin
    ...
    begin
      ...
      if mysql_real_query(aTMySQLServerConnectionDetails.hConnection, PAnsiChar(AnsiString(aQuery)), Length(aQuery)) <> 0
      then
        Routines_GenerateError('Возникла ошибка при выполнении последнего SQL-запроса!', sErrorMessage, bError)
      else
      begin
        ...
        if Result < 0 then
          Routines_GenerateError('Количество обработанных строк (' + IntToStr(Result) +
            ') не соответствует требуемому (>=0)!', sErrorMessage, bError)
        else
...
procedure TMainForm.MySQL_OpenConnection(const aLogGroupGUID: string;
  var aTMySQLServerConnectionDetails: TMySQLServerConnectionDetails; var bError: Boolean; var sErrorMessage: string);
begin
  with aTMySQLServerConnectionDetails do
  begin
    ...
    if bConnected then
      Routines_GenerateError('Была предпринята попытка подключения к подключенному в настоящий момент серверу MySQL!',
        sErrorMessage, bError)
    else
    begin
      ...
      begin
        ...
        bConnected := mysql_real_connect(hConnection, PAnsiChar(AnsiString(sMySQLHost)), PAnsiChar('overseer'),
          PAnsiChar(''), PAnsiChar(AnsiString(sMySQLDatabase)), iMySQLPort, nil,
          Integer(bMySQLCompress) * CLIENT_COMPRESS) <> nil;
        if not bConnected then
          Routines_GenerateError('Возникла ошибка при попытке подключения к серверу MySQL!' +
            Routines_GetMySQLErrorInfo(aTMySQLServerConnectionDetails), sErrorMessage, bError)
        else
          LogThis('Подключение к MySQL-серверу ' + aTMySQLServerConnectionDetails.sMySQLHost + ' выполнено успешно.',
            aLogGroupGUID, lmtDebug);
      end;
    end;
    LogThis('<< Выполнение операции подключения к MySQL-серверу ' + aTMySQLServerConnectionDetails.sMySQLHost +
      ' завершено.', aLogGroupGUID, lmtDebug);
  end;
end;

procedure TMainForm.MySQL_CheckTableExistance(const aLogGroupGUID: string;
  const aTMySQLServerConnectionDetails: TMySQLServerConnectionDetails; const aMySQLDatabase: string; var bError: Boolean;
  var sErrorMessage: string; const aTableName: string);
var
  ResultSet: PMYSQL_RES;
  ...
  i: Integer;
begin
  LogThis('>> Выполняется операция проверки наличия таблицы данных "' + aTableName + '" в базе данных...', aLogGroupGUID,
    lmtDebug);
  if not aTMySQLServerConnectionDetails.bConnected then
    Routines_GenerateError('Для выполнения операции необходимо подключение к серверу MySQL!', sErrorMessage, bError)
  else
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          begin
            if i <> 1 then
              Routines_GenerateError('Количество строк выборки (' + IntToStr(i) + ') не соответствует требуемому (1)!',
                sErrorMessage, bError)
            else
            begin
  ...
  LogThis('<< Выполнение операции проверки наличия таблицы данных "' + aTableName + '" в базе данных завершено.',
    aLogGroupGUID, lmtDebug);
end;

procedure TMainForm.MySQL_CreateTable(const aLogGroupGUID: string;
  const aTMySQLServerConnectionDetails: TMySQLServerConnectionDetails; var bError: Boolean; var sErrorMessage: string;
  const aQuery: string);
begin
...
procedure TMainForm.MySQL_DropTable(const aLogGroupGUID: string;
  const aTMySQLServerConnectionDetails: TMySQLServerConnectionDetails; var bError: Boolean; var sErrorMessage: string;
  const aQuery: string);
begin
...
function TMainForm.MySQL_InsertRecords(const aLogGroupGUID: string;
  const aTMySQLServerConnectionDetails: TMySQLServerConnectionDetails; var bError: Boolean; var sErrorMessage: string;
  const aQuery: string): Integer;
begin
  Result := -1;
  ...
  begin
    ...
    begin
      ...
      begin
        Routines_GenerateError('Возникла ошибка при выполнении последнего SQL-запроса! ' +
          mysql_error(aTMySQLServerConnectionDetails.hConnection), sErrorMessage, bError);
      end
      else
      begin
        LogThis('Запрос выполнен успешно.', aLogGroupGUID, lmtDebug);
...
begin
  ...
  for i := 0 to aPayoffList.Count - 1 do
    ...
    begin
      ...
      aDate := EncodeDate(StrToIntDef(Copy(aIRDTableRecord.datum, 7, 4), 1900),
        StrToIntDef(Copy(aIRDTableRecord.datum, 4, 2), 1), StrToIntDef(Copy(aIRDTableRecord.datum, 1, 2), 1));
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            MySQL_CheckTableExistance(LogGroupGUID, Configuration.DBServer,
              Configuration.DBServer.sMySQLDatabase_IRD_Location, bError, sErrorMessage,
              'ird_' + FormatDateTime('yyyymmdd', aDate));

            if not bError then
            begin
              MySQL_UpdateRecords(LogGroupGUID, Configuration.DBServer, bError, sErrorMessage, //
                'UPDATE ' + Configuration.DBServer.sMySQLDatabase_IRD_Location + '.ird_' + FormatDateTime('yyyymmdd',
                aDate) + ' ' //
        ...
        begin
          ...
          begin
            ...
            begin
              // блокирование таблицы
              MySQL_LockTables(LogGroupGUID, Configuration.StatServer, bError, sErrorMessage,
                Configuration.StatServer.sMySQLDatabase_IRD_Location + '.ird_' + FormatDateTime('yyyymmdd', aDate) +
                ' WRITE');

              if not bError then
              begin
                MySQL_UpdateRecords(LogGroupGUID, Configuration.StatServer, bError, sErrorMessage, //
                  'UPDATE ' + Configuration.StatServer.sMySQLDatabase_IRD_Location + '.ird_' + FormatDateTime('yyyymmdd',
                  aDate) + ' ' //
                  + 'SET rm="-' + aIRDTableRecord.rm + '" ' //
                  + 'WHERE' //
                  + ' ani="' + aIRDTableRecord.ani + '" AND' //
                ...
                begin
                  ...
                  begin
                    ...
                    begin
                      ...
                      begin
                        MySQL_UpdateRecords(LogGroupGUID, Configuration.DBServer, bError, sErrorMessage, //
                          'UPDATE ' + Configuration.DBServer.sMySQLDatabase_IRD_Location + '.ird_' +
                          FormatDateTime('yyyymmdd', aDate) + ' ' //
                          + 'SET rm="-' + aIRDTableRecord.rm + '" ' //
                          + 'WHERE' //
                          + ' ani="' + aIRDTableRecord.ani + '" AND' //
                          + ' ddi="' + aIRDTableRecord.ddi + '" AND' //
  ...
  begin
    Application.ProcessMessages;
    MessageBox(Handle, PWideChar('Действие выполнено успешно.'), PWideChar('OVERSEER - Информация'),
...
begin
  ...
  begin
    ...
    try
      ...
      if sl.Count = 4 then
      begin
        if ((sl[0] = IntToStr(WMCD_THREADLOG)) or (sl[0] = IntToStr(WMCD_MODALLOG))) and
          ((sl[1] = 'ERROR') or (sl[1] = 'WARNING') or (sl[1] = 'INFO') or (sl[1] = 'SQL') or (sl[1] = 'DEBUG')) then
        begin
          if sl[1] = 'ERROR' then
            MessageType := lmtError
          else
