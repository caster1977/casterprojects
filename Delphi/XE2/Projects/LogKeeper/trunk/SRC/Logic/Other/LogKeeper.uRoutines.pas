...
procedure Routines_CutStringByLimiterToStringList(const Source: string; var Destination: TStringList;
  const Limiter: char);
...
begin
  Result := Routines_GetConditionalMessage(mysql_errno(aTMySQLServerConnectionDetails.hConnection) <> 0,
    sLineBreak + sLineBreak + 'Код ошибки: ' + sLineBreak +
    IntToStr(mysql_errno(aTMySQLServerConnectionDetails.hConnection)) + sLineBreak + sLineBreak +
    'Наименование ошибки: ' + sLineBreak + string(mysql_error(aTMySQLServerConnectionDetails.hConnection)), '');
end;
...
begin
  ...
  begin
    ...
    try
      ...
      if AddCommas then
        Routines_NormalizeStringForQuery := '"' + string(System.AnsiStrings.StrPas(z)) + '"'
      else
        Routines_NormalizeStringForQuery := string(System.AnsiStrings.StrPas(z));
    finally
      if z <> nil then
