...
begin
  ...
  LogDebug(Format(TEXT_ACTION_SWITCHED, [Action_Send.Caption, CommonFunctions.GetConditionalString(Action_Send.Enabled,
    TEXT_ACTION_ON, TEXT_ACTION_OFF)]));

  Action_Cancel.Enabled := Action_Send.Enabled;
  Action_Cancel.Visible := btnSend_btnCancel.Action = Action_Cancel;
  LogDebug(Format(TEXT_ACTION_SWITCHED, [Action_Cancel.Caption, CommonFunctions.GetConditionalString(Action_Cancel.Enabled,
    TEXT_ACTION_ON, TEXT_ACTION_OFF)]));
  Application.ProcessMessages;
end;
...
begin
  ...
  LogDebug(Format(TEXT_CONNECTION_SWITCHED, [CommonFunctions.GetConditionalString(FConnectedToServer, TEXT_CONNECTION_ON,
    TEXT_CONNECTION_OFF)]));
  Application.ProcessMessages;
end;
...
begin
  ...
  begin
    ...
    // отправляем сигнал серверу об отмене передачи файла
...
resourcestring
  ...
  resourcestring
    ...
    TEXT_ERROR_READING_FILE =
      'Не удалось открыть для чтения указанный файл! Файл уже открыт другой программой либо недостаточно прав доступа.';
