...
begin
  ...
  if (GetWindowLong(h, GWL_STYLE) and WS_VSCROLL) = WS_VSCROLL then
    lvLog.Column[1].Width := lvLog.Width - (lvLog.BevelWidth * 2) - 2 - GetSystemMetrics(SM_CXVSCROLL) -
      lvLog.Column[0].Width
  else
...
begin
  i := -1;
  if (((lmtError in Configuration.KeepLogTypes) and (aMessageType = lmtError)) or
    ((lmtWarning in Configuration.KeepLogTypes) and (aMessageType = lmtWarning)) or
    ((lmtInfo in Configuration.KeepLogTypes) and (aMessageType = lmtInfo)) or ((lmtDebug in Configuration.KeepLogTypes) and
...
begin
  ...
  LogInfo(Format(TEXT_STATUSBAR_SWITCHED, [CommonFunctions.GetConditionalString(StatusBar1.Visible, TEXT_STATUSBAR_ON,
    TEXT_STATUSBAR_OFF)]));
end;
...
begin
  ...
  LogDebug(Format(TEXT_CONNECTION_SWITCHED, [CommonFunctions.GetConditionalString(FClientConnected, TEXT_CONNECTION_ON,
    TEXT_CONNECTION_OFF)]));
...
begin
  MessageBox(aHandle, PWideChar(aErrorMessage), PWideChar(Format(TEXT_ERROR, [TEXT_MAINFORM_CAPTION])),
...
resourcestring
  ...
  TEXT_DESTINATION_FOLDER_NOT_EXISTS =
    'Папка приёма файлов не существует! Проверьте правильность указанного в настройках пути [%s]!';
var
  sErrorMessage: string;
  bError: Boolean;
begin
  ...
  begin
    ...
    CommonFunctions.GenerateError(Format(TEXT_DESTINATION_FOLDER_NOT_EXISTS, [Configuration.DestinationFolder]),
...
begin
  ...
  try
    ...
    begin
      ...
      if MessageBox(Handle, PWideChar(TEXT_DO_YOU_WANNA_RETRY_CONFIGURATION_SAVING),
        PWideChar(Format(TEXT_WARNING, [MainForm.Caption])), MB_OKCANCEL + MB_ICONWARNING + MB_DEFBUTTON1) = IDOK then
        try
          Screen.Cursor := crHourGlass;
          try
            Configuration.Save;
...
begin
  ...
  if Configuration.ShowConfirmationOnQuit then
    CanClose := MessageBox(Handle, PWideChar(TEXT_ARE_YOU_SURE_YOU_WANNA_QUIT),
      PWideChar(Format(TEXT_QIUT_CONFIRMATION, [TEXT_MAINFORM_CAPTION])), MB_OKCANCEL + MB_ICONQUESTION +
      MB_DEFBUTTON2) = IDOK
  else
    CanClose := True;
  Application.ProcessMessages;
  ...
  begin
    ...
    if FClientConnected then
      // если соединение с клиентом установлено, отправляем клиенту уведомление о завершении работы сервера
...
procedure TMainForm.WndProc(var Message: TMessage);
  ...
  begin
    ...
    // отправляем клиенту размер блока общей памяти для обмена данными
  ...
  begin
    ...
    begin
      FChunkedFile := TChunkedFileClass.Create(Configuration.DestinationFolder + FFileName,
        Configuration.SharedMemSize, aSize);
      ...
      // требуем от клиента указанный блок файла
  ...
  begin
    ...
    PostMessage(FClientHandle, WM_SERVER, WPARAM_SERVER_WANNA_CRC32, FChunkedFile.Index);
    // требуем от клиента CRC32 указанной порции данных файла
      ...
      begin
        ...
        try
          try
            ...
            // уведомляем клиент об успешном окончании передачи файла
            ...
            begin
              ...
              // уведомляем клиент об ошибке при передаче файла
    ...
    if message.WParam = WPARAM_CLIENT_SHUTDOWN then // клиент сообщает о своём отключении от сервера
      ...
      if FClientConnected then
        case message.WParam of
          ...
          WPARAM_CLIENT_SENDS_FILESIZE:
            // клиент отправляет размер файла (LPARAM = размер символьного представления размера файла в байтах)
...
begin
  ...
  LogInfo(Format(TEXT_SCROLLLOG_SWITCHED, [CommonFunctions.GetConditionalString(Configuration.ScrollLogToBottom,
    TEXT_SCROLLLOG_ON, TEXT_SCROLLLOG_OFF)]));
end;
...
var
  ...
  begin
    ...
    imConnectionState.SetBounds(PanelRect.Left + 2, PanelRect.Top + 1, PanelRect.Right - PanelRect.Left - 4,
