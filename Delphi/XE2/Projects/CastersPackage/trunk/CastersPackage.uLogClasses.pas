...
{$IFDEF DEBUG}
  if Assigned(FOwner) then
    NameThreadForDebugging(AnsiString(FOwner.FOwner.Owner.Name + '.' + GetOwner.GetNamePath + '.' +
      Self.ClassName));
{$ENDIF}
end;
    ...
    begin
      ...
      begin
        ...
        try
          ...
          XMLFile.DOMDocument.insertBefore(XMLFile.DOMDocument.createProcessingInstruction('xml',
            'version="1.0" encoding="UTF-8"'), XMLFile.DOMDocument.childNodes[0]);
                    ...
                    if FOwner is TLogFile then
                      ...
                      begin
                        Synchronize(
                          procedure
                          begin
                            XMLMessageSource := XMLLogkeeperdata.Message[0];
                            XMLMessageDestination := TLogFile(FOwner).FLogKeeperData.Add;
                            XMLMessageDestination.Index := XMLMessageSource.Index;
                            XMLMessageDestination.Date.Day := XMLMessageSource.Date.Day;
                            XMLMessageDestination.Date.Month := XMLMessageSource.Date.Month;
                            XMLMessageDestination.Date.Year := XMLMessageSource.Date.Year;
                            XMLMessageDestination.Time.Hour := XMLMessageSource.Time.Hour;
                            XMLMessageDestination.Time.Minute := XMLMessageSource.Time.Minute;
                            XMLMessageDestination.Time.Second := XMLMessageSource.Time.Second;
                            XMLMessageDestination.Time.Msecond := XMLMessageSource.Time.Msecond;
                            XMLMessageDestination.Host := XMLMessageSource.Host;
                            XMLMessageDestination.Application.Handle :=
                              XMLMessageSource.Application.Handle;
                            XMLMessageDestination.Application.Filename :=
                              XMLMessageSource.Application.Filename;
                            XMLMessageDestination.Application.FilePath :=
                              XMLMessageSource.Application.FilePath;
                            XMLMessageDestination.Application.Form.Handle :=
                              XMLMessageSource.Application.Form.Handle;
                            XMLMessageDestination.Application.Form.Name :=
                              XMLMessageSource.Application.Form.Name;
                            XMLMessageDestination.Application.Method.Guid :=
                              XMLMessageSource.Application.Method.Guid;
                            XMLMessageDestination.Application.User :=
                              XMLMessageSource.Application.User;
                            XMLMessageDestination.MessageType := XMLMessageSource.MessageType;
                            XMLMessageDestination.Text := XMLMessageSource.Text;
                          end);
