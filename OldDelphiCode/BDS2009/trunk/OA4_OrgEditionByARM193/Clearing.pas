...
begin
      ...
      try
        ...
        if not bError then
          begin
            ...
            q:='SELECT msr_id FROM '+
              MainForm.Configuration.sMySQLDatabase+'._msr WHERE msr_erased="1" '+
      ...
      try
        ...
        if not bError then
          begin
            ...
            pbClearingOrganizationsDetail.Position:=pbClearingOrganizationsDetail.Min;
            Application.ProcessMessages;
            q:='SELECT org_id FROM '+
              MainForm.Configuration.sMySQLDatabase+'._org WHERE org_erased="1" '+
  ...
  MainForm.btbtnSearchClick(Sender);
end;
...
procedure TClearingForm.FormKeyUp(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
begin
  b:=
  (chkbxClearingPhoneTable.Enabled and chkbxClearingPhoneTable.Checked) or
  (chkbxClearingOrganizationTable.Enabled and chkbxClearingOrganizationTable.Checked) or
  (chkbxClearingMeasureTable.Enabled and chkbxClearingMeasureTable.Checked) or
  (chkbxClearingMessageTable.Enabled and chkbxClearingMessageTable.Checked) or
  (chkbxClearingEventTable.Enabled and chkbxClearingEventTable.Checked) or
