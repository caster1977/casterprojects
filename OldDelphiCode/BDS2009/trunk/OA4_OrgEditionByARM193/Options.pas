...
uses
	...
	Dialogs, ComCtrls, ExtCtrls, StdCtrls, Grids, Buttons, Main;
...
begin
  if (sgAutoReplaceList.Selection.Top+1)<=(sgAutoReplaceList.RowCount-1) then
    TStringGridX(sgAutoReplaceList).MoveRow(sgAutoReplaceList.Selection.Top, sgAutoReplaceList.Selection.Top+1);
end;
...
procedure TOptionsForm.chkbxAddEditPhoneFormPositionByCenterClick(
  Sender: TObject);
begin
...
procedure TOptionsForm.chkbxCreateViewPostFormPositionByCenterClick(
  Sender: TObject);
begin
...
procedure TOptionsForm.chkbxMaintenanceFormPositionByCenterClick(
  Sender: TObject);
begin
...
procedure TOptionsForm.chkbxSetPasswordFormPositionByCenterClick(
  Sender: TObject);
begin
...
begin
  ...
  edbxMainFormPositionX.Enabled:=not (chkbxMainFormPositionByCenter.Checked or chkbxStartupFullScreen.Checked);
  if edbxMainFormPositionX.Enabled then
    edbxMainFormPositionX.Text:=IntToStr(MainForm.Configuration.fpMainForm.x)
  else edbxMainFormPositionX.Clear;

  edbxMainFormPositionY.Enabled:=not (chkbxMainFormPositionByCenter.Checked or chkbxStartupFullScreen.Checked);
  if edbxMainFormPositionY.Enabled then
    edbxMainFormPositionY.Text:=IntToStr(MainForm.Configuration.fpMainForm.y)
  else edbxMainFormPositionY.Clear;
...
procedure TOptionsForm.chkbxViewPostListFormPositionByCenterClick(
  Sender: TObject);
begin
...
procedure TOptionsForm.edbxMainFormWidthKeyPress(Sender: TObject;
  var Key: Char);
begin
...
begin
  ...
  { TODO : исправить процедуру перемещения формы }
  bNormalizeNow:=True;
