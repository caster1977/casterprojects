...
begin
  ...
  with TCreateViewPostForm.Create(Owner) do
    try
      ...
      else MainForm.LogThis('Список пользователей получен успешно...',GroupGUID,lmtInfo);
      ...
      if mr=mrOk then
        begin
          ...
          try
            ...
            else MainForm.LogThis('Список идентификаторов получен успешно...',GroupGUID,lmtInfo);
            if ResultSet<>nil then
              begin
                MainForm.LogThis('Освобождение ресурсов, занятых результатом выборки.',GroupGUID,lmtInfo);
                mysql_free_result(ResultSet);
...
begin
  ...
  else MainForm.LogThis('Удаление сообщения отменено пользователем...',GroupGUID,lmtInfo);
end;
