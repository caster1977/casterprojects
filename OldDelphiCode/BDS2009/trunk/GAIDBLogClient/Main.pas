...
begin
  ...
  gsFileVersionInfo1.Filename:=Application.ExeName;
	...
	s:=ExtractFilePath(ExpandFileName(Application.ExeName))+'GAIDB_UnSendedLog.dat';
...
begin
  ...
  // и сделать CanClose:=False;
	if (FindWindow('TMainForm','GAIDB')<>0) then // если не было найдено окно операторской программы
    begin
      MessageBox(MainForm.Handle,
				PChar('Работу данной программы завершать нельзя, т.к. в'+#10#13+'данный момент она используется программой "GAIDB"!'),
  ...
  // и завершить работу программы, установив CanClose:=True;
    ...
    begin
      ...
      if ListBox1.Items.Count>0 then
				ListBox1.Items.SaveToFile(ExtractFilePath(ExpandFileName(Application.ExeName))+'GAIDB_UnSendedLog.dat'); // если остались непереданные сообщения - записать их в файл
...
begin
	if bAboutWindowExist then
    begin
			SetForegroundWindow(FindWindow('TAboutForm','About "GAIDB Log Server"...'));
    end
	else
		begin
  ...
  if i=0 then // если окно основной программы пропало
    begin
      ...
      if not bStartingNormalShutdown then // если не установлена переменная нормального завершения работы
        begin
					a:=GlobalFindAtom('GAIDBNormalShutdown'); // пытаемся найти атом
      ...
      if not bStartingNormalShutdown then // если в итоге переменная нормального завершения работы так и не установлена
        begin
		        ...
		        ''+#09+ // текущий залогированный пользователь
						'Обнаружено аварийное завершение работы программы "GAIDB"!'); // само сообщение
//          MessageBox(MainForm.Handle, // и выдать сообщение на экран
//            PChar('Обнаружено аварийное завершение работы программы "GAIDB"!'),
//            PChar(MainForm.Caption+' - '+'Внимание!'),
//            MB_OK+MB_ICONWARNING+MB_DEFBUTTON1);
					if MessageBox(Application.Handle, PChar('Работа приложения "GAIDB" была некорректно завершена!'+#13#10+'Вы хотите вновь запустить программу "GAIDB"?'),PChar(MainForm.Caption+' - '+'Ошибка!'),MB_YESNO+MB_ICONERROR+MB_DEFBUTTON1)=IDYES then // если пользователь решает всё же перезапустить приложение
            begin
              {$IFDEF DEBUG}
							s:='D:\My Documents\Borland Studio Projects\GAIDB\GAIDB.exe';
							{$ELSE}
              s:=ExtractFilePath(ExpandFileName(Application.ExeName))+'GAIDB.exe';
              {$ENDIF}
